"""Centralized logging setup for Cadoo Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.cadoo/logs/`` (profile-aware via ``get_cadoo_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``cadoo_cli.web_server``,
    ``cadoo_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Cadoo (TUI, gateway, ``hy_memory`` server, MCP
...
from cadoo_constants import get_config_path, get_cadoo_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_cadoo_session_injector", False):
    ...
    _session_record_factory._cadoo_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``cadoo logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``cadoo logs --component gateway``.
    "gateway": ("gateway", "cadoo_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("cadoo_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "cadoo_cli.web_server",
        "cadoo_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Cadoo logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    cadoo_home
        Override for the Cadoo home directory.  Falls back to
        ``get_cadoo_home()`` (profile-aware).
    log_level
    ...
    home = cadoo_home or get_cadoo_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_cadoo_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._cadoo_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the cadoo group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from cadoo_cli.config import is_managed
