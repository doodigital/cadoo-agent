#!/bin/sh
# shellcheck shell=sh
# /opt/cadoo/bin/cadoo — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `cadoo` user (UID 10000). When an operator runs `docker exec <c> cadoo ...`
# the default UID is root (0), and any file the command writes under
# $CADOO_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> cadoo login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Cadoo is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> cadoo
...
# Fix
# ---
# This shim sits at /opt/cadoo/bin/cadoo and is placed earliest on PATH.
# When invoked as root, it drops to the cadoo user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $CADOO_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user cadoo`, kanban subagents, etc. — it short-circuits
...
# (/opt/cadoo/.venv/bin/cadoo), so the second hop cannot re-enter this
...
# state via the cadoo CLI. Default is to drop.

set -e

REAL=/opt/cadoo/.venv/bin/cadoo

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "cadoo-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user cadoo`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi
...
# Root, no opt-out. Drop to the cadoo user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "cadoo-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "cadoo-shim: re-run with --user cadoo or set HERMES_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the cadoo user's home before dropping privileges. Without
