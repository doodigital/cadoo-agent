...
#
# Wired into the image as /etc/cont-init.d/01-cadoo-setup by the
...
CADOO_HOME="${CADOO_HOME:-/opt/data}"
INSTALL_DIR="/opt/cadoo"

# Drop to cadoo via s6-setuidgid, but skip it when already non-root.
as_hermes() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid cadoo "$@"; }
...
# starts non-root. The baked install tree under /opt/cadoo is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $CADOO_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# `id -u cadoo` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# cadoo UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u cadoo)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-cadoo UID).
...
and the baked /opt/cadoo install tree is intentionally root-owned and
...
The image remaps the cadoo user to that UID/GID at boot and chowns the data
...
# --- Bootstrap CADOO_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid cadoo mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom CADOO_HOME paths whose parents only
# root can create (e.g. `CADOO_HOME=/home/cadoo/.cadoo` in a Compose
...
if [ -n "${HERMES_UID:-}" ] && validate_uid_gid "$HERMES_UID" && [ "$HERMES_UID" != "$(id -u cadoo)" ]; then
    echo "[stage2] Changing cadoo UID to $HERMES_UID"
    usermod -u "$HERMES_UID" cadoo
fi
if [ -n "${HERMES_GID:-}" ] && validate_uid_gid "$HERMES_GID" && [ "$HERMES_GID" != "$(id -g cadoo)" ]; then
    echo "[stage2] Changing cadoo GID to $HERMES_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$HERMES_GID" cadoo 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised cadoo user (UID 10000)
...
# cadoo process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes cadoo. Idempotent across container
...
#   - cadoo is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G cadoo 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] cadoo already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" cadoo 2>/dev/null; then
        echo "[stage2] Added cadoo to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group cadoo failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When HERMES_UID is remapped or the top-level $CADOO_HOME isn't owned by
# the runtime cadoo UID, restore ownership to cadoo — but ONLY for the
# directories cadoo actually writes to. The full $CADOO_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of cadoo-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_hermes_uid=$(id -u cadoo)
needs_chown=false
if [ "$(stat -c %u "$CADOO_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $CADOO_HOME (targeted) to cadoo ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $CADOO_HOME: chown the directory itself (not its contents)
    # so cadoo can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown cadoo:cadoo "$CADOO_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $CADOO_HOME failed (rootless container?) — continuing"
    # Cadoo-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by cadoo (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$CADOO_HOME/$sub" ]; then
            chown -R cadoo:cadoo "$CADOO_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $CADOO_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi
...
# cadoo user. Hosted/container instances keep mutable state under
# $CADOO_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# HERMES_DISABLE_LAZY_INSTALLS=1. Keeping /opt/cadoo root-owned and
...
# Always reset ownership of $CADOO_HOME/profiles to cadoo on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> cadoo …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as cadoo and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$CADOO_HOME/profiles" ]; then
    chown -R cadoo:cadoo "$CADOO_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $CADOO_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged cadoo runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$CADOO_HOME/cron" ]; then
    chown -R cadoo:cadoo "$CADOO_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of cadoo-owned top-level state files on every boot.
# The targeted data-volume chown above only covers cadoo-owned
# *subdirectories*; loose state files living directly under $CADOO_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> cadoo …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged cadoo runtime then hits
...
# sweep so host-owned files in a bind-mounted $CADOO_HOME are never
...
# entries of cadoo_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$CADOO_HOME/$f" ]; then
        chown cadoo:cadoo "$CADOO_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the cadoo runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$CADOO_HOME/config.yaml" ]; then
    chown cadoo:cadoo "$CADOO_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$CADOO_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as cadoo user ---
# Run as cadoo via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $CADOO_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_hermes mkdir -p \
    "$CADOO_HOME/cron" \
    "$CADOO_HOME/sessions" \
    "$CADOO_HOME/logs" \
    "$CADOO_HOME/logs/gateways" \
    "$CADOO_HOME/hooks" \
    "$CADOO_HOME/memories" \
    "$CADOO_HOME/skills" \
    "$CADOO_HOME/skins" \
    "$CADOO_HOME/plans" \
    "$CADOO_HOME/workspace" \
    "$CADOO_HOME/home" \
    "$CADOO_HOME/pairing" \
    "$CADOO_HOME/platforms/pairing"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/cadoo/.install_method (see Dockerfile), NOT written here into
# $CADOO_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $CADOO_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.cadoo:/opt/data) and sometimes shared with a
...
# 'cadoo update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $CADOO_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/cadoo stamp, so deleting the data-dir copy is safe).
if [ -f "$CADOO_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$CADOO_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$CADOO_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$CADOO_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_hermes cp "$INSTALL_DIR/$src" "$CADOO_HOME/$dest"
    fi
}
...
if [ -f "$CADOO_HOME/.env" ]; then
    chown cadoo:cadoo "$CADOO_HOME/.env" 2>/dev/null || true
    chmod 600 "$CADOO_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $CADOO_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `cadoo update` runs for non-Docker installs,
...
if [ -f "$CADOO_HOME/config.yaml" ]; then
    s6-setuidgid cadoo "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$CADOO_HOME/auth.json" ] && [ -n "${HERMES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$HERMES_AUTH_JSON_BOOTSTRAP" > "$CADOO_HOME/auth.json"
    chown cadoo:cadoo "$CADOO_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$CADOO_HOME/auth.json"
fi
...
if [ ! -f "$CADOO_HOME/gateway_state.json" ] && \
        [ "${HERMES_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$CADOO_HOME/gateway_state.json"
    chown cadoo:cadoo "$CADOO_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$CADOO_HOME/gateway_state.json"
fi
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/cadoo/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Cadoo spawns for the
...
# main-wrapper.sh propagates it into the supervised ``cadoo`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-cadoo, dashboard, etc.).
