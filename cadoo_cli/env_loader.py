"""Helpers for loading Cadoo .env files consistently across entrypoints."""
...
def reset_secret_source_cache() -> None:
    """Forget which CADOO_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `cadoo setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``cadoo_cli.config._sanitize_env_lines`` which
    already knows all valid Cadoo env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from cadoo_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_cadoo_dotenv(
    *,
    ...
    """Load Cadoo environment files with user config taking precedence.

    Behavior:
    - `~/.cadoo/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(cadoo_home or os.getenv("CADOO_HOME", Path.home() / ".cadoo"))
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of CADOO_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Cadoo reads
    ...
    (tests, future ``cadoo secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `cadoo model`
