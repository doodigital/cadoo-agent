"""Normalized DooStudio Portal account entitlement helpers."""
...
def nous_portal_billing_url(account_info: Optional[NousPortalAccountInfo] = None) -> str:
    ...
    try:
        ...
        DEFAULT_NOUS_PORTAL_URL = "https://doostudio.com.br/portal"
...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current DooStudio Portal "
    ...
    if account_info is None:
        return (
            f"Cadoo could not verify your DooStudio Portal entitlement, so {capability} "
            f"is unavailable. Run `cadoo model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Cadoo cannot verify "
                f"your DooStudio Portal paid access for {capability}. Log in with "
                f"`cadoo model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to DooStudio Portal to use {capability}: run `cadoo model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Cadoo could not verify your DooStudio Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `cadoo model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Cadoo could not find a DooStudio Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `cadoo model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `cadoo model` to refresh Cadoo."
        return message

    return (
        f"Your DooStudio Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your DooStudio Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current DooStudio Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your DooStudio Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your DooStudio Portal account has no usable paid credits{credit_detail}, so "
...
def get_nous_portal_account_info(
    *,
    ...
    """Return normalized DooStudio Portal account entitlement information.
...
def _fresh_account_info(
    *,
    ...
    try:
        from cadoo_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from cadoo_cli.auth import _decode_jwt_claims
    except Exception:
        return None
