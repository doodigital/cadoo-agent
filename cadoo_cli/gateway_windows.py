...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m cadoo_cli.main gateway run --replace`` invocation) and
...
_TASK_DESCRIPTION = "Cadoo Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Cadoo profile."""
    from cadoo_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "cadoo_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during cadoo_cli boot.
    from cadoo_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\cadoo\\gateway-service\\<task_name>.cmd``
    (or ``<CADOO_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Cadoo installs stay self-contained).
    """
    _assert_windows()
    from cadoo_cli.config import get_cadoo_home

    script_dir = Path(get_cadoo_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``CADOO_HOME`` whenever it
    ...
    source checkout only if ``CADOO_HOME`` cannot be resolved yet.
    """
    from cadoo_cli.config import get_cadoo_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports CADOO_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m cadoo_cli.main [--profile X] gateway run``
    ...
    prog_args = [pythonw_path, "-m", "cadoo_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('CADOO_HOME')}) = {_quote_vbs_string(cadoo_home)}",
...
def _write_task_script() -> Path:
    ...
    cadoo_home = str(Path(get_cadoo_home()).resolve())
    profile_arg = _profile_arg(cadoo_home)

    content = _build_gateway_cmd_script(python_path, working_dir, cadoo_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, cadoo_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Cadoo builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "cadoo_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m cadoo_cli.main gateway run``
    ...
    log_dir = Path(get_cadoo_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from cadoo_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `cadoo -p <profile> gateway install` must be safe.
    ...
    from cadoo_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"cadoo {profile_arg} gateway start" if profile_arg else "cadoo gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: cadoo gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Cadoo gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: cadoo gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: cadoo gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from cadoo_cli.config import get_cadoo_home
        print(f"    type {Path(get_cadoo_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_cadoo_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from cadoo_cli.config import get_cadoo_home

    cadoo_home = Path(get_cadoo_home()).resolve()
    print()
    print("Next steps:")
    print("  cadoo gateway status                      # Check status")
    print(f"  type {cadoo_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Cadoo gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: cadoo gateway status")
                    return
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_cadoo_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: cadoo gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: cadoo gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from cadoo_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
...
def restart() -> None:
    ...
    """
    _assert_windows()
    from cadoo_cli.gateway import kill_gateway_processes
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `cadoo gateway status`."
        )
