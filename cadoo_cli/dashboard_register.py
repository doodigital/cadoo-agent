"""``cadoo dashboard register`` — register a self-hosted dashboard OAuth client.

Automates what a user otherwise does by hand: open the DooStudio Portal
``/local-dashboards`` page in a browser, click "register", copy the
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.cadoo/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh DooStudio Portal access token from the existing login
     (``~/.cadoo/auth.json``), refreshing it if needed. Fails fast with a
     "run `cadoo setup`" hint when the user isn't logged in.
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.cadoo/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from cadoo_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``cadoo dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "DooStudio Portal rejected the access token (401). "
                "Try `cadoo auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
        ...
        raise RuntimeError(
            f"Could not reach DooStudio Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from cadoo_cli.config import get_env_path
    ...
    print(
        ...
        "  `cadoo dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    cadoo dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from cadoo_cli.auth import AuthError, resolve_nous_access_token
    from cadoo_cli.config import get_env_value, is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `cadoo dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into DooStudio Portal.")
            print("  Run `cadoo setup` (or `cadoo auth login nous`) first, then retry.")
        else:
            print(f"✗ Could not resolve a DooStudio Portal access token: {exc}")
        sys.exit(1)
    ...
    default_portal = "https://doostudio.com.br/portal"
    ...
    # the portal, e.g. https://cadoo.example.com/auth/callback. At serve time
