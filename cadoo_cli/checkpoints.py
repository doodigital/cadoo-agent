"""`cadoo checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.cadoo/checkpoints/``.  Actions:

    cadoo checkpoints               # same as `status`
    cadoo checkpoints status        # total size, project count, breakdown
    cadoo checkpoints list          # per-project checkpoint counts + workdir
    cadoo checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    cadoo checkpoints clear [-f]    # nuke the entire base (asks first)
    cadoo checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    cadoo checkpoints
    cadoo checkpoints prune --retention-days 3 --max-size-mb 200
    cadoo checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: cadoo checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``cadoo checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `cadoo checkpoints` → status
