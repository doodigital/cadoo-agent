...
A system-level directory (default ``/etc/cadoo``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.cadoo/config.yaml`` and ``~/.cadoo/.env`` on a per-leaf-key basis.

This is DISTINCT from ``cadoo_cli.config.is_managed()`` / ``HERMES_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/cadoo")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/cadoo`` during tests so a real
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      ...
      2. ``/etc/cadoo`` — POSIX default, when it exists. Ignored under pytest so
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through cadoo_cli.config.load_config) to honor managed
    scope. Mirrors cadoo_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from cadoo_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
