"""cadoo hooks — inspect and manage shell-script hooks.

Usage::

    cadoo hooks list
    cadoo hooks test <event> [--for-tool X] [--payload-file F]
    cadoo hooks revoke <command>
    cadoo hooks doctor

Consent records live under ``~/.cadoo/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.cadoo/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``cadoo hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: cadoo hooks {list|test|revoke|doctor}")
        print("Run 'cadoo hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.cadoo/config.yaml.")
        print("See `cadoo hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `cadoo hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `cadoo hooks test` and `cadoo hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Cadoo wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `cadoo hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `cadoo hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `cadoo hooks doctor`.")
