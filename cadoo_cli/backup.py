"""
Backup and import commands for cadoo CLI.

`cadoo backup` creates a zip archive of the entire ~/.cadoo/ directory
(excluding the cadoo repo and transient files).

`cadoo import` restores from a backup zip, overlaying onto the current
CADOO_HOME root.
"""
...
# ``cadoo`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/cadoo/``
...
# CADOO_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "cadoo",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under CADOO_HOME) —
...
# File names that ``cadoo import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE CADOO_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside CADOO_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to cadoo root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``cadoo`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/cadoo/`` — must be preserved.
        if part == "cadoo" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside CADOO_HOME.
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"cadoo-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"cadoo-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_cadoo_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``cadoo`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "cadoo" and not is_root)
        ]
    ...
    # outside CADOO_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_cadoo_home()}."
        )
    ...
    print(f"\nRestore with: cadoo import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Cadoo backup.
    ...
    # Look for telltale files that a cadoo home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Cadoo backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.cadoo/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a cadoo dir name
        if prefix in {".cadoo", "cadoo"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_cadoo_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Cadoo configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under CADOO_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_cadoo_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # cadoo_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: cadoo profile list  (after installing cadoo)")

        # Guidance
        print()
        if not (hermes_root / "cadoo").is_dir():
            print("Note: The cadoo codebase was not included in the backup.")
            print("  If this is a fresh install, run: cadoo update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  cadoo -p {pname} gateway install")

        print("Done. Your Cadoo configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and cadoo backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to CADOO_HOME).
...
# — `cadoo update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(cadoo_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(cadoo_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside CADOO_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``cadoo update``.
    ...
    Args:
        ...
        cadoo_home: Override for the Cadoo home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(cadoo_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for cadoo backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_cadoo_home()}/state-snapshots/")
...
def _pre_update_backup_dir(cadoo_home: Optional[Path] = None) -> Path:
    home = cadoo_home or get_cadoo_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of CADOO_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<CADOO_HOME>/backups/pre-update-<timestamp>.zip`` and
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `cadoo claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of CADOO_HOME under ``backups/`` before a
    ``cadoo claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``cadoo import <archive>``.  Writes to
    ``<CADOO_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `cadoo import` and the
