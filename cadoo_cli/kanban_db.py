...
``<root>`` is the **shared Cadoo root** (the parent of any active
...
``cadoo -p <profile>`` joins the same board as the dispatcher that
...
the profile's Cadoo home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``cadoo kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.cadoo``. In Docker / custom
deployments where ``CADOO_HOME`` points outside ``~/.cadoo`` (e.g.
``/opt/cadoo``), ``<root>`` is ``CADOO_HOME``. Legacy env-var
...
coding workloads.  See ``docs/cadoo-kanban-v1-spec.pdf`` for the full
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active CADOO_HOME so dispatcher- and
...
# enough that kebab-case names like ``atm10-server`` or ``cadoo``
...
def kanban_home() -> Path:
    """Return the shared Cadoo root that anchors the kanban board.
    ...
    profile's ``CADOO_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``cadoo kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``cadoo kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``cadoo kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``cadoo kanban log`` unambiguous even when multiple
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``cadoo gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Cadoo treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # holder (e.g. an external `cadoo kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from cadoo_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See cadoo_state._WAL_INCOMPAT_MARKERS for detection logic.
                from cadoo_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``cadoo kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from cadoo_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``cadoo --skills ...``. Use this to pin a task to a
    ...
    # `cadoo --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``cadoo kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `cadoo kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``cadoo kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``cadoo kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``cadoo kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``cadoo kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Cadoo' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Cadoo-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``cadoo kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Cadoo checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Cadoo treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Cadoo creates/reuses that linked worktree. With
      no ``workspace_path``, Cadoo anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Cadoo uses
      ``wt/<task-id>``.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Cadoo
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``cadoo kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``cadoo kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Cadoo profile.
    ...
    if not rows:
        return False
    try:
        from cadoo_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from cadoo_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Cadoo profile.
        # `_default_spawn` invokes ``cadoo -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Cadoo
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `cadoo kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Cadoo CLI invocation."""
    # ``cadoo_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``cadoo`` package — there is no
    # ``cadoo`` package to import.
    return [sys.executable, "-m", "cadoo_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Cadoo shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Cadoo executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``cadoo`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("cadoo")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\cadoo.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m cadoo_cli.main`` — fallback for setups where
       Cadoo is launched from a venv and the ``cadoo`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hermes_bin`` for the same reason. Kept
    local (not imported from gateway) because ``cadoo_cli`` sits below
    ...
    hermes_bin = _safe_which_no_cwd("cadoo") if _IS_WINDOWS else shutil.which("cadoo")
    if hermes_bin:
...
def _resolve_worker_cli_toolsets(cadoo_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_cadoo_home_override(cadoo_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for CADOO_HOME=%r (%s)",
            cadoo_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``cadoo -p <profile> chat -q ...`` subprocess.
    ...
    # Inject CADOO_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `cadoo -p <name>` the
    # _apply_profile_override() runs *before* cadoo_constants is imported.
    # If CADOO_HOME is absent from the child's env, get_cadoo_home() falls
    # back to Path.home() / ".cadoo" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from cadoo_cli.profiles import resolve_profile_env
    try:
        env["CADOO_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # CADOO_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # `cadoo -p <assignee>` activates the profile, but the env var is
    ...
    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped CADOO_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("CADOO_HOME"))
    if worker_toolsets:
    ...
    # `cadoo kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`cadoo` executable not found on PATH. "
            "Install Cadoo Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``cadoo kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HERMES_PROFILE values like "cadoo-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Cadoo root exists

    Reads profile paths directly so this module has no import dependency on
    ``cadoo_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from cadoo_constants import get_default_hermes_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``cadoo kanban assignees`` for the terminal.
