"""
Cadoo Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.cadoo/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Cadoo PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing cadoo or cadoo PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Cadoo Agent" comment and following line
                if '# Cadoo Agent' in line or '# cadoo' in line:
                    skip_next = True
                    continue
                if skip_next and ('cadoo' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing cadoo
                if 'cadoo' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the cadoo wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "cadoo",
        Path("/usr/local/bin/cadoo"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains cadoo_cli reference)
                content = wrapper.read_text()
                if 'cadoo_cli' in content or 'cadoo' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(cadoo_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``cadoo`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Cadoo home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (cadoo_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``cadoo gateway run`` processes
    ...
    try:
        from cadoo_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from cadoo_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CADOO_HOME`` and ``HERMES_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\cadoo\git\cmd``, ``%LOCALAPPDATA%\cadoo\git\bin``,
#      ``%LOCALAPPDATA%\cadoo\git\usr\bin``, ``%LOCALAPPDATA%\cadoo\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\cadoo\git\`` and Node to
#      ``%LOCALAPPDATA%\cadoo\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``cadoo dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\cadoo\gateway-service\`` and sometimes
...
def _hermes_path_markers(cadoo_home: Path) -> list[str]:
    """Path-entry substrings that identify Cadoo-owned User-PATH entries."""
    root = str(cadoo_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare cadoo install dir.
    markers = [root + "\\cadoo", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if CADOO_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "cadoo".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-cadoo as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(cadoo_home: Path) -> list[str]:
    """Strip Cadoo-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hermes_path_markers(cadoo_home)
            kept: list[str] = []
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete CADOO_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CADOO_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CADOO_HOME directory.

    We shell out to ``cadoo -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CADOO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m cadoo_cli.main` so we don't depend on a `cadoo`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "cadoo_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's CADOO_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``cadoo uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$CADOO_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from cadoo_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(cadoo_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         🟣 Cadoo Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Cadoo Chat GUI installation was found.")
        print(f"  Checked: {cadoo_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Cadoo agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(cadoo_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Cadoo agent at {cadoo_home / 'cadoo'}")
        print(f"  • Your config, sessions, and secrets under {cadoo_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Cadoo agent is still installed. Run 'cadoo' to use the CLI,")
    print("or 'cadoo uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.cadoo/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.cadoo/ for future reinstall
    """
    project_root = get_project_root()
    cadoo_home = get_cadoo_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie CADOO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_cadoo_home(cadoo_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.cadoo data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            🟣 Cadoo Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {cadoo_home / 'config.yaml'}")
    print(f"  Secrets: {cadoo_home / '.env'}")
    print(f"  Data:    {cadoo_home / 'cron/'}, {cadoo_home / 'sessions/'}, {cadoo_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CADOO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Cadoo data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Cadoo code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``cadoo`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$CADOO_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in cadoo_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\cadoo\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(cadoo_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Cadoo-owned PATH entries in User environment")

        log_info("Removing CADOO_HOME / HERMES_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Cadoo-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing cadoo command...")
    ...
    #     (only when they still point into this Cadoo home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Cadoo-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(cadoo_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Cadoo-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(cadoo_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE CADOO_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from cadoo_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(cadoo_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.cadoo/, just remove the cadoo subdir
            if cadoo_home in project_root.parents or project_root.parent == cadoo_home:
    ...
    #     under CADOO_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(cadoo_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(cadoo_home)
    ...
    # 5. Optionally remove ~/.cadoo/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CADOO_HOME dirs live under
        ...
        try:
            if cadoo_home.exists():
                shutil.rmtree(cadoo_home)
                log_success(f"Removed {cadoo_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {cadoo_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {cadoo_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://doostudio.com.br/cadoo/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://doostudio.com.br/cadoo/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Cadoo Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m cadoo_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import cadoo_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``cadoo_constants`` + ``cadoo_cli.colors``
    (and lazily ``cadoo_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m cadoo_cli.uninstall")
    parser.add_argument(
        "--mode",
