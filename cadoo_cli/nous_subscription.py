...
# coverage category (cadoo_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `cadoo tools` picker scope its entitlement gate to the selected backend, so a
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `cadoo model`, so direct credentials should NOT
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from cadoo_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline DooStudio Portal login for the Tool Gateway picker (`cadoo tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``cadoo tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (DooStudio Portal)").  Unlike ``cadoo model``'s Nous
    login, this:
    ...
    It only performs the DooStudio Portal device-code OAuth (when the user isn't
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    """Run the DooStudio Portal device-code OAuth and persist credentials only.
    ...
    try:
        ...
        print(f"  Could not start DooStudio Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a DooStudio Portal login.")
    try:
        proceed = input("  Log in to DooStudio Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped DooStudio Portal login.")
        return False
    ...
    try:
        ...
        _sync_nous_pool_from_auth_store()
        print("  DooStudio Portal login successful.")
        return True
        ...
        print(f"  DooStudio Portal login failed: {exc}")
        return False
