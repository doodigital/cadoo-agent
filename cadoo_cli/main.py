#!/usr/bin/env python3
"""
Cadoo CLI - Main entry point.

Usage:
    cadoo                     # Interactive chat (default)
    cadoo chat                # Interactive chat
    cadoo gateway             # Run gateway in foreground
    cadoo gateway start       # Start gateway as service
    cadoo gateway stop        # Stop gateway service
    cadoo gateway status      # Show gateway status
    cadoo gateway install     # Install gateway service
    cadoo gateway uninstall   # Uninstall gateway service
    cadoo setup               # Interactive setup wizard
    cadoo logout              # Clear stored authentication
    cadoo status              # Show status of all components
    cadoo cron                # Manage cron jobs
    cadoo cron list           # List cron jobs
    cadoo cron status         # Check if cron scheduler is running
    cadoo doctor              # Check configuration and dependencies
    cadoo honcho setup                    # Configure Honcho AI memory integration
    cadoo honcho status                   # Show Honcho config and connection status
    cadoo honcho sessions                 # List directory → session name mappings
    cadoo honcho map <name>               # Map current directory to a session name
    cadoo honcho peer                     # Show peer names and dialectic settings
    cadoo honcho peer --user NAME         # Set user peer name
    cadoo honcho peer --ai NAME           # Set AI peer name
    cadoo honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    cadoo honcho mode                     # Show current memory mode
    cadoo honcho mode [hybrid|honcho|local]  # Set memory mode
    cadoo honcho tokens                   # Show token budget settings
    cadoo honcho tokens --context N       # Set session.context() token cap
    cadoo honcho tokens --dialectic N     # Set dialectic result char cap
    cadoo honcho identity                 # Show AI peer identity representation
    cadoo honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    cadoo honcho migrate                  # Step-by-step migration guide: OpenClaw native → Cadoo + Honcho
    cadoo version             Show version
    cadoo update              Update to latest version
    cadoo uninstall           Uninstall Cadoo Agent
    cadoo acp                 Run as an ACP server for editor integration
    cadoo sessions browse     Interactive session picker with search

    cadoo claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: cadoo_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``cadoo_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``cadoo update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``cadoo_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, cadoo crashes on import and the user can't run
# ``cadoo update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import cadoo_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'cadoo' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``cadoo tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``cadoo.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("cadoo")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"cadoo", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"cadoo")
        # Windows: the .exe name is already ``cadoo.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* cadoo_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("CADOO_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".cadoo", "config.yaml")
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`cadoo --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from cadoo_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Cadoo Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``cadoo --version`` before config/logging imports on Termux."""
...
from cadoo_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from cadoo_cli.subcommands.cron import build_cron_parser
from cadoo_cli.subcommands.gateway import build_gateway_parser
...
from cadoo_cli.subcommands.login import build_login_parser
from cadoo_cli.subcommands.logout import build_logout_parser
from cadoo_cli.subcommands.auth import build_auth_parser
from cadoo_cli.subcommands.status import build_status_parser
from cadoo_cli.subcommands.webhook import build_webhook_parser
from cadoo_cli.subcommands.hooks import build_hooks_parser
from cadoo_cli.subcommands.doctor import build_doctor_parser
from cadoo_cli.subcommands.security import build_security_parser
from cadoo_cli.subcommands.dump import build_dump_parser
from cadoo_cli.subcommands.debug import build_debug_parser
from cadoo_cli.subcommands.backup import build_backup_parser
from cadoo_cli.subcommands.import_cmd import build_import_cmd_parser
from cadoo_cli.subcommands.config import build_config_parser
from cadoo_cli.subcommands.version import build_version_parser
from cadoo_cli.subcommands.update import build_update_parser
from cadoo_cli.subcommands.uninstall import build_uninstall_parser
from cadoo_cli.subcommands.dashboard import build_dashboard_parser
from cadoo_cli.subcommands.gui import build_gui_parser
from cadoo_cli.subcommands.logs import build_logs_parser
from cadoo_cli.subcommands.prompt_size import build_prompt_size_parser
from cadoo_cli.subcommands.memory import build_memory_parser
from cadoo_cli.subcommands.acp import build_acp_parser
from cadoo_cli.subcommands.tools import build_tools_parser
from cadoo_cli.subcommands.insights import build_insights_parser
from cadoo_cli.subcommands.skills import build_skills_parser
from cadoo_cli.subcommands.pairing import build_pairing_parser
from cadoo_cli.subcommands.plugins import build_plugins_parser
from cadoo_cli.subcommands.mcp import build_mcp_parser
from cadoo_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (cadoo tools, cadoo setup, cadoo model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'cadoo {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any cadoo module import.
#
# Many modules cache CADOO_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("CADOO_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.cadoo/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set CADOO_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `cadoo mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Cadoo' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo cadoo -p <name>` against the invoking user's home.
        ...
        candidate = home / ".cadoo" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`cadoo chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors cadoo_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If CADOO_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.cadoo/profiles/coder or
    # /opt/data/profiles/coder).  If CADOO_HOME points to the cadoo root
    # instead (e.g. systemd hardcodes CADOO_HOME=/root/.cadoo), we must
    # still read active_profile — the user may have switched profiles via
    # `cadoo profile use` and the gateway should honour that choice.
    # See issue #22502.
    cadoo_home_env = os.environ.get("CADOO_HOME", "")
    if profile_name is None and cadoo_home_env:
        if Path(cadoo_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the cadoo root.
    #
    ...
    # reserved ``gateway-default`` slot runs bare ``cadoo gateway run`` to mean
    # "the root CADOO_HOME profile". If the reserved default child read
    ...
    # 3. If we found a profile, resolve and set CADOO_HOME
    if profile_name is not None:
        try:
            from cadoo_cli.profiles import resolve_profile_env

            cadoo_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            cadoo_home = _resolve_sudo_user_profile_env(profile_name)
            if not cadoo_home:
            ...
            # A bug in profiles.py must NEVER prevent cadoo from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["CADOO_HOME"] = cadoo_home
...
try:
    ...
    _cfg_path = get_cadoo_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from cadoo_cli import managed_scope
...
# Initialize centralized file logging early — all `cadoo` subcommands
...
try:
    from cadoo_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from cadoo_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if cadoo_constants not importable yet
...
from cadoo_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to cadoo_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (cadoo_cli.main._model_flow_*) keep resolving unchanged.
from cadoo_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_cadoo_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from cadoo_cli.config import get_env_path, get_cadoo_home, load_config
    from cadoo_cli.auth import get_auth_status

    # Determine whether Cadoo itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from cadoo_cli.config import DEFAULT_CONFIG
    ...
    from cadoo_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for DooStudio Portal OAuth credentials
    auth_file = get_cadoo_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Cadoo has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Cadoo to use their tokens.
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from cadoo_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'cadoo')
    """
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo cadoo {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo cadoo {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  cadoo --tui --resume {target}")
    if title:
        print(f'  cadoo --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @cadoo/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@cadoo" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/cadoo-ink/package.json",
    "packages/cadoo-ink/index.js",
    "packages/cadoo-ink/text-input.js",
)
...
def _ensure_tui_node() -> None:
    ...
    cadoo_home = os.environ.get("CADOO_HOME") or str(Path.home() / ".cadoo")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "CADOO_HOME": cadoo_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(cadoo_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(cadoo_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if cadoo_cli_dir is None:
        cadoo_cli_dir = Path(__file__).parent
    bundled = cadoo_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``cadoo update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Cadoo checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `cadoo update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Cadoo checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `cadoo --tui`\n"
        "If the checkout is still inconsistent, run `cadoo update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from cadoo_cli.dep_ensure import ensure_dependency
    ...
    if tui_dev:
        # Keep the local @cadoo/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @cadoo/ink resolves through
        # packages/cadoo-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "cadoo-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from cadoo_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from cadoo_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="cadoo-tui-active-session-", suffix=".json"
    )
    ...
    # in the user's shell would otherwise make a plain `cadoo --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `cadoo update` so
    ...
    if code == 42:
        from cadoo_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    (``cadoo kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``cadoo kanban boards switch`` from another session can flip the file
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from cadoo_cli.kanban_db import get_current_board
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.cadoo/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Cadoo — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Cadoo still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'cadoo sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from cadoo_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from cadoo_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'cadoo doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Cadoo isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  cadoo setup")
        print()

        from cadoo_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'cadoo setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from cadoo_cli.banner import prefetch_update_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from Cadoo itself.
    ...
    # cadoo_cli/plugins.py and tools/mcp_tool.py).
    ...
    # user's ~/.cadoo/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from cadoo_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from cadoo_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from cadoo_cli.config import get_env_value, save_env_value
    from cadoo_constants import find_node_executable, with_hermes_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Cadoo?")
        print()
    ...
    # has no creds.json.  Every subsequent `cadoo gateway` then paid a 30s
    ...
    session_dir = get_cadoo_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: cadoo gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `cadoo gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  cadoo gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '🟣 Cadoo Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  cadoo gateway")
        ...
        print()
        print("  Or install as a service: cadoo gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'cadoo whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``cadoo whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``cadoo_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from cadoo_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from cadoo_cli.config import stamp_install_method
    from cadoo_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("🟣 Cadoo post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from cadoo_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``cadoo model``) and the setup wizard
    ...
    from cadoo_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from cadoo_cli.providers import resolve_provider_full
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from cadoo_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'cadoo model' for "
                "available providers, or run 'cadoo doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in cadoo_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.cadoo/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.cadoo/.env if the active provider is not 'custom'.
    ...
    """
    from cadoo_cli.config import get_env_value, save_env_value, load_config
...
#
# Cadoo uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `cadoo model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `cadoo model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`cadoo_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from cadoo_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Cadoo only falls back to a lightweight backend (OpenRouter,")
        print("  DooStudio Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``cadoo model`` flow, then route aux tasks to them here.
    """
    from cadoo_cli.config import load_config
    from cadoo_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from cadoo_cli.auth import _prompt_model_selection
    from cadoo_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from cadoo_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from cadoo_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from cadoo_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Cadoo URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from cadoo_cli.curses_ui import curses_radiolist
...
# downstream call sites read `cadoo_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `cadoo` invocation — including
# fast paths like `cadoo --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``cadoo setup`` / ``cadoo model``.
    ...
    recover from a malformed paste without editing ``~/.cadoo/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from cadoo_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from cadoo_cli.config import save_env_value
    from cadoo_cli.secret_prompt import masked_secret_prompt
    ...
    from cadoo_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `cadoo setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from cadoo_constants import display_cadoo_home as _dhh_fn

            print(
                f"    Cadoo will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Cadoo CLI with a provider."""
    from cadoo_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from cadoo_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from cadoo_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from cadoo_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from cadoo_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from cadoo_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``cadoo slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: cadoo slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `cadoo slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from cadoo_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from cadoo_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from cadoo_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from cadoo_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `cadoo security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from cadoo_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from cadoo_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from cadoo_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from cadoo_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Cadoo home directory to a zip file."""
    if getattr(args, "quick", False):
        from cadoo_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from cadoo_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Cadoo backup from a zip file."""
    from cadoo_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from cadoo_cli.banner import format_banner_version_label
    ...
    try:
        from cadoo_cli.banner import check_for_updates
        from cadoo_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Cadoo Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from cadoo_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from cadoo_cli.uninstall import run_gui_uninstall
    ...
    from cadoo_cli.uninstall import run_uninstall
...
# Critical files that every ``cadoo`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``cadoo update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "cadoo_cli/main.py",
    "cadoo_cli/config.py",
    "cadoo_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``cadoo`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="cadoo-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``cadoo update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``cadoo_cli/web_dist/`` (per vite.config.ts
    outDir: "../cadoo_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "cadoo_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``cadoo`` launcher present but ``cadoo_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the cadoo venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — cadoo venv python3, no nix-shell overhead
    ...
    # a cadoo venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``cadoo update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``cadoo web``).
    ...
    # build path usable on a stock `py -m cadoo_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (cadoo web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``cadoo update`` can unconditionally call ``cadoo desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``cadoo desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $CADOO_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $CADOO_HOME."""
    from cadoo_constants import get_cadoo_home
    return get_cadoo_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``cadoo update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Cadoo.app/Contents/MacOS/Cadoo"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Cadoo.exe",
            release_dir / "win-ia32-unpacked" / "Cadoo.exe",
            release_dir / "win-arm64-unpacked" / "Cadoo.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "cadoo",
            release_dir / "linux-unpacked" / "Cadoo",
            release_dir / "linux-arm64-unpacked" / "cadoo",
            release_dir / "linux-arm64-unpacked" / "Cadoo",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Cadoo'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Cadoo.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Cadoo.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Cadoo.exe: Access is
    ...
    an unrelated "Cadoo" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Cadoo is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Cadoo.app/Contents/MacOS/Cadoo  ->  app bundle = .../Cadoo.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Cadoo Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Cadoo Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from cadoo_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "hermes_root", None):
        env["HERMES_DESKTOP_HERMES_ROOT"] = str(Path(args.cadoo_root).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  cadoo gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Cadoo.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Cadoo.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Cadoo.exe, close any")
                    print("  running Cadoo desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> cadoo desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Cadoo is
    ...
    if source_mode:
        print("→ Launching Cadoo Desktop from source build...")
    ...
    print(f"→ Launching packaged Cadoo Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``cadoo dashboard`` processes other than ourselves.

    ``cadoo dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``cadoo update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Cadoo Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``cadoo dashboard`` as
    ...
    patterns = [
        "cadoo dashboard",
        "cadoo_cli.main dashboard",
        "cadoo_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "cadoo.*dashboard"`) keeps us consistent
            # with `cadoo_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `cadoo update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``cadoo update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``cadoo update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: cadoo curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``cadoo dashboard`` processes.

    Called at the end of ``cadoo update`` (default ``reason``) and also
    from ``cadoo dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Cadoo Desktop Electron app spawns this dashboard as a
    ...
    if killed:
        ...
        print("    cadoo dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.cadoo-update-staging"
    backup = f"{dst}.cadoo-update-old"
...
def _update_via_zip(args):
    """Update Cadoo Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `cadoo update "
            f"--branch {branch}`, or update against main with `cadoo update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/DooStudio/cadoo/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="cadoo-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"cadoo-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for cadoo itself
        ...
        # GitHub ZIPs extract to cadoo-<branch>/
        extracted = os.path.join(tmp_dir, f"cadoo-{branch}")
    ...
    from cadoo_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: cadoo skills list-modified  "
    ...
    try:
        from cadoo_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "cadoo-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Cadoo behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes cadoo completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Cadoo couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Cadoo couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Cadoo behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Cadoo couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Cadoo couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `cadoo update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_cadoo_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_cadoo_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Cadoo repository.")
        print("  This means you may miss updates from DooStudio/cadoo.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/DooStudio/cadoo.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/DooStudio/cadoo.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``cadoo update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``cadoo update`` mutates the
...
# survives and the next ``cadoo`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $CADOO_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``cadoo update`` left half-done.
    ...
    protocol stream (``cadoo acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `cadoo update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from cadoo_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``cadoo update`` is still progressing even if pip/uv itself is silent.
    """
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "cadoo.exe",
        scripts_dir / "cadoo-gateway.exe",
    ]
...
def _detect_concurrent_hermes_instances(
    ...
    The Cadoo Desktop Electron app spawns ``cadoo.EXE`` as a backend child,
    so during ``cadoo update`` the user-invoked process and the desktop's
    ...
    shims (``cadoo.exe`` / ``cadoo-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``cadoo update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``cadoo.exe``) is a separate process from the Python
    ...
    # setuptools-generated cadoo.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``cadoo update``
    ...
    #      cadoo.exe sitting *under* a non-Cadoo parent (e.g. a Cadoo
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "cadoo.exe"
    lines = ["✗ Another cadoo.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Cadoo Desktop, exit any open `cadoo` REPLs, and")
    lines.append("  stop the gateway (`cadoo gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `cadoo update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    """Pre-empt Windows file lock on the running ``cadoo.exe``.
    ...
    when ``cadoo update`` runs, ``cadoo.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``cadoo.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next cadoo invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Cadoo Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Cadoo Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Cadoo Desktop, exit other `cadoo` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `cadoo update`."
        )
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``cadoo.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``cadoo.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``cadoo`` drops off PATH. This wraps
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``cadoo.exe.old.*`` left by prior updates.

    Called early on every cadoo invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``cadoo update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `cadoo update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``cadoo.exe`` / ``cadoo-gateway.exe`` shims
    ...
    # hours later inside ``cadoo update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``cadoo update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``cadoo.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``cadoo`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `cadoo update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `cadoo update --force` after closing other cadoo processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from cadoo_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$CADOO_HOME/bin/uv``, but on Termux the official
    ...
    """
    from cadoo_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `cadoo update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``cadoo update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.cadoo/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``cadoo update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``cadoo update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.cadoo/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``cadoo update --gateway``) the update is already
    ...
    try:
        ...
        # cadoo_cli.config.get_cadoo_home to simulate setup failure.
        from cadoo_cli.config import get_cadoo_home as _get_cadoo_home

        logs_dir = _get_cadoo_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``cadoo update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from cadoo_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``cadoo update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from cadoo_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from cadoo_cli.config import recommended_update_command
        from cadoo_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``cadoo update`` without requiring a reinstall.  The
    ...
    ``cadoo`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/cadoo.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v cadoo'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/cadoo, code at /usr/local/lib/cadoo).
    fhs_link = Path("/usr/local/bin/cadoo")
    ...
    path_comment = (
        "# Cadoo Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of CADOO_HOME before running the update.
    ...
    CADOO_HOME directories.  The ``--backup`` flag on ``cadoo update``
    ...
    # ``cadoo_cli/config.py``). Fall back to true if the key is missing
    ...
    try:
        from cadoo_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_cadoo_home so the user sees ~/.cadoo/...
    try:
        from cadoo_constants import get_cadoo_home, display_cadoo_home

        home = get_cadoo_home()
        try:
            display_path = f"{display_cadoo_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  cadoo import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    cadoo.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from cadoo_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Cadoo...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m cadoo_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: cadoo gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``cadoo gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from cadoo_cli import gateway_windows
        from cadoo_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``cadoo update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Cadoo Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Cadoo Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `cadoo update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Cadoo via pip (for PyPI installs)."""
    from cadoo_cli import __version__
    from cadoo_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        ...
        cmd = [pipx, "upgrade", "cadoo"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "cadoo"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "cadoo"]
    ...
    print("✓ Update complete! Restart cadoo to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("🟣 Atualizando Cadoo Agent...")
    print()

    # On Windows, abort early if another cadoo.exe is holding the venv shim
    ...
    try:
        ...
        # though `git pull` can't touch $CADOO_HOME, this is cheap
        ...
        try:
            from cadoo_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in cadoo_cli/config.py bricked
        # every user who ran ``cadoo update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``cadoo
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``cadoo update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_cadoo_home added to cadoo_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``cadoo`` launch finishes the
        ...
        # build.  ``cadoo desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``cadoo desktop`` shouldn't be forced into a full
        # Electron build by ``cadoo update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from cadoo_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "cadoo_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `cadoo desktop` to retry)")
        ...
        # current — so copy it straight over ~/.cadoo/cache/model_catalog.json
        ...
        # modules in this process.  Reload cadoo_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_cadoo_home() added since the last release.
        try:
            import importlib
            import cadoo_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit CADOO_HOME so
        # it is not affected by sync_skills()'s module-level CADOO_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's CADOO_HOME env var points at the default or a named profile.
        try:
            from cadoo_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from cadoo_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'cadoo config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: cadoo config migrate")
            else:
                print()
                print("Skipped. Run 'cadoo config migrate' later to configure.")
        else:
        ...
        try:
            from cadoo_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `cadoo curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``cadoo update`` gives users a
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from cadoo_cli.tools_config import install_cua_driver
        ...
        # When running as ``cadoo update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_cadoo_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... cadoo-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... cadoo-gateway*)
            ...
            # Discover all cadoo-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. cadoo-gateway.service or cadoo-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `cadoo update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: cadoo gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: cadoo gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: cadoo -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Cadoo gateway unit files are still installed.
        # When both cadoo.service (from a pre-rename install) and the
        # current cadoo-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `cadoo update` surfaces the issue until the user migrates.
        try:
            from cadoo_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Cadoo gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hermes_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (cadoo.service) fight the current")
                print("  cadoo-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    cadoo gateway migrate-legacy")
                print()
        ...
        print("  cadoo model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``cadoo -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `cadoo profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_cadoo_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → cadoo -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.cadoo)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  cadoo profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     cadoo -p {name} chat")
                else:
        ...
        from cadoo_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from cadoo_constants import get_cadoo_home as _hh
        ...
        from cadoo_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `cadoo profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → cadoo -p {name}  ({wrapper})")
        print()
        ...
        from cadoo_cli.profiles import profile_exists
        ...
        from cadoo_cli.profiles import rename_profile
        ...
        from cadoo_cli.profiles import export_profile
        ...
        from cadoo_cli.profiles import import_profile
        ...
        import tempfile
        from cadoo_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  cadoo -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      cadoo -p {plan.manifest.name} chat")
        ...
        from cadoo_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `cadoo profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"cadoo -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from cadoo_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Cadoo {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from cadoo_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.cadoo_requires:
        print(f"  Requires: Cadoo {mf.cadoo_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``cadoo dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``cadoo dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No cadoo dashboard processes running.")
        return 0

    print(f"{len(pids)} cadoo dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``cadoo dashboard register`` for OAuth.
    ...
    try:
        from cadoo_cli.web_server import should_require_auth
    ...
    try:
        from cadoo_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via DooStudio Portal (run `cadoo dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from cadoo_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No cadoo dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `cadoo update`.
    ...
    # launches the dashboard (`worker dashboard` → CADOO_HOME points into
    ...
    try:
        from cadoo_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "cadoo_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # CADOO_HOME.  We must resolve the root explicitly instead of just
        # dropping CADOO_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV CADOO_HOME=/opt/data`), so an unset
    ...
    # the same logs directory as every other Cadoo surface.
    try:
        from cadoo_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "cadoo_cli" / "web_dist"
        )
    ...
    try:
        from cadoo_cli.mcp_startup import start_background_mcp_discovery
    ...
    from cadoo_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with DooStudio Portal."""
    from cadoo_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from cadoo_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from cadoo_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from cadoo_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Cadoo log files."""
    from cadoo_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from cadoo_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``cadoo -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``cadoo_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``cadoo -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``cadoo`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from cadoo_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from cadoo_cli.oneshot import run_oneshot
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `cadoo --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_cadoo_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_cadoo_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_cadoo_home()}/memories/\n")
    else:
        from cadoo_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Cadoo Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from cadoo_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from cadoo_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from cadoo_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from cadoo_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from cadoo_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from cadoo_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from cadoo_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from cadoo_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from cadoo_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for cadoo CLI."""
    # Cosmetic: make the process show up as 'cadoo' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from cadoo_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``cadoo.exe.old.*`` quarantine files left by previous
    # ``cadoo update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # Self-heal a venv left half-built by an interrupted ``cadoo update``
    ...
    # ``cadoo skills install update``) merely defers recovery one launch;
    # under-matching (missing ``cadoo -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in cadoo_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from cadoo_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `cadoo model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from cadoo_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from cadoo_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in cadoo_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # whatsapp command  (parser built in cadoo_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `cadoo whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in cadoo_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from cadoo_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in cadoo_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in cadoo_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in cadoo_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in cadoo_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in cadoo_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in cadoo_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # portal command — DooStudio Portal status + Tool Gateway routing
    # =========================================================================
    from cadoo_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from cadoo_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in cadoo_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in cadoo_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in cadoo_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in cadoo_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in cadoo_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in cadoo_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.cadoo/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo cadoo uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from cadoo_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in cadoo_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in cadoo_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in cadoo_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in cadoo_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from cadoo_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in cadoo_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``cadoo --help``, ``cadoo version``, ``cadoo logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from cadoo_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from cadoo_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Cadoo shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from cadoo_cli.pets import register_cli as _register_pets_cli
    ...
    # =========================================================================
    # memory command  (parser built in cadoo_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in cadoo_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `cadoo computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `cadoo tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `cadoo computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Cadoo app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from cadoo_cli.tools_config import _cua_driver_cmd
            ...
            if path:
                version = ""
                try:
                    from cadoo_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: cadoo computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: cadoo computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: cadoo computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: cadoo computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: cadoo computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: cadoo computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in cadoo_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_cadoo_home() / "sessions"
            ...
            # Launch cadoo --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in cadoo_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in cadoo_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in cadoo_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in cadoo_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in cadoo_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in cadoo_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in cadoo_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in cadoo_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Cadoo-Setup.exe success screen tells users to
    # run `cadoo desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in cadoo_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in cadoo_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in cadoo_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``cadoo -c Pokemon Agent Dev`` → ``cadoo -c 'Pokemon Agent Dev'``
    ...
    from cadoo_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'cadoo -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (cadoo hooks list, cron
