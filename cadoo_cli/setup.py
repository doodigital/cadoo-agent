"""
Interactive setup wizard for Cadoo Agent.
...
_DOCS_BASE = "https://doostudio.com.br/cadoo/docs"
...
# display_cadoo_home imported lazily at call sites (stale-module safety during cadoo update)
...
from cadoo_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from cadoo_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("🟣 Cadoo Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Cadoo using environment variables or config commands:")
    print_info("  cadoo config set model.provider custom")
    print_info("  cadoo config set model.base_url http://localhost:8080/v1")
    print_info("  cadoo config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'cadoo setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``HERMES_NONINTERACTIVE=1`` (see ``cadoo_cli/web_server.py``). In that
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from cadoo_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'cadoo setup')")


def _print_setup_summary(config: dict, cadoo_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'cadoo setup' to configure"))
    ...
    # Video generation — opt-in via `cadoo tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from cadoo_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'cadoo setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'cadoo setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'cadoo setup terminal'"))
    ...
    # Spotify (OAuth via cadoo auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'cadoo setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {cadoo_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('cadoo setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('cadoo setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('cadoo setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('cadoo setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('cadoo setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('cadoo config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('cadoo config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('cadoo config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('cadoo', Colors.GREEN)}              Start chatting")
    print(f"   {color('cadoo gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('cadoo doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `cadoo tools` and `cadoo setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``cadoo model``)
    ...
    provider added to ``cadoo model`` is automatically available here.
    ...
    # Delegate to the shared cadoo model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from cadoo_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: cadoo model")
    ...
    # on demand via `cadoo auth add`, `cadoo setup` vision, and
    # `cadoo setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``cadoo model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from cadoo_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.cadoo/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Cadoo auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via cadoo setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'cadoo setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Cadoo runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `cadoo setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `cadoo setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `cadoo setup agent` later to customize.")
...
def _setup_telegram_auto_result():
    ...
    try:
        from cadoo_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_cadoo_home(Path(get_cadoo_home()))
    except Exception:
        pass
...
def _profile_name_from_cadoo_home(cadoo_home) -> str | None:
    """Return the active profile name when CADOO_HOME is a profile dir."""
    if cadoo_home.parent.name == "profiles":
        return cadoo_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Cadoo delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Cadoo to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from cadoo_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://doostudio.com.br/cadoo/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://doostudio.com.br/cadoo/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  cadoo config edit")
    print_info("   Open config in your editor:  cadoo config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from cadoo_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Cadoo from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'cadoo setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     cadoo config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo cadoo gateway install --system")
                print_info("  Or run in foreground:  cadoo gateway")
        else:
            from cadoo_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   cadoo gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   cadoo gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `cadoo setup tools` and `cadoo tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``cadoo_cli.auth`` — lists every supported
    ...
    """
    try:
        from cadoo_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from cadoo_cli.config
    ...
    if section_key == "model":
        ...
        from cadoo_cli.gateway import _all_platforms, _platform_status
...
# Config values may have different semantics between OpenClaw and Cadoo.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Cadoo to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Cadoo at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Cadoo at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Cadoo at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Cadoo at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Cadoo equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Cadoo config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Cadoo.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Cadoo's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(cadoo_home: Path) -> bool:
    ...
    print_info("Cadoo can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: cadoo claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=cadoo_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: cadoo claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Cadoo configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Cadoo config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Cadoo (use cadoo claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot DooStudio Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``cadoo setup --portal`` and ``cadoo portal``. This is the
    ...
    Cadoo session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``cadoo setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``cadoo model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``cadoo portal`` always offers a model picker,
    ...
    print(color("│     🟣 Cadoo Setup — DooStudio Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your DooStudio Portal sub.")
    print()
    print_info("  Sign up: https://doostudio.com.br/portal/manage-subscription")
    print()
    ...
    # setup calls, so `cadoo portal` == quick setup's Nous step.
    try:
        from cadoo_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `cadoo portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `cadoo portal`: %s", exc)
        print()
        print_error(f"  DooStudio Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `cadoo portal`.")
        return
    ...
    print_info("  Run `cadoo portal info` to inspect routing.")
    print_info("  Run `cadoo` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      cadoo setup           — full or quick (auto-detected)
      cadoo setup model     — just model/provider
      cadoo setup tts       — just text-to-speech
      cadoo setup terminal  — just terminal backend
      cadoo setup gateway   — just messaging platforms
      cadoo setup tools     — just tool configuration
      cadoo setup agent     — just agent settings
    """
    from cadoo_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot DooStudio Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     🟣 Cadoo Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             🟣 Cadoo Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Cadoo Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, cadoo_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Cadoo configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'cadoo setup model|terminal|")
        ...
        if setup_mode == 0:
            ...
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, cadoo_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {cadoo_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'cadoo config edit'")
    ...
    # Tune later with `cadoo setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, cadoo_home)


def _run_first_time_quick_setup(config: dict, cadoo_home, is_existing: bool):
    """Streamlined first-time setup via DooStudio Portal: OAuth, model, terminal & messaging.

    Routes straight to the DooStudio Portal provider — runs the device-code OAuth
    ...
    settings, tools); the user can customize later via ``cadoo setup <section>``
    or switch providers with ``cadoo model``.
    """
    from cadoo_cli.config import load_config

    # Step 1: DooStudio Portal — OAuth login + model selection.
    # _model_flow_nous() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Nous model picker). Provider is set to "nous" by the login/model save.
    print()
    print_header("DooStudio Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://doostudio.com.br/portal/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("DooStudio Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nous error during quick setup: %s", exc)
        print_warning(f"DooStudio Portal setup encountered an error: {exc}")
        print_info("You can try again later with: cadoo model")
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'cadoo setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    cadoo setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  cadoo setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``cadoo tools`` (which
    ...
    try:
        ...
        from cadoo_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``cadoo setup agent`` /
    ``cadoo config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, cadoo_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `cadoo update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        cadoo tools")
        print_info("  Seed skills:         cadoo skills opt-in --sync")
        print_info("  Add MCP servers:     cadoo mcp add")
        print_info("  Enable plugins:      cadoo plugins")
        print_info("  Tune agent settings: cadoo setup agent")
        print()
        _print_setup_summary(config, cadoo_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, cadoo_home)


def _blank_slate_walkthrough(config: dict, cadoo_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`cadoo update` runs from re-injecting them. Opt back in any")
            print_info("time with `cadoo skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `cadoo tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `cadoo plugins list` / `cadoo plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `cadoo plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `cadoo mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `cadoo mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   cadoo tools")
    print_info("  Seed skills:         cadoo skills opt-in --sync")
    print_info("  Add MCP servers:     cadoo mcp add")
    print_info("  Tune agent settings: cadoo setup agent")
    print()
...
def _run_quick_setup(config: dict, cadoo_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'cadoo setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Cadoo to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'cadoo setup gateway'.")
