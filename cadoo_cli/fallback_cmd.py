"""
cadoo fallback — manage the fallback provider chain.
...
Subcommands:
  cadoo fallback [list]   Show the current fallback chain (default when no subcommand)
  cadoo fallback add      Pick provider + model via the same picker as `cadoo model`,
                           then append the selection to the chain
  cadoo fallback remove   Pick an entry to delete from the chain
  cadoo fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.cadoo/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from cadoo_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `cadoo model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  cadoo fallback add")
        print()
        return
    ...
    print("  Docs: https://doostudio.com.br/cadoo/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `cadoo model`, then append the selection to the chain."""
    from cadoo_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `cadoo model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `cadoo fallback list` to view, or `cadoo fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``cadoo fallback [subcommand]``."""
