"""
Single source of truth for provider identity in Cadoo Agent.
...
2. **Cadoo overlays** — transport type, auth patterns, aggregator flags,
...
# -- Cadoo overlay ----------------------------------------------------------
# Cadoo-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Cadoo-specific provider metadata layered on top of models.dev."""
...
HERMES_OVERLAYS: Dict[str, HermesOverlay] = {
    ...
    "nous": HermesOverlay(
        ...
        base_url_override="https://inference-api.doostudio.com.br/v1",
    ),
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "cadoo", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Cadoo overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Cadoo overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + cadoo extra
    ...
    if overlay is not None:
        # Cadoo-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
