...
Add, remove, or reorder entries here — both `cadoo setup` and
`cadoo` provider-selection will pick up the change automatically.
"""
...
from cadoo_cli import __version__ as _HERMES_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_HERMES_USER_AGENT = f"cadoo-cli/{_HERMES_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `cadoo model`
    flow without maintaining a separate static list.
    """
    from cadoo_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $CADOO_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Cadoo-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $CADOO_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Cadoo refreshes the cache from
...
# ---------------------------------------------------------------------------
# DooStudio Portal free-model helper
# ---------------------------------------------------------------------------
# The DooStudio Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# DooStudio Portal account tier detection
# ---------------------------------------------------------------------------
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Cadoo that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Cadoo that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current DooStudio Portal user is on a free (unpaid) tier.
    ...
    try:
        from cadoo_cli.nous_account import get_nous_portal_account_info
...
# ---------------------------------------------------------------------------
# DooStudio Portal recommended models
#
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_cadoo_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the DooStudio Portal's curated recommended-models payload.
    ...
    (``$CADOO_HOME/cache/nous_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://doostudio.com.br/portal").rstrip("/")
    now = time.monotonic()
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://doostudio.com.br/portal"
...
# this list:  cadoo model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `cadoo model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `cadoo model` TUI
...
#
# Some vendors expose several Cadoo provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `cadoo model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``cadoo model``, the
...
# sensible default, but DooStudio Portal is a per-token *metered aggregator* whose
...
# path. The *interactive* default (GUI onboarding / ``cadoo model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in cadoo_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``cadoo auth add openai-codex`` without
    ``cadoo model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``cadoo model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    cadoo is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (DooStudio Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from cadoo_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — cadoo
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated DooStudio Portal model-id list.
    ...
    """
    try:
        from cadoo_cli.model_catalog import get_curated_nous_models
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, DooStudio Portal).
    """
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.doostudio.com.br"


def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for DooStudio Portal pricing.
    ...
    """
    try:
        from cadoo_cli.auth import resolve_nous_runtime_credentials
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.doostudio.com.br/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``cadoo model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from cadoo_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from cadoo_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Cadoo' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``cadoo_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Cadoo should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``cadoo auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.cadoo/.env``.
    ...
    """
    try:
        from cadoo_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from cadoo_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Cadoo release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Cadoo release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from cadoo_cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized == "nous":
        # Try live DooStudio Portal /models endpoint
        try:
            from cadoo_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Cadoo release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``cadoo model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $CADOO_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from cadoo_constants import get_cadoo_home
    return get_cadoo_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$CADOO_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from cadoo_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "CadooAgent/1.0",
...
def _lmstudio_fetch_raw_models(
    ...
    try:
        ...
        if exc.code in {401, 403}:
            from cadoo_cli.auth import AuthError
            raise AuthError(
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Cadoo' default Claude IDs elsewhere use
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from cadoo_constants import get_cadoo_home
    return get_cadoo_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Cadoo will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Cadoo cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Cadoo actually ships.  (#46850)
