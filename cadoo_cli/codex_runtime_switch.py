...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Cadoo'
...
The actual runtime resolution happens in cadoo_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "cadoo"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Cadoo' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Cadoo for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from cadoo_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the cadoo-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "cadoo-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Cadoo tool callback are always-on production
            ...
            if "cadoo-tools" in mig_report.migrated:
                msg_lines.append(
                    "Cadoo tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Cadoo runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Cadoo tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Cadoo runtime.")
