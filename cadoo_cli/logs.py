"""``cadoo logs`` — view and filter Cadoo log files.
...
Usage examples::
    ...
    cadoo logs                    # last 50 lines of agent.log
    cadoo logs -f                 # follow agent.log in real time
    cadoo logs errors             # last 50 lines of errors.log
    cadoo logs gateway -n 100    # last 100 lines of gateway.log
    cadoo logs gui -f            # follow gui.log (dashboard/pty/ws)
    cadoo logs desktop -f        # follow desktop.log (Electron app boot/backend)
    cadoo logs --level WARNING    # only WARNING+ lines
    cadoo logs --session abc123   # filter by session ID substring
    cadoo logs --component tools  # only tool-related lines
    cadoo logs --since 1h         # lines from the last hour
    cadoo logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_cadoo_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Cadoo runs — try 'cadoo chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_cadoo_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_cadoo_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_cadoo_home()}/logs/")
        return

    print(f"Log files in {display_cadoo_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'cadoo chat' to generate logs)")
