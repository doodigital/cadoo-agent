"""
Configuration management for Cadoo Agent.

Config files are stored in ~/.cadoo/ for easy access:
- ~/.cadoo/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.cadoo/.env         - API keys and secrets

This module provides:
- cadoo config          - Show current configuration
- cadoo config edit     - Open config in editor
- cadoo config set      - Set a specific value
- cadoo config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``cadoo config set`` (which rewrites
    ...
    ``config.yaml`` in place: cadoo never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.cadoo/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``cadoo logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  cadoo config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Cadoo makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Cadoo itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``cadoo update``, the TUI build.
...
#   on every plugin install / ``cadoo update``.
...
# * ``CADOO_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
#   ``HERMES_ENV`` — Cadoo runtime location flags. Writing these into
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Cadoo runtime location "
            "(CADOO_HOME, HERMES_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `cadoo plugins enable
    # observability/langfuse` or `cadoo tools → Langfuse`); credentials gate
...
from cadoo_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_cadoo_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Cadoo is running in package-manager-managed mode.

    Two signals: the HERMES_MANAGED env var (set by the systemd service),
    or a .managed marker file in CADOO_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``cadoo_cli/`` — i.e. the git checkout for source
    installs, ``/opt/cadoo`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$CADOO_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Cadoo was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$CADOO_HOME/.install_method`` — read for
    ...
    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.cadoo``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$CADOO_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.cadoo:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$CADOO_HOME``, a
    ...
    ``docker`` and ``cadoo update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/cadoo`` image bakes a ``docker``
        stamp into ``/opt/cadoo`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $CADOO_HOME.
    try:
    ...
    #    host install whose shared $CADOO_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``cadoo update``.
    try:
        method = (
            (get_cadoo_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``cadoo_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$CADOO_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/cadoo`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Cadoo lives in a ``uv tool`` layout.

    ``uv tool install cadoo`` places the install at
    ``.../uv/tools/cadoo/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``cadoo`` happens to be uv-tool-installed on the machine while
    the *active* Cadoo is a regular pip/venv install, causing
    ``cadoo update`` to upgrade the wrong copy. It would also block on a
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade cadoo"
    if method == "docker":
        return "docker pull nousresearch/cadoo:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade cadoo"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade cadoo"
        return "pip install --upgrade cadoo"
    return "cadoo update"
...
# Long-form text for ``cadoo update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# cadoo_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Cadoo, it doesn't update
...
✗ ``cadoo update`` doesn't apply inside the Docker container.

Cadoo Agent runs as a published image (nousresearch/cadoo), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/cadoo:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate cadoo
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/cadoo:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/cadoo/tags
  • Your config and session history live under ``$CADOO_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``cadoo update`` inside Docker.
...
def format_managed_message(action: str = "modify this Cadoo installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Cadoo installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.cadoo.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Cadoo installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade cadoo"
        )
    ...
    return (
        f"Cannot {action}: this Cadoo installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Cadoo."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CADOO_HOME/.container-mode.
    ...
    container_mode_file = get_cadoo_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "cadoo")
    exec_user = info.get("exec_user", "cadoo")
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/cadoo")
...
# Re-export from cadoo_constants — canonical definition lives there.
from cadoo_constants import get_cadoo_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_cadoo_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_cadoo_home() / ".env"
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Cadoo commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level CADOO_HOME once, but
    subdirectories created at runtime by ``ensure_cadoo_home()`` (especially
...
def _chown_to_hermes_uid(path) -> None:
    ...
    directories created by :func:`ensure_cadoo_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the cadoo group can
    share state with the gateway service.

    The mode can be overridden via the CADOO_HOME_MODE environment variable
    (e.g. CADOO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CADOO_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CADOO_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Cadoo runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CADOO_HOME if the user doesn't have one yet."""
...
def ensure_cadoo_home():
    """Ensure ~/.cadoo directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_cadoo_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_cadoo_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CADOO_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Cadoo-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Cadoo
        ...
        # app, ACP) in a code workspace, Cadoo adds a coding operating brief
    ...
    "terminal": {
        ...
        #             CADOO_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force CADOO_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Cadoo auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Cadoo sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Cadoo
        ...
        # bundled Cadoo image, which drops to the `cadoo` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Cadoo sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Cadoo should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``cadoo chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.cadoo/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: cadoo sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .cadoo.md, or .cursorrules before Cadoo applies
    ...
    # single read_file page exceeds these limits, Cadoo truncates the
    ...
    # See tools/kanban_tools.py and cadoo_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``cadoo dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``cadoo kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``cadoo kanban decompose`` and the kanban
        ...
        # ``cadoo profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `cadoo model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `cadoo` (and `cadoo chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `cadoo --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `cadoo -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `cadoo --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/cadoo`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `cadoo pets`. Disabled until
        ...
        "pet": {
            ...
            # get_cadoo_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled DooStudio Portal plugin reads
        ...
        # optional path prefix (e.g. ``https://example.com/cadoo``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Cadoo appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.cadoo/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.cadoo/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Cadoo feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Cadoo auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `cadoo curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `cadoo curator run --consolidate` overrides
        ...
        # normally restored on every `cadoo update`, so pruning them only
        ...
        # skipped), snapshot ~/.cadoo/skills/ into
        # ~/.cadoo/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `cadoo curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for cadoo-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Cadoo Agent*" header.
    ...
    # replace Cadoo' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.cadoo/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `cadoo doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``cadoo_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Cadoo to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $CADOO_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing DooStudio Portal token — there is no token key here.
        "chronos": {
    ...
    # todos to ready, and fires `cadoo -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `cadoo` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `cadoo kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     cadoo's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Cadoo tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.cadoo/logs/.
    ...
    # curated model lists for OpenRouter and DooStudio Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a cadoo release.
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.cadoo/.env,
        ...
        # behavior: files must live under the Cadoo cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Cadoo
        # cache (~/.cadoo/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.cadoo/state.db.
    ...
    "sessions": {
        ...
        # default of ``cadoo sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.cadoo/sessions/session_{sid}.json`` on every turn
    ...
    # ``cadoo update`` behaviour.
    "updates": {
        # Run a full ``cadoo backup``-style zip of CADOO_HOME before every
        # ``cadoo update``.  Backups land in ``<CADOO_HOME>/backups/`` and
        # can be restored with ``cadoo import <path>``.  Defaults to true
        # after the #48200 incident: a ``cadoo update --yes`` run that
        ...
        "backup_keep": 5,
        # What `cadoo update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<CADOO_HOME>/lsp/bin/`` on first use.
    ...
    # `cadoo tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.cadoo/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.cadoo/.env (or your shell) and never in config.yaml.
            ...
            # ~/.cadoo/bin/ on first use.  When False you must install
    ...
    "computer_use": {
        ...
        # by default upstream. Cadoo disables it for our users unless they
        # explicitly opt in here. When false (default), Cadoo sets
...
# selection step (DooStudio Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "DooStudio Portal base URL override",
        "prompt": "DooStudio Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'cadoo model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Cadoo auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Cadoo, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @cadoo:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #cadoo)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: cadoo-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'cadoo' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Cadoo API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Cadoo API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Cadoo API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.cadoo.config`` entries, then checks
    ...
    try:
        ...
        # should never break `cadoo update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Cadoo versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``cadoo_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'cadoo setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Cadoo won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'cadoo doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CADOO_HOME", "~/.cadoo")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CADOO_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_cadoo_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `cadoo plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `cadoo curator status` has no
    ...
    #   3. Creates `~/.cadoo/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_cadoo_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_cadoo_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from cadoo_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: cadoo config set <key> <value>")
    ...
    # metadata.cadoo.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``cadoo config set``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _cadoo_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.cadoo/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.cadoo/config.yaml.
    ...
    (which change ``CADOO_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/cadoo/config.yaml invalidates the
...
#   openai-codex (OAuth — cadoo auth) — OpenAI Codex
#   nous         (OAuth — cadoo auth) — DooStudio Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.cadoo/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.cadoo/.env.
    ...
    interactive menu render (`cadoo tools`, `cadoo setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `cadoo tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Cadoo env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.cadoo/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.cadoo/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.cadoo/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.cadoo/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Cadoo — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Cadoo vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.cadoo/.env or environment."""
...
def show_config():
    ...
    print(color("│              🟣 Cadoo Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from cadoo_cli.auth import get_anthropic_key
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'cadoo doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  cadoo config edit     # Edit config file", Colors.DIM))
    print(color("  cadoo config set <key> <value>", Colors.DIM))
    print(color("  cadoo setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `cadoo config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `cadoo config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: cadoo config set <key> <value>")
            print()
            print("Examples:")
            print("  cadoo config set model anthropic/claude-sonnet-4")
            print("  cadoo config set terminal.backend docker")
            print("  cadoo config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'cadoo config migrate' to add them")
        ...
        print("  cadoo config           Show current configuration")
        print("  cadoo config edit      Open config in editor")
        print("  cadoo config set <key> <value>   Set a config value")
        print("  cadoo config check     Check for missing/outdated config")
        print("  cadoo config migrate   Update config with new options")
        print("  cadoo config path      Show config file path")
        print("  cadoo config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``cadoo config`` UI so users
