...
Three cookies in play:
  ...
  - hermes_session_rt:   the OAuth refresh token
                         ...
                         DooStudio Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / cadoo
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/cadoo) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/cadoo`` covers
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /cadoo`` we want ``Path=/cadoo`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. DooStudio Portal
    issues a 24h rotating refresh token (cadoo #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/cadoo``)
