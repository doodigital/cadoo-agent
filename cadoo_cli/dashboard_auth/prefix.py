...
prefix (e.g. ``mission-control.tilos.com/cadoo/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /cadoo`` so the backend can
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``cadoo.example.com`` instead of
    ``https://cadoo.example.com``) — the single most common cause of
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "cadoo.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/cadoo"`` (no trailing slash) or ``""``
