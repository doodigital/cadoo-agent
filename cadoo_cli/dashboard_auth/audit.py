...
Profile-aware location: ``$CADOO_HOME/logs/dashboard-auth.log``.
...
from ``cadoo_constants`` or other cadoo_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$CADOO_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``cadoo_constants.get_cadoo_home`` semantics: env var wins,
    else ``~/.cadoo``. A local copy avoids an import cycle with the
    middleware which lives below ``cadoo_cli``.
    """
    home = os.environ.get("CADOO_HOME") or str(Path.home() / ".cadoo")
