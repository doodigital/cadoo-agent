"""Windows ConPTY bridge for the `cadoo dashboard` chat tab.

Drop-in counterpart to ``cadoo_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``cadoo_cli.web_server`` consumes: ``spawn``, ``read``,
