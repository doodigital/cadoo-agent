"""``cadoo slack ...`` CLI subcommands.

Today only ``cadoo slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ cadoo slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Cadoo. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Cadoo deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from cadoo_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Cadoo in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Cadoo agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``cadoo_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$CADOO_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Cadoo")
    ...
    """
    name = getattr(args, "name", None) or "Cadoo"
    description = getattr(args, "description", None) or "Your Cadoo agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("CADOO_HOME") or str(Path.home() / ".cadoo")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Cadoo app\n"
