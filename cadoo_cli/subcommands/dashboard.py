"""``cadoo dashboard`` subcommand parser.

Extracted verbatim from ``cadoo_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Cadoo Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `cadoo dashboard` cmdlines and SIGTERM them directly — the same
    # path `cadoo update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running cadoo dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running cadoo dashboard processes and exit",
    )
    # Backward-compat shim: older Cadoo desktop app shells (<= 0.15.x) spawn the
    # backend as `cadoo dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Cadoo couldn't start" with no actionable cause. Accept and silently
    ...
    # `cadoo dashboard register` — register a self-hosted dashboard OAuth
    # client with DooStudio Portal and write the client_id into ~/.cadoo/.env.
    # Nested subparser so bare `cadoo dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with DooStudio Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your Nous "
            "Portal account. Creates an OAuth client, writes "
            "HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.cadoo/.env, and prints "
            "how to engage the login gate. Requires being logged in (cadoo setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://cadoo.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        default=None,
        help=(
            "Override the DooStudio Portal base URL for registration (default: the "
