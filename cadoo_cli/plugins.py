...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with cadoo;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.cadoo/plugins/<name>/``
3. **Project plugins** – ``./.cadoo/plugins/<name>/`` (opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``cadoo.plugins``
   entry-point group.
...
from cadoo_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
# stderr in addition to ~/.cadoo/logs/agent.log. Aimed at plugin authors
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Cadoo loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by cadoo_cli.kanban_db when a task
    ...
    # separate `cadoo -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `cadoo kanban dispatch`),
...
class PluginManifest:
    ...
    #              in ~/.cadoo/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``cadoo plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Cadoo profile name (e.g. ``"default"``).

        Derived from ``CADOO_HOME`` via
        :func:`cadoo_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.cadoo/profiles/<name>``, or ``"custom"`` when
        ``CADOO_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``cadoo honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``cadoo <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`cadoo_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from cadoo_cli.dashboard_auth import (
    ...
    def register_slack_action_handler(
        ...
        Cadoo' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``cadoo model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: cadoo_cli.main imports plugins indirectly
        from cadoo_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.cadoo/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to cadoo_cli/. Two layouts are
        ...
        # 2. User plugins (~/.cadoo/plugins/)
        user_dir = get_cadoo_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.cadoo/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".cadoo" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with cadoo and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Cadoo ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `cadoo plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `cadoo plugins list`.
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="cadoo-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``cadoo tools`` TUI so plugin-provided toolsets appear
