...
Pure display functions with no CadooCLI state dependency.
"""
...
# (e.g. nix-built cadoo — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/DooStudio/cadoo.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/cadoo"
...
def _fetch_pypi_latest(package: str = "cadoo") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Cadoo update is available.
    ...
    cache_file = cadoo_home / ".update_check"
    ...
    # and `cadoo update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/cadoo/update/check`
    ...
    if embedded_rev:
        ...
        # $CADOO_HOME/cadoo/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = cadoo_home / "cadoo"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Cadoo git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$CADOO_HOME/cadoo/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``cadoo_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from cadoo_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/DooStudio/cadoo/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Cadoo checkout. Cached per-process. Release URL always points at the
    canonical DooStudio/cadoo repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Cadoo Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else CADOO_CADUCEUS
    except Exception:
        _bskin = None
        _hero = CADOO_CADUCEUS
    left_lines = [_hero]
    left_content = "\n".join(left_lines)
    ...
    ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} ctx[/]" if context_length else ""
    ...
    right_lines = [
        f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]DooStudio[/]",
    ]
    if os.getenv("HERMES_YOLO_MODE"):
        right_lines.append(f"[bold red]⚠ YOLO mode[/] [dim {dim}]— all approval prompts bypassed[/]")
    right_lines.append(f"[dim {dim}]{cwd}[/]")
    if session_id:
        right_lines.append(f"[dim {session_color}]Session: {session_id}[/]")
    right_lines.append("")
    right_lines.append(f"[bold {accent}]{_lbl_tools}[/]")
    ...
    try:
        from cadoo_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from cadoo_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built cadoo; we know an update
    ...
    # Pip-install warning — `pip install cadoo` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else CADOO_AGENT_LOGO
        console.print(_logo)
        console.print()
