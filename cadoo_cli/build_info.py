"""
Baked-in build metadata for Cadoo Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``cadoo_cli/dump.py`` and ``cadoo_cli/banner.py``).  That doesn't work inside
...
To make ``cadoo dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$HERMES_GIT_SHA``
arg into ``<project_root>/.cadoo_build_sha``.  This module is the single
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".cadoo_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.cadoo_build_sha`` if present.  The file is
