"""Profile distributions — shareable, packaged Cadoo profiles via git.

A distribution is a Cadoo profile published as a git repository (or
...
* ``cadoo profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``cadoo skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``cadoo profile``, not a parallel tree):

    cadoo profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "cadoo", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.cadoo_requires:
            out["hermes_requires"] = self.cadoo_requires
        if self.author:
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Cadoo {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``cadoo profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Cadoo profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Cadoo distribution."
        )

    # Version check up-front so we fail fast
    check_hermes_requires(manifest.cadoo_requires, hermes_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.cadoo).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `cadoo profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`cadoo profile install <source> --name {canon} --force`."
        )
