"""``cadoo portal`` — the human-readable entry point for DooStudio Portal.

Running ``cadoo portal`` with no subcommand performs the one-shot Portal
...
``cadoo auth add nous --type oauth`` (which still works), is identical to
``cadoo setup --portal``, and runs the same Nous flow as the first-time quick
setup.

Subcommands:
  (none)   Log in to DooStudio Portal + set it up (one-shot onboarding).
...
already in ``cadoo auth`` or ``cadoo tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    print()
    print(color("  DooStudio Portal", Colors.MAGENTA))
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `cadoo model`)")
    ...
    for feat in features.items():
        if feat.managed_by_nous:
            state = color("via DooStudio Portal", Colors.GREEN)
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into DooStudio Portal — sign in with `cadoo portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    """Run the one-shot DooStudio Portal onboarding (login + model + provider + tools).

    This is the human-readable front door for `cadoo auth add nous --type
    oauth`. It reuses the exact wiring behind `cadoo setup --portal` (which in
    ...
    """
    from cadoo_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `cadoo portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `cadoo portal` is the
        # human-readable alias for `cadoo auth add nous --type oauth` /
        # `cadoo setup --portal`.
    ...
    print("Run `cadoo portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `cadoo portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up DooStudio Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `cadoo portal` with no subcommand to log in to DooStudio Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `cadoo auth add "
            "nous --type oauth`, identical to `cadoo setup --portal`). "
    ...
    portal_sub.add_parser(
        "login",
        help="Log in to DooStudio Portal + set it up (default; one-shot onboarding)",
    )
    portal_sub.add_parser(
        "info",
