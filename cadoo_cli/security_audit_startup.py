...
3. Running inside a container with no persistent volume mount over the
   CADOO_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("cadoo.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Cadoo as an unprivileged user (or in a "
...
def _container_no_volume_mount(cadoo_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    home = cadoo_home or Path(
        os.environ.get("CADOO_HOME", os.path.expanduser("~/.cadoo"))
    )
    try:
...
def run_security_audit(
    *, cadoo_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(cadoo_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(cadoo_home=cadoo_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
