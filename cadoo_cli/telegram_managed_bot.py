...
manual BotFather token copy-paste. Cadoo talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.doostudio.com.br/cadoo"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Cadoo Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Cadoo Telegram onboarding service.")
