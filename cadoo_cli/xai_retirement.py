...
Pure logic: walks a Cadoo config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `cadoo doctor` and a future `cadoo migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Cadoo config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Cadoo config and return retirement issues.
