"""
Dump command for cadoo CLI.

Outputs a compact, plain-text summary of the user's Cadoo setup
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.cadoo_build_sha`` by the Dockerfile's
    ``HERMES_GIT_SHA`` build-arg (see ``cadoo_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``cadoo dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from cadoo_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(cadoo_home: Path) -> int:
...
def _cron_summary(cadoo_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = cadoo_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"cadoo_home:      {display_cadoo_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `cadoo auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `cadoo auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["cadoo-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(cadoo_home)}")
    lines.append(f"  skills:             {_count_skills(cadoo_home)}")
