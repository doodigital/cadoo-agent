"""DooStudio Portal terminal-billing HTTP client (Phase 2b).

Thin, fail-loud client for the four ``/api/billing/*`` endpoints the terminal
billing screens drive. Companion to ``cadoo_cli/nous_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Cadoo already holds for inference
...
DEFAULT_PORTAL_BASE_URL = "https://doostudio.com.br/portal"
...
# cadoo_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into DooStudio Portal — run `cadoo portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
...
def _request(
    ...
    *,
    ...
    try:
        ...
        raise BillingError(
            f"Could not reach DooStudio Portal: {exc.reason}", error="network_error"
        ) from exc
