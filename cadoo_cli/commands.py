"""Slash command definitions and autocomplete for the Cadoo CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Cadoo config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Cadoo works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Cadoo is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.cadoo/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Cadoo Agent to the latest version", "Info"),
    CommandDef("version", "Show Cadoo Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`cadoo_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/cadoo`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Cadoo ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Cadoo
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``cadoo skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``cadoo skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``cadoo skills list`` and
...
# unrelated command landed. These claim their slots right after /cadoo,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /cadoo <command>).
...
# native slot, the alias spelling stays reachable via /cadoo reset).
...
# ``/cadoo <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/cadoo" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /cadoo credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /cadoo billing.
#   - debug: the log/report upload surface; reached via /cadoo debug on Slack.
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/cadoo <verb>`` subcommand.
    ...
    can still reach them via ``/cadoo <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/cadoo`` is always reserved as the first entry so the
    legacy ``/cadoo <subcommand>`` form keeps working for anything that
    ...
    # Reserve /cadoo as the catch-all top-level command.
    entries.append(("cadoo", "Talk to Cadoo or run a subcommand", "[subcommand] [args]"))
    seen.add("cadoo")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_VIA_HERMES_ONLY:
            # Intentionally Slack-via-/cadoo only (see _SLACK_VIA_HERMES_ONLY).
            return
    ...
    # everything except /cadoo, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://cadoo.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /cadoo handler.

    Maps both canonical names and aliases so /cadoo bg do stuff works
    the same as /cadoo background do stuff.

    Plugin-registered slash commands are included so ``/cadoo <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from cadoo_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
