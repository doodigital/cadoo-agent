"""Shell completion script generation for cadoo CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Cadoo Agent bash completion
# Add to ~/.bashrc:
#   eval "$(cadoo completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.cadoo/profiles"
...
complete -F _hermes_completion cadoo
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef cadoo
# Cadoo Agent zsh completion
# Add to ~/.zshrc:
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.cadoo/profiles" ]]; then
        profiles+=($HOME/.cadoo/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'cadoo command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Cadoo Agent fish completion",
        "# Add to your config:",
        "#   cadoo completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.cadoo/profiles",
        "        for d in $HOME/.cadoo/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c cadoo -f -s p -l profile"
