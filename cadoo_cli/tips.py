...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or cadoo -c.",
    ...
    "Ctrl+Z suspends Cadoo to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "cadoo -c resumes your most recent CLI session. cadoo -c \"project name\" resumes by title.",
    "cadoo -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "cadoo -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "cadoo chat -t web,terminal enables only specific toolsets for a focused session.",
    "cadoo chat -s github-pr-workflow preloads a skill at launch.",
    "cadoo chat -q \"query\" runs a single non-interactive query and exits.",
    "cadoo chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "cadoo chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "cadoo --yolo bypasses all dangerous command approval prompts for the entire session.",
    "cadoo chat --source telegram tags the session for filtering in cadoo sessions list.",
    "cadoo -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "cadoo doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "cadoo dump outputs a compact setup summary — great for bug reports.",
    "cadoo config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "cadoo config edit opens config.yaml in your default editor.",
    "cadoo config check scans for missing or stale configuration options.",
    "cadoo sessions browse opens an interactive session picker with search.",
    "cadoo sessions stats shows session counts by platform and database size.",
    "cadoo sessions prune --older-than 30 cleans up old sessions.",
    "cadoo skills search react --source skills-sh searches the skills.sh public directory.",
    "cadoo skills check scans installed hub skills for upstream updates.",
    "cadoo skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "cadoo skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "cadoo mcp add github --command npx adds MCP servers from the command line.",
    "cadoo mcp serve runs Cadoo itself as an MCP server for other agents.",
    "cadoo auth add lets you add multiple API keys for credential pool rotation.",
    "cadoo completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "cadoo logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "cadoo backup creates a zip backup of your entire Cadoo home directory.",
    "cadoo profile create coder creates an isolated profile that becomes its own command.",
    "cadoo profile create work --clone copies your current config and keys to a new profile.",
    "cadoo update syncs new bundled skills to ALL profiles automatically.",
    "cadoo gateway install sets up Cadoo as a system service (systemd/launchd).",
    "cadoo memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "cadoo webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: cadoo tools disables unused tools, cadoo skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "cadoo models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as cadoo -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Cadoo tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'cadoo profile create coder' creates the 'coder' command.",
    "cadoo profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Cadoo prints a resume command with session ID and stats.",
    "cadoo sessions export backup.jsonl exports all sessions for backup or analysis.",
    "cadoo -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "cadoo skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .cadoo/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: cadoo cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Cadoo runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "cadoo gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Cadoo via a local macOS server.",
    ...
    "Context files (.cadoo.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Cadoo loads project context from .cadoo.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "cadoo mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "cadoo mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "cadoo mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — cadoo tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.cadoo/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "cadoo chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "cadoo plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "cadoo acp runs Cadoo as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "cadoo auth add nous or cadoo auth add openai-codex sets up OAuth-based providers.",
    ...
    "cadoo status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.cadoo/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Cadoo your own.",
    ...
    "Before a gateway session reset, Cadoo auto-flushes important facts to memory in the background.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'cadoo profile create ls' would be rejected.",
    "cadoo profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "cadoo claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "The skills quarantine at ~/.cadoo/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Cadoo auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Cadoo config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Cadoo is working.',
    ...
    '/reload reloads ~/.cadoo/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.cadoo/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.cadoo/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'cadoo curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "cadoo curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'cadoo curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'cadoo auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'cadoo dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'cadoo dashboard embeds the full Cadoo TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.cadoo/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.cadoo/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.cadoo/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.cadoo/skills/ so drop-in skills appear without restarting the session.',
    ...
    'cadoo -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'cadoo chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'cadoo chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'cadoo chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "cadoo chat --source tool tags programmatic chats so they don't clutter cadoo sessions list.",
    'cadoo dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'cadoo sessions rename <ID> "new title" renames any past session; cadoo sessions delete <ID> removes one.',
    'cadoo import restores a session export or profile archive produced by sessions export or profile export.',
    'cadoo fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'cadoo pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'cadoo setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Cadoo block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from cadoo sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.cadoo/dashboard-plugins/.',
]
