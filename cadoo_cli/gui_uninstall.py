"""
Cadoo Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``cadoo desktop`` / ``cadoo gui``)
     Built inside the agent checkout under ``$CADOO_HOME/cadoo/``:
       ...
       - ``$CADOO_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Cadoo.app`` or ``~/Applications/Cadoo.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Cadoo`` (NSIS per-user)
...
the app name ("Cadoo"), separate from ``$CADOO_HOME``:
  - macOS:   ``~/Library/Application Support/Cadoo``
  - Windows: ``%APPDATA%\\Cadoo``
  - Linux:   ``$XDG_CONFIG_HOME/Cadoo`` (default ``~/.config/Cadoo``)
...
``cadoo uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Cadoo"
...
def source_built_gui_artifacts(cadoo_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``cadoo desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``cadoo/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(cadoo_home)
    ...
    return [
        ...
        cadoo_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Cadoo" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Cadoo.app"),
            home / "Applications" / "Cadoo.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Cadoo).
            local_base / "Programs" / "Cadoo",
            # Older / alternate layout some builds used.
            local_base / "cadoo-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Cadoo")
    else:
        ...
        paths += [
            data_base / "applications" / "cadoo.desktop",
            data_base / "applications" / "Cadoo.desktop",
        ]
    return paths


def agent_is_installed(cadoo_home: Path) -> bool:
    """Return True when a usable Python agent install exists under CADOO_HOME.
    ...
    if (agent_root / "cadoo_cli").is_dir():
        return True
...
def gui_is_installed(cadoo_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(cadoo_home):
        if p.exists():
            return True
...
def gui_install_summary(cadoo_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = cadoo_home if cadoo_home is not None else get_cadoo_home()
...
def uninstall_gui(cadoo_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``cadoo/cadoo_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$CADOO_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove cadoo' or "
            "'sudo dnf remove cadoo'). AppImage builds are a single file you "
