...
on Japanese locales, etc.).  Cadoo's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``cadoo_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Cadoo is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Cadoo-managed Git install directories so
    ...
    # already-running shells don't see the change — which means cadoo
    ...
    # install without Cadoo-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Cadoo's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Cadoo's
    ``cadoo config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Cadoo (User env var in Windows Settings, or
    export in a PowerShell profile) and Cadoo picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Cadoo-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\cadoo\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``cadoo`` from right after install) retain their old PATH.

    Any subprocess Cadoo spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Cadoo-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "cadoo", "git", "cmd"),
        os.path.join(local_appdata, "cadoo", "git", "bin"),
        os.path.join(local_appdata, "cadoo", "git", "usr", "bin"),
        # Cadoo venv Scripts directory — host of the cadoo.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes cadoo, but harmless to include.
        os.path.join(local_appdata, "cadoo", "cadoo", "venv", "Scripts"),
