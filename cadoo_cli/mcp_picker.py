"""MCP picker — interactive `cadoo mcp picker` (also the default `cadoo mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``cadoo mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `cadoo plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from cadoo_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Cadoo session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from cadoo_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `cadoo mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: cadoo mcp install <name>    Picker: cadoo mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Cadoo is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Cadoo — run `cadoo update` "
...
def show_catalog() -> None:
    """`cadoo mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`cadoo mcp picker` (and default `cadoo mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`cadoo mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `cadoo mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
