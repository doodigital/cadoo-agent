"""
Unified tool configuration for Cadoo Agent.

`cadoo tools` and `cadoo setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.cadoo/config.yaml under
...
from cadoo_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from cadoo_cli.nous_account import format_nous_portal_entitlement_message
...
from cadoo_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `cadoo tools` → Video Generation, which walks
...
# `cadoo tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from cadoo_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `cadoo tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``cadoo tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from cadoo_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``cadoo tools`` checklist actually offers
    for ``platform``.
    ...
    so ``cadoo tools`` never claims to add or remove a toolset the user was
...
from cadoo_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "video_gen": {
        ...
        # FAL video generation billed via the DooStudio Portal.  Plugin-backed
    ...
    "x_search": {
        ...
        "setup_note": (
            "Cadoo routes X searches through xAI's built-in x_search "
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Cadoo telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``cadoo update`` and
      by ``cadoo computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `cadoo update` calls this
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Cadoo process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_cadoo_home()}/cadoo && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in cadoo directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/cadoo:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.cadoo/config.yaml")
        ...
        # Run the full `cadoo auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.cadoo/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from cadoo_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: cadoo auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `cadoo auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: cadoo auth spotify")
        except Exception as exc:
        ...
        try:
            from cadoo_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: cadoo plugins enable observability/langfuse")
        _print_info("    Restart Cadoo for tracing to take effect.")
        _print_info("    Verify: cadoo plugins list")
        ...
        try:
            ...
            _print_info("    Run later: cadoo auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `cadoo auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: cadoo auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: cadoo auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``cadoo tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``cadoo tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: cadoo tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # ``[cadoo-cli, spotify]`` after enabling Spotify via ``cadoo
        ...
        # (e.g. "cadoo-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``cadoo tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `cadoo tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"cadoo-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("cadoo-")}
    ...
    # shipped as a bundled plugin but user must opt in via `cadoo tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `cadoo tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by cadoo tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (cadoo-cli, cadoo-telegram, etc.)
    ...
    # Opening `cadoo tools` is the user's opt-in to reconfigure tools, so treat
...
def _visible_providers(
    ...
    *,
    ...
    that the capability exists.  Selecting one drives an inline DooStudio Portal
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `cadoo tools` gets a silent no-op
    ...
    *,
    ...
    if len(providers) == 1:
        ...
        for p in providers:
            ...
            # "included" star; everyone else gets a "via DooStudio Portal" hint so
            ...
            if p.get("managed_nous_feature"):
                if _nous_logged_in:
                    ...
                    sub_marker = "  ★ via DooStudio Portal (login on select)"
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on DooStudio Portal auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `cadoo model`).
    if managed_feature:
        from cadoo_cli.nous_subscription import (
        ...
        if not ensure_nous_portal_access(
            ...
            _print_warning(
                "  Not enabled — DooStudio Portal access is required for this backend."
            )
            return
    ...
    if provider.get("requires_nous_auth") and not managed_feature:
        ...
        if not features.nous_auth_present or not entitled:
            ...
            _print_warning(
                f"  {message or 'Nous Subscription is only available after logging into DooStudio Portal.'}"
            )
            return
    ...
    if _show_portal_hint:
        _print_info("  Available through DooStudio Portal subscription.")
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline DooStudio Portal login + entitlement gate as _configure_provider:
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `cadoo tools` and `cadoo setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("🟣 Cadoo Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://doostudio.com.br/cadoo/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from cadoo_constants import display_cadoo_home
    print(color(f"  Tool configuration saved to {display_cadoo_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'cadoo' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, cadoo mcp
