...
# xAI's public API is OpenAI-compatible for the endpoints Cadoo commonly
# uses. The Responses endpoint is included because Cadoo' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Cadoo-managed OAuth credentials."""

    auth_hint = "cadoo auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`cadoo auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`cadoo auth reset xai-oauth` or re-authenticate with "
                    "`cadoo auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `cadoo auth add xai-oauth --type oauth`."
            )
