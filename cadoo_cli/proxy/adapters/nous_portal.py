"""DooStudio Portal upstream adapter.

Reads the user's Nous OAuth state from ``~/.cadoo/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the DooStudio Portal inference API."""
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into DooStudio Portal. Run `cadoo auth add nous` first."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to refresh DooStudio Portal credentials: {exc}"
                ) from exc
            ...
            if not runtime_key:
                raise RuntimeError(
                    "DooStudio Portal refresh did not return a usable inference JWT. "
                    "Try `cadoo auth add nous` to re-authenticate."
                )
    ...
    # to cadoo_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
