...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_cadoo_home() / "sessions")
    ...
    # gateway behaves exactly as before — single CADOO_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.cadoo/config.yaml (primary user-facing config)
    3. ~/.cadoo/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_cadoo_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through cadoo_cli.config.load_config, so without this a managed
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``cadoo config set gateway.streaming.*``
