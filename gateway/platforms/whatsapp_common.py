...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Cadoo Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to CADOO_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/cadoo), this function
    mirrors the bridge source to a writable CADOO_HOME location and returns that
    ...
    # Try CADOO_HOME location first
    cadoo_home = get_cadoo_home()
    cadoo_home_bridge = cadoo_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to CADOO_HOME if needed
    if cadoo_home_bridge.exists():
        return cadoo_home_bridge

    # Mirror the bridge source to CADOO_HOME
    try:
        cadoo_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            cadoo_home_bridge,
            dirs_exist_ok=False,
