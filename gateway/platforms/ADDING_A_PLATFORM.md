...
There are two ways to add a platform to the Cadoo gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.cadoo/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Cadoo code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `cadoo gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `cadoo_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Cadoo core.
...
And add it to the `cadoo-gateway` composite:

```python
"cadoo-gateway": {
    "includes": [..., "cadoo-your-platform"]
}
```
...
## 12. Status Display (`cadoo_cli/status.py`)
...
## 13. Gateway Setup Wizard (`cadoo_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ cadoo_cli/ toolsets.py \
  --include="*.py" -l | sort -u
