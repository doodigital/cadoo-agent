...
# Audio file extensions Cadoo recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Cadoo' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Cadoo-created Telegram private-chat
...
from cadoo_constants import get_default_cadoo_root, get_hermes_dir, get_cadoo_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.cadoo/.env manually."
)
...
# Default location: {CADOO_HOME}/cache/images/ (legacy: image_cache/)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; CadooAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
_HERMES_ROOT = get_default_cadoo_root()
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _CADOO_HOME / "image_cache",
    _CADOO_HOME / "audio_cache",
    _CADOO_HOME / "video_cache",
    _CADOO_HOME / "document_cache",
    _CADOO_HOME / "browser_screenshots",
    ...
    _CADOO_HOME / "cache" / "images",
    _CADOO_HOME / "cache" / "audio",
    _CADOO_HOME / "cache" / "videos",
    _CADOO_HOME / "cache" / "documents",
    _CADOO_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Cadoo profile and shared Cadoo root both contain control
    ...
    # CADOO_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.cadoo
    ...
    for hermes_root in (_CADOO_HOME, _HERMES_ROOT):
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Cadoo
    secrets (``~/.cadoo/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Cadoo-managed cache, under an operator-allowlisted root
    ...
    # credential/secret stores under the Cadoo root (~/.cadoo/.env,
    ...
    # ``MEDIA:~/.cadoo/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?cadoo\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+cadoo[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Cadoo commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Cadoo sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.cadoo/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from cadoo_cli.commands import should_bypass_active_session
