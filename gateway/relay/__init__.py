"""Relay/connector support package for the Cadoo gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``cadoo gateway enroll`` writes them to
    ``~/.cadoo/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``cadoo gateway enroll``).
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.cadoo/.env``.
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``cadoo gateway enroll``: has a PINNED
    ...
    try:
        from cadoo_cli.auth import resolve_nous_access_token
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'cadoo'}"
