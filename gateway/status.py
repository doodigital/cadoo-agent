...
The PID file lives at ``{CADOO_HOME}/gateway.pid``.  CADOO_HOME defaults to
``~/.cadoo`` but can be overridden via the environment variable.  This means
separate CADOO_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting CADOO_HOME."""
    home = get_cadoo_home()
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "cadoo" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Cadoo gateway lifecycle subcommand from a command line.
    ...
    test also matched ``cadoo_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\cadoo-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Cadoo's
    ...
    for token in tokens:
        ...
        if basename in ("cadoo-gateway", "cadoo-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "cadoo_cli.main" in joined
        or "cadoo_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("cadoo", "cadoo.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `cadoo gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Cadoo-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Cadoo gateway."""
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `cadoo status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``CADOO_HOME`` in-process.  Defaults to
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different CADOO_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``cadoo.service`` + ``cadoo-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``cadoo gateway stop`` on Windows would be misclassified as an
