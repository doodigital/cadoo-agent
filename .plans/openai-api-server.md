# OpenAI-Compatible API Server for Cadoo Agent
...
SSE streaming. By exposing this endpoint, cadoo becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.cadoo/.env`
2. Run `cadoo gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with cadoo through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns cadoo as a model) |
...
Authorization: Bearer cadoo-api-key-here
...
But cadoo sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "cadoo"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured cadoo model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of cadoo's core prompt
...
Once implemented, cadoo works as a drop-in backend for:
