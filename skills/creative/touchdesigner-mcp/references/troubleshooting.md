...
## 5. Cadoo Configuration

### Config location

`$CADOO_HOME/config.yaml` (defaults to `~/.cadoo/config.yaml` when `CADOO_HOME` is unset)
...
Restart the Cadoo session for changes to take effect. The MCP connection is
