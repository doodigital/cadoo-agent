"""Windows UTF-8 bootstrap for Cadoo entry points.
...
should be imported at the very top of every Cadoo entry point
(``cadoo``, ``cadoo``, ``cadoo-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Cadoo modules.

    Cadoo ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Cadoo source root
...
# Apply on import — entry points just need ``import cadoo_bootstrap``
# (or ``from cadoo_bootstrap import apply_windows_utf8_bootstrap``) at
