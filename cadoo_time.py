"""
Timezone-aware clock for Cadoo.
...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.cadoo/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Cadoo never
...
from cadoo_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Cadoo requires 3.9+)
