...
The canonical transcript store is SQLite via `SessionDB` (from `cadoo_state`). The
...
Counts consecutive restarts via a JSON file (`{CADOO_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `cadoo update`, `cadoo gateway restart`,
or `/restart`.
