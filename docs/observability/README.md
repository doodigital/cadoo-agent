# Cadoo Observer Hooks

Cadoo observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Cadoo' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Cadoo catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Cadoo only builds
