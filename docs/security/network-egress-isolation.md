...
When running Cadoo inside Docker, the default `network_mode: host` gives the
...
The Cadoo [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=cadoo,cadoo-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://cadoo-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Cadoo trust model and vulnerability reporting
