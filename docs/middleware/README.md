...
Cadoo middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Cadoo'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Cadoo calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Cadoo retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Cadoo guardrails,
...
- `telemetry_schema_version`: currently `cadoo.observer.v1`
- `middleware_schema_version`: currently `cadoo.middleware.v1`
...
Cadoo stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Cadoo runs
them as a nested chain in registration order. Middleware failures are fail-open:
Cadoo logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Cadoo applies middleware in this order:
...
For each tool call, Cadoo applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Cadoo pre-execution path against the effective arguments:
...
```bash
cadoo plugins enable <plugin-name>
```

For isolated local testing, use one `CADOO_HOME` for plugin enablement and the
agent run:

```bash
export CADOO_HOME=/tmp/cadoo-middleware-test
mkdir -p "$CADOO_HOME"
cadoo plugins enable <plugin-name>
cadoo chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run cadoo plugins enable <plugin-name>
uv run cadoo chat --query 'Reply exactly ok'
```
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/cadoo-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Cadoo expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Cadoo treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Cadoo preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Cadoo does not convert downstream
