...
Supersedes: PR #31781 (prompt_toolkit `cadoo profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `cadoo profile wizard` in the terminal.
...
needs, and a profile is just a CADOO_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.cadoo/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — CADOO_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with CADOO_HOME override |
...
### Seam #1 — hub-skill install cannot use the CADOO_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = CADOO_HOME / "skills"` at **module
import time**. The context-local `set_cadoo_home_override()` swap (which makes
...
runs `python -m cadoo_cli.main <subcommand>`, and `_apply_profile_override()`
...
A fresh subprocess re-imports `skills_hub` with the profile's CADOO_HOME bound
...
2. write model (synchronous, CADOO_HOME override)
3. write selected MCP servers (synchronous, CADOO_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `cadoo -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, CADOO_HOME override
...
page afterward. Mirror `_write_profile_model`'s CADOO_HOME-override helper for
...
- Backend E2E with isolated CADOO_HOME: POST a full create body
