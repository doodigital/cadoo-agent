...
This document is the formal interface between the **Cadoo gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`DooStudio/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `cadoo gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`cadoo gateway enroll` → connector `/relay/enroll`, or
...
(`DooStudio/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `cadoo` CLI.
