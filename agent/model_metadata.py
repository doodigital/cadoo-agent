...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Cadoo also honours HERMES_CA_BUNDLE (its own convention)
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_cadoo_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Cadoo Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Cadoo from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.doostudio.com.br": "nous",
    ...
    # (often 8K) makes it unusable for Cadoo' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    from cadoo_constants import get_cadoo_home
    return get_cadoo_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Cadoo grow conversations past the runtime limit and Ollama
...
def _resolve_nous_context_length(
    ...
    """Resolve DooStudio Portal model context length.
...
def get_model_context_length(
    ...
    if base_url and provider != "lmstudio":
        ...
        if cached is not None:
            ...
            # leftover — drop it and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # DooStudio Portal: the portal /v1/models endpoint is authoritative.
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from cadoo_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Cadoo sends to providers:
