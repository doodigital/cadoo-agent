...
from cadoo_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # The Cadoo Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``cadoo_cli.config`` on the
...
# which becomes the dominant cost of ``cadoo`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.cadoo/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``cadoo`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against CADOO_HOME, not cwd
        if not p.is_absolute():
            p = (cadoo_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.cadoo/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    cadoo = metadata.get("cadoo") or {}
    if not isinstance(cadoo, dict):
        cadoo = {}
    return {
        "fallback_for_toolsets": cadoo.get("fallback_for_toolsets", []),
        "requires_toolsets": cadoo.get("requires_toolsets", []),
        "fallback_for_tools": cadoo.get("fallback_for_tools", []),
        "requires_tools": cadoo.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    cadoo = metadata.get("cadoo")
    if not isinstance(cadoo, dict):
        return []
    raw = cadoo.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Cadoo metadata, VCS, virtualenv/dependency, cache, and skill
