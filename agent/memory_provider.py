...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `cadoo backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - cadoo_home (str): The active CADOO_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.cadoo``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "cadoo").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'cadoo memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], cadoo_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'cadoo memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``cadoo_home`` is the active CADOO_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE CADOO_HOME.

        ``cadoo backup`` only walks CADOO_HOME, so any provider state kept
        ...
        of the archive, and ``cadoo import`` restores them to their original
