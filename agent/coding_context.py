"""Coding-context awareness — base Cadoo, every interactive surface.

When the user runs Cadoo inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Cadoo shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("cadoo.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "cadoo", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _CADOO_CORE_TOOLS, so they're present on every surface this fires on.
...
def is_coding_context(
    *,
    ...
    """Whether Cadoo should operate in its coding posture right now."""
