...
upstream prefix cache warm.  See ``cadoo-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Cadoo never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CADOO_HOME persona while keeping
    ...
    # Pointer to the cadoo skill + docs for user questions about Cadoo itself.
    stable_parts.append(CADOO_AGENT_HELP_GUIDANCE)
    ...
    # Coding posture (base Cadoo, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Cadoo profile the agent is running
    # under so it doesn't conflate ~/.cadoo/skills/ (default profile) with
    # ~/.cadoo/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Cadoo profile: default. Other profiles (if any) live "
            "under ~/.cadoo/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Cadoo profile: {active_profile}. This session reads "
            f"and writes ~/.cadoo/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.cadoo/skills/, ~/.cadoo/plugins/, "
            f"~/.cadoo/cron/, ~/.cadoo/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Cadoo never rebuilds or reinjects parts of it
