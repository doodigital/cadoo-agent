...
Owns one Codex thread per Cadoo session. Drives `turn/start`, consumes
...
# Cadoo' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Cadoo/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Cadoo session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Cadoo' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Cadoo' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own cadoo-tools callback we
            # auto-accept — the user already approved Cadoo' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Cadoo approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Cadoo returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort Cadoo version string for codex's userAgent line."""
    try:
