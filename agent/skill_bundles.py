...
Storage
-------
Bundles live in ``~/.cadoo/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CADOO_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``cadoo bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CADOO_HOME.

    Honors ``HERMES_BUNDLES_DIR`` for tests; falls back to
    ``<CADOO_HOME>/skill-bundles``.
    """
    override = os.environ.get("HERMES_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_cadoo_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `cadoo bundles` CLI subcommand.
# ---------------------------------------------------------------------------
