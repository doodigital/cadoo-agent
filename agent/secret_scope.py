...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the CADOO_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Cadoo runtime / deployment
    "CADOO_HOME", "HERMES_PROFILE", "HERMES_GATEWAY_LOCK_DIR",
...
def build_profile_secret_scope(cadoo_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(cadoo_home) / ".env")
