...
Cadoo pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.cadoo/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<cadoo_home>/bin/bws`` on
  first use.  Cadoo pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.cadoo/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``cadoo`` invocations don't
  hammer the API.
* Failures NEVER block Cadoo startup.  Missing binary, no network,
...
# In-process cache so repeated load_cadoo_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `cadoo chat -q ...`
...
# <cadoo_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.cadoo/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under cadoo_home/cache/.

    `home_path` is what `load_cadoo_dotenv()` already resolved; falling back
    to `$CADOO_HOME` / `~/.cadoo` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("CADOO_HOME", Path.home() / ".cadoo"))
...
def _hermes_bin_dir() -> Path:
    """Where Cadoo stores its managed binaries.  Profile-aware."""
    from cadoo_constants import get_cadoo_home

    return get_cadoo_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<cadoo_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``cadoo secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="cadoo-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "cadoo"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<cadoo_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from cadoo_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_cadoo_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `cadoo secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `cadoo secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `cadoo secrets bitwarden setup` to install."
        )
        return result
