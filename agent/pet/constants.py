...
row taxonomy is inferred from the atlas shape so Cadoo can render both legacy
...
# rows (1536x1872), while older Cadoo/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Cadoo' activity state names. They are not always identical to the
...
# Legacy Cadoo/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Cadoo activity names -> accepted row-name aliases in descending
