...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_CADOO_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Cadoo expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``cadoo_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_cadoo_home()`` themselves.
        """
        if "cadoo_home" not in kwargs:
            from cadoo_constants import get_cadoo_home
            kwargs["cadoo_home"] = str(get_cadoo_home())
        for provider in self._providers:
            try:
