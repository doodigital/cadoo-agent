...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Cadoo with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.cadoo/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Cadoo
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``cadoo tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``cadoo setup`` for
