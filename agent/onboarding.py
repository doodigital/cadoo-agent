...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Cadoo starts and finds ``~/.openclaw/``.

    Points users at ``cadoo claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``cadoo claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Cadoo, run "
        "`cadoo claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `cadoo claw cleanup` (renames it to ~/.openclaw.pre-migration — "
