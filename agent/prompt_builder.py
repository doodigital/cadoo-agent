...
from cadoo_constants import get_cadoo_home, get_skills_dir, is_wsl
...
_HERMES_MD_NAMES = (".cadoo.md", "HERMES.md")


def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.cadoo.md`` or ``HERMES.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Cadoo Agent, an intelligent AI assistant created by DooStudio. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.cadoo/kanban.db`. Your task id is in "
    ...
    "every assignee in a real profile (`cadoo profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `cadoo kanban <verb>` for board operations. Use "
...
#
# The cadoo runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to cadoo's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `cadoo computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Cadoo "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Cadoo terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Cadoo WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Cadoo itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Cadoo runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Cadoo itself is running. The host OS, home, and cwd "
                f"of the Cadoo process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Cadoo "
    ...
    # Cadoo desktop GUI — any agent running under the desktop app should know
    # it. HERMES_DESKTOP marks the backend powering the chat; HERMES_DESKTOP_TERMINAL
    # marks a cadoo launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Cadoo desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Cadoo
...
def _skills_prompt_snapshot_path() -> Path:
    return get_cadoo_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.cadoo/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Cadoo Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `cadoo` skill "
            "first. It has the actual commands (e.g. `cadoo config set …`, `cadoo tools`, "
            "`cadoo setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: cadoo setup, cadoo setup tools, cadoo setup terminal, cadoo status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from CADOO_HOME and return its content, or None.
    ...
    """
    try:
        from cadoo_cli.config import ensure_cadoo_home
        ensure_cadoo_home()
    except Exception as e:
        logger.debug("Could not ensure CADOO_HOME before loading SOUL.md: %s", e)

    soul_path = get_cadoo_home() / "SOUL.md"
...
def _load_hermes_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.cadoo.md / HERMES.md — walk to git root."""
    ...
    try:
        ...
        return _truncate_content(
            result, ".cadoo.md", context_length=context_length,
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .cadoo.md / HERMES.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from CADOO_HOME is independent and always included when present.
    ...
    # SOUL.md from CADOO_HOME only — skip when already loaded as identity
    if not skip_soul:
