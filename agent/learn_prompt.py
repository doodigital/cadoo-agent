...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a single ``SKILL.md`` via ``skill_manage`` that follows the Cadoo
     skill-authoring standards (description <=60 chars, the modern section
     order, Cadoo-tool framing, no invented commands).
...
# (HARDLINE)" and the cadoo-dev new-skill salvage reference. Embedded in
# the prompt so the agent authors skills the way a maintainer would by hand.
_AUTHORING_STANDARDS = """\
Follow the Cadoo skill-authoring standards exactly:
...
- version: 0.1.0
- metadata.cadoo.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Cadoo tools.
...
Cadoo-tool framing (this is what makes it a skill, not shell docs):
