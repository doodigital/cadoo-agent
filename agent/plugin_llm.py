...
Plugins built on Cadoo Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Cadoo
...
:class:`~cadoo_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Cadoo' main config
...
handles every provider, fallback chain, and per-task override Cadoo
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`cadoo_cli.plugins.PluginContext`
