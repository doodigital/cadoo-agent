...
``auth.json``.  They intentionally have no dependency on ``cadoo_cli.auth`` so
...
# Sources Cadoo owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Cadoo-owned OAuth/device-code state)
