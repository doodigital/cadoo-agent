...
Resolution order for text tasks (auto mode):
  ...
  3. DooStudio Portal (~/.cadoo/auth.json active provider)
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Cadoo triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://doostudio.com.br/cadoo",
    "X-Title": "Cadoo Agent",
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "CadooAgent",
}
...
# DooStudio Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by DooStudio Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with cadoo_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    """Return a fresh DooStudio Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``cadoo_cli.__version__`` is
...
# Set at resolve time — True if the auxiliary client points to DooStudio Portal
auxiliary_is_nous: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_NOUS_MODEL = "google/gemini-3-flash-preview"
_NOUS_DEFAULT_BASE_URL = "https://inference-api.doostudio.com.br/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_cadoo_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Cadoo Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``cadoo_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.cadoo/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from cadoo_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from cadoo_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``cadoo auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``cadoo_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from cadoo_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Cadoo auth store.
    ...
    try:
        from cadoo_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from cadoo_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from cadoo_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from cadoo_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping DooStudio Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    logger.debug("Auxiliary client: DooStudio Portal")
    ...
    model = _NOUS_MODEL
    try:
        from cadoo_cli.models import get_nous_recommended_aux_model
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the DooStudio Portal's recommended model after a stale-model 404.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `cadoo setup` or `cadoo model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`cadoo_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from cadoo_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second cadoo
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``cadoo config aux reset``)."""
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.doostudio.com.br"):
        return "nous"
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Cadoo' built-in discovery
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `cadoo model` but the
    #    old OPENAI_BASE_URL lingers in ~/.cadoo/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: cadoo model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.cadoo/.env",
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but DooStudio Portal not configured (run: cadoo auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: cadoo model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: cadoo model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from cadoo_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: cadoo doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("cadoo_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from cadoo_cli.models import _should_use_copilot_responses_api
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #   3. DooStudio Portal (vision-capable aggregator fallback)
        #   4. Stop
        main_provider = _read_main_provider()
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes DooStudio Portal product tags when the auxiliary client is backed
    by DooStudio Portal. Returns empty dict otherwise.
    """
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`cadoo_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def call_llm(
    ...
    *,
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `cadoo model`."
                )
    ...
    try:
        ...
        # ── Stale-model self-heal (DooStudio Portal recommendation drift) ───
        ...
        _heal_is_nous = (
            ...
            or base_url_host_matches(_base_info, "inference-api.doostudio.com.br")
        )
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        _heal_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.doostudio.com.br")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_nous_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
