...
def _cadoo_home_path() -> Path:
    """Resolve the active CADOO_HOME (profile-aware) without circular imports."""
    try:
        from cadoo_constants import get_cadoo_home  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            str(cadoo_home / ".anthropic_oauth.json"),
...
def is_write_denied(path: str) -> bool:
    ...
    hermes_dirs = []
    for base in (_cadoo_home_path(), _hermes_root_path()):
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Cadoo path.

    Three categories are blocked:

      * Internal Cadoo cache files under ``CADOO_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under CADOO_HOME and the global Cadoo
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.cadoo/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active CADOO_HOME (profile-aware) AND the global
    # Cadoo root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (CADOO_HOME points at
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Cadoo cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Cadoo credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Cadoo MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Cadoo MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Cadoo profiles are separate CADOO_HOME dirs under
...
#
# Reference: May 2026 incident where a cadoo-security profile session
# edited skills under both ``~/.cadoo/profiles/cadoo-security/skills/``
# AND ``~/.cadoo/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under CADOO_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from CADOO_HOME.
    ...
    """
    try:
        home_real = _cadoo_home_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Cadoo scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Cadoo entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Cadoo profile {info['target_profile']!r}, but the "
...
#
#   <CADOO_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.cadoo/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.cadoo/…`` segment and warns
# regardless of which Cadoo profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.cadoo/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.cadoo`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.cadoo/…`` and returns the
    index where the inner Cadoo-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .cadoo / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".cadoo":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Cadoo state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.cadoo``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.cadoo`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Cadoo resolver to
    succeed, so it works correctly even when called from contexts where
    CADOO_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Cadoo process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real CADOO_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.cadoo/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.cadoo/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host CADOO_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Cadoo state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Cadoo "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real CADOO_HOME. Use the "
