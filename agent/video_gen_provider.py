...
as ``kind: backend``) or ``~/.cadoo/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``cadoo tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CADOO_HOME/cache/videos/``, creating parents as needed."""
    from cadoo_constants import get_cadoo_home

    path = get_cadoo_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CADOO_HOME/cache/videos/``.
