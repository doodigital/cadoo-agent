# Cadoo Agent - Development Guide

Instructions for AI coding assistants and developers working on the cadoo codebase.

**Never give up on the right solution.**

## What Cadoo Is

Cadoo is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Cadoo ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`cadoo tools`, `cadoo setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `CADOO_HOME`. Mocks hide integration bugs.
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `cadoo tools`
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `cadoo <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `cadoo webhook`, `cadoo cron`, `cadoo tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.cadoo/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.cadoo/cadoo/venv` (for worktrees that share a venv with the
main checkout).
...
├── cli.py                # CadooCLI class — interactive CLI orchestrator (~11k LOC)
├── cadoo_state.py       # SessionDB — SQLite session store (FTS5 search)
├── cadoo_constants.py   # get_cadoo_home(), display_cadoo_home() — profile-aware paths
├── cadoo_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── cadoo_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── cadoo-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `cadoo --tui`
...
**User config:** `~/.cadoo/config.yaml` (settings), `~/.cadoo/.env` (API keys only).
**Logs:** `~/.cadoo/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_cadoo_home()`.
Browse with `cadoo logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Cadoo: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`cadoo_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `CadooCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.cadoo/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`cadoo_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/cadoo` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `cadoo_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `CadooCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `cadoo --tui` or `HERMES_TUI=1`.
...
npm run dev       # watch mode (rebuilds cadoo-ink + tsx --watch)
npm start         # production
npm run build     # full build (cadoo-ink + tsc)
...
### TUI in the Dashboard (`cadoo dashboard` → `/chat`)

The dashboard embeds the real `cadoo --tui` — **not** a rewrite.  See `cadoo_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `cadoo_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `cadoo --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `cadoo --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `cadoo --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `cadoo-desktop-app-work` skill, not `cadoo-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Cadoo built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Cadoo core.
Use the plugin route instead: create `~/.cadoo/plugins/<name>/plugin.yaml`
and `~/.cadoo/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Cadoo tool that should ship in the base system.
...
**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_cadoo_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `CADOO_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_cadoo_home()` for the base directory — never `Path.home() / ".cadoo"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `cadoo_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `cadoo_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `cadoo tools`, `cadoo setup`, most CLI subcommands | `cadoo_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`cadoo_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
cadoo_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.cadoo/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Cadoo gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `cadoo_cli/skin_engine.py`:
...
Users create `~/.cadoo/skins/<name>.yaml`:
...
Cadoo has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.cadoo/plugins/` and pip-installed entry points.

### General plugins (`cadoo_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.cadoo/plugins/`, `./.cadoo/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `cadoo` at startup so
  `cadoo <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(cadoo_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `cadoo <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `cadoo --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `cadoo_cli/main.py`, etc.).
...
into `~/.cadoo/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `cadoo memory setup` / `post_setup()` without
...
2. User: `$CADOO_HOME/plugins/model-providers/<name>/`
...
[`cadoo-example-plugins`](https://github.com/doostudio/cadoo-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `cadoo skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.cadoo.tags`, `metadata.cadoo.category`,
`metadata.cadoo.related_skills`, `metadata.cadoo.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.cadoo.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Cadoo tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Cadoo Agent" is the secondary collaborator. If the
   contributor's commit shows "Cadoo Agent" as author (because they
   used Cadoo to draft the skill), replace it with their actual name
...
lives in the `cadoo-dev` skill at
...
Enable/disable per platform via `cadoo tools` (the curses UI) or the
...
Cadoo returns a delegation id immediately and the result re-enters the
...
go to `~/.cadoo/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `cadoo_cli/curator.py` wires `cadoo curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.cadoo/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `cadoo cron <verb>`
...
- File lock at `~/.cadoo/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `cadoo kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `cadoo_cli/kanban.py` wires `cadoo kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`cadoo-kanban-dispatcher.service` for
...
Cadoo-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
Cadoo supports **profiles** — multiple fully isolated instances, each with its own
`CADOO_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `cadoo_cli/main.py` sets
`CADOO_HOME` before any module imports. All `get_cadoo_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_cadoo_home()` for all CADOO_HOME paths.** Import from `cadoo_constants`.
   NEVER hardcode `~/.cadoo` or `Path.home() / ".cadoo"` in code that reads/writes state.
   ```python
   # GOOD
   from cadoo_constants import get_cadoo_home
   config_path = get_cadoo_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".cadoo" / "config.yaml"
   ```

2. **Use `display_cadoo_home()` for user-facing messages.** Import from `cadoo_constants`.
   This returns `~/.cadoo` for default or `~/.cadoo/profiles/<name>` for profiles.
   ```python
   # GOOD
   from cadoo_constants import display_cadoo_home
   print(f"Config saved to {display_cadoo_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.cadoo/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_cadoo_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_cadoo_home()`,
   not `Path.home() / ".cadoo"`.

4. **Tests that mock `Path.home()` must also set `CADOO_HOME`** — since code now uses
   `get_cadoo_home()` (reads env var), not `Path.home() / ".cadoo"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"CADOO_HOME": str(tmp_path / ".cadoo")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not CADOO_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".cadoo" / "profiles"`, NOT `get_cadoo_home() / "profiles"`.
   This is intentional — it lets `cadoo -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.cadoo` paths
Use `get_cadoo_home()` from `cadoo_constants` for code paths. Use `display_cadoo_home()`
for user-facing print/log messages. Hardcoding `~/.cadoo` breaks profiles — each profile
has its own `CADOO_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `cadoo_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `cadoo_cli/curses_ui.py` — see
`cadoo_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `CADOO_HOME`.

### Tests must not write to `~/.cadoo/`
The `_isolate_cadoo_home` autouse fixture in `tests/conftest.py` redirects `CADOO_HOME` to a temp dir. Never hardcode `~/.cadoo/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_cadoo_home()` resolve within the temp dir.
Use the pattern from `tests/cadoo_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CADOO_HOME", str(home))
    return home
```
...
| HOME / `~/.cadoo/` | Your real config+auth.json | Temp dir per test |
