...
// (e.g. https://mission-control.tilos.com/cadoo/). The Python backend
...
const SESSION_HEADER = "X-Cadoo-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``cadoo update``, ``cadoo gateway restart``, etc.). A tab kept
    ...
    if (!window.__HERMES_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      ...
      try {
        alreadyReloaded =
          sessionStorage.getItem("cadoo.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("cadoo.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    try {
      sessionStorage.removeItem("cadoo.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  throw new Error("Session token not available — page must be served by the Cadoo dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``cadoo_cli.web_server._ws_auth_ok``)
 ...
 *  - loopback / ``--insecure``: attach the ``X-Cadoo-Session-Token`` header.
...
export const api = {
  ...
  updateHermes: () =>
    fetchJSON<ActionResponse>("/api/cadoo/update", { method: "POST" }),
  checkHermesUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/cadoo/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * DooStudio Portal contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** cadoo-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``cadoo update``. */
