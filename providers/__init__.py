...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with cadoo)
2. User plugins: ``$CADOO_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$CADOO_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$CADOO_HOME/plugins/model-providers/`` if it exists."""
    try:
        from cadoo_constants import get_cadoo_home

        d = get_cadoo_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple CADOO_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$CADOO_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with cadoo.
    ...
    # 2. User plugins — under $CADOO_HOME/plugins/model-providers/<name>/.
