"""
Cadoo MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Cadoo-specific extra)

Usage:
    cadoo mcp serve
    cadoo mcp serve --verbose
...
logger = logging.getLogger("cadoo.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CADOO_HOME."""
    try:
        from cadoo_constants import get_cadoo_home
        return get_cadoo_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CADOO_HOME", Path.home() / ".cadoo")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from cadoo_constants import get_cadoo_home
        directory_file = get_cadoo_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CADOO_HOME", Path.home() / ".cadoo")
...
class EventBridge:
    ...
    This is the Cadoo equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from cadoo_constants import get_cadoo_home
            db_file = get_cadoo_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CADOO_HOME", Path.home() / ".cadoo")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Cadoo MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "cadoo",
        instructions=(
            "Cadoo Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Cadoo MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
