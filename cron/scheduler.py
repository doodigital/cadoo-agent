...
Uses a file-based lock (~/.cadoo/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `cadoo update` reloads
# the module) fail with ModuleNotFoundError for cadoo_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from cadoo_constants import get_cadoo_home
from cadoo_cli._subprocess_compat import windows_hide_flags
from cadoo_cli.config import load_config, _expand_env_vars
from cadoo_time import now as _hermes_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy cadoo_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from cadoo_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``cadoo tools`` config for the ``cron`` platform.
    ...
    try:
        from cadoo_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_cadoo_home: Path | None = None


def _get_cadoo_home() -> Path:
    """Resolve Cadoo home dynamically while preserving test monkeypatch hooks."""
    return _cadoo_home or get_cadoo_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    cadoo_home = _get_cadoo_home()
    lock_dir = cadoo_home / "cron"
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within CADOO_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Cadoo-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against CADOO_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_cadoo_home() / "scripts"
    ...
    # escape — scripts MUST reside within CADOO_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_cadoo_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_cadoo_home() / ".env"), override=True, encoding="latin-1")
        ...
        try:
            import yaml
            _cfg_path = str(_get_cadoo_home() / "config.yaml")
        ...
        agent = AIAgent(
            ...
            # CADOO_HOME. When a workdir is configured, also inject project
