...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``cadoo://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``cadoo://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"cadoo://blueprint/{quote(blueprint.key)}{qs}"
