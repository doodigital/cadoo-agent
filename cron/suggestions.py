...
A *suggestion* is a ready-to-run cron job spec that Cadoo surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.cadoo/cron/suggestions.json``, atomic
...
CRON_DIR = get_cadoo_home().resolve() / "cron"
