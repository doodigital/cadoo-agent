...
Jobs are stored in ~/.cadoo/cron/jobs.json
Output is saved to ~/.cadoo/cron/output/{job_id}/{timestamp}.md
"""
...
HERMES_DIR = get_cadoo_home().resolve()
CRON_DIR = HERMES_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``cadoo cron status`` process share it
...
# threshold in `cadoo cron status` (cadoo_cli/cron.py), so the two never
# drift apart.
...
def _jobs_lock():
    ...
    and standalone ``cadoo`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Cadoo timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `cadoo_time.now()`, which uses the configured zone. If a
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Cadoo configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Cadoo timezone.
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Cadoo/system local time
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall cadoo or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `cadoo cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``cadoo cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `cadoo cron status` can tell a thread that is merely *alive and looping*
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.cadoo/scripts/; ``.sh`` / ``.bash`` files run via bash,
