...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `cadoo tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    # Coding posture (base Cadoo — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in cadoo_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Cadoo toolsets (CLI + messaging platforms)
    #
    ...
    # the `cadoo send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "cadoo-cron": {
        # Mirrors cadoo-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `cadoo tools` filters
    ...
    "cadoo-gateway": {
        ...
        "tools": [],
        "includes": ["cadoo-telegram", "cadoo-discord", "cadoo-whatsapp", "cadoo-slack", "cadoo-signal", "cadoo-bluebubbles", "cadoo-homeassistant", "cadoo-email", "cadoo-sms", "cadoo-mattermost", "cadoo-matrix", "cadoo-dingtalk", "cadoo-feishu", "cadoo-wecom", "cadoo-wecom-callback", "cadoo-weixin", "cadoo-qqbot", "cadoo-webhook", "cadoo-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    ...
    Bundle nesting is one level deep in practice (only ``cadoo-gateway``
