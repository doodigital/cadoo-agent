...
Let the cadoo agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where cadoo runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `cadoo meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `cadoo meet` CLI |
...
| `cli.py` | `cadoo meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CADOO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `cadoo meet node {run,list,approve,remove,status,ping}` |
...
cadoo meet install                                      # pip + Chromium
cadoo meet setup                                        # preflight
cadoo meet auth                                         # optional
cadoo meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
cadoo meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.cadoo/.env
cadoo meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
cadoo meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
cadoo meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.cadoo/.env
cadoo meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, cadoo will **not** switch your system audio input automatically — the
...
cadoo plugins enable google_meet
cadoo meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
cadoo meet node approve my-mac ws://<mac-ip>:18789 <token>
cadoo meet node ping my-mac
