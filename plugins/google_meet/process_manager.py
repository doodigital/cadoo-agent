...
session-scoped state file under ``$CADOO_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $CADOO_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_cadoo_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this cadoo install, leave it first —
