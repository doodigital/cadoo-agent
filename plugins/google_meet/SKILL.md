...
cadoo meet install                 # pip deps + Chromium (transcribe only)
cadoo meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
cadoo meet auth                    # optional; skips guest-lobby wait
cadoo meet setup                   # preflight checks
```

`cadoo meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or HERMES_MEET_REALTIME_KEY in ~/.cadoo/.env
```
...
pip install playwright websockets && python -m playwright install chromium
cadoo plugins enable google_meet
cadoo meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
cadoo meet node approve my-mac ws://<mac-ip>:18789 <token>
cadoo meet node ping my-mac                   # confirm reachable
```

Run `cadoo meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Cadoo agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `cadoo meet auth` avoids this.
...
Local:
```
$CADOO_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$CADOO_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `cadoo meet node approve`.
