...
``{title, url, description, position}`` rows every other Cadoo web
provider produces.
...
prefers Cadoo-managed xAI Grok OAuth (via ``cadoo auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.cadoo/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``cadoo tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `cadoo auth` to sign in with "
