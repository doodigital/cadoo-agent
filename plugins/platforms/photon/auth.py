...
Credential storage mirrors every other Cadoo channel:

    * runtime SDK creds  -> ``~/.cadoo/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.cadoo/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a cadoo dependency
...
# registers Cadoo as its own client_id.
...
# Default name of the project Cadoo provisions for the operator.
DEFAULT_PROJECT_NAME = "Cadoo Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of cadoo.

def _auth_json_path() -> Path:
    """Resolve ``~/.cadoo/auth.json`` honouring the active Cadoo profile."""
    try:
        from cadoo_constants import get_cadoo_home
        return Path(get_cadoo_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.cadoo")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.cadoo/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.cadoo/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.cadoo/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: cadoo_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `cadoo photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `cadoo photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `cadoo photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `cadoo photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `cadoo photon setup`)"
