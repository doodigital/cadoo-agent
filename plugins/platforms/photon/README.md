...
This plugin connects Cadoo Agent to iMessage (and other Spectrum
...
TypeScript-only, Cadoo runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Cadoo-Sidecar-Token`.
...
There is no separate `login` command; like every other Cadoo channel,
...
Run `cadoo photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.cadoo/.env` (the same place every other
...
Management metadata lives in `~/.cadoo/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `cadoo photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `cadoo photon status`, a DM and a group roundtrip,
