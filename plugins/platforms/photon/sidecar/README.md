...
Small Node helper that bridges Cadoo Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Cadoo is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Cadoo-Sidecar-Token`)
...
The Cadoo plugin's `cadoo photon setup` command runs `npm install`
here automatically.
