"""
IRC Platform Adapter for Cadoo Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Cadoo agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "cadoo-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Cadoo Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Cadoo Agent shutting down")
...
def check_requirements() -> bool:
    ...
    # cadoo setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `cadoo gateway setup` flow for the IRC platform.

    Lazy-imports ``cadoo_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Cadoo to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #cadoo — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.cadoo/.env")
    print_info("Restart the gateway for changes to take effect: cadoo gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "cadoo-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "cadoo-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Cadoo Agent (cron)")
