"""SimpleX Chat platform adapter (Cadoo plugin).
...
This adapter ships as a Cadoo platform plugin under
``plugins/platforms/simplex/``. The Cadoo plugin loader scans the
...
discoverable and ``cadoo setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Cadoo — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``cadoo setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.cadoo/.env`` via
    ``cadoo_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "cadoo_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.cadoo/.env"
        )
        return
