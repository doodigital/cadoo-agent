...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Cadoo command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Cadoo accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Cadoo ``/command`` text."""
...
# Uses get_cadoo_home() so each profile gets its own Matrix store.
from cadoo_constants import get_hermes_dir as _get_hermes_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Cadoo
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "cadoo"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Cadoo Agent" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in cadoo_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces cadoo_cli/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where Cadoo delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via cadoo_cli.gateway.get_env_value so setup-status
