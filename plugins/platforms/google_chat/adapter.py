...
# ``model_tools`` import time, so ``cadoo status``, ``cadoo chat``, etc.
...
#     fresh "Cadoo is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a cadoo MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CADOO_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _cadoo_home = _get_cadoo_home()
        except (ModuleNotFoundError, ImportError):
            _cadoo_home = _Path.home() / ".cadoo"
        self._thread_count_store = _ThreadCountStore(
            _cadoo_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Cadoo is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CADOO_HOME", str(_Path.home() / ".cadoo"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CADOO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a cadoo MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Cadoo is thinking…' marker message.
        ...
        "Cadoo is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Cadoo is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Cadoo is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Cadoo is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Cadoo is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``cadoo setup``.

    The setup wizard at ``cadoo_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.cadoo/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. cadoo-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.cadoo/.env")
    print_info("Restart the gateway: cadoo gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``cadoo cron`` running as a
    separate process from ``cadoo gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Cadoo plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'cadoo[google_chat]'",
        ...
        # the "Cadoo is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Cadoo is thinking…' marker message "
