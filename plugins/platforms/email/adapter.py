"""
Email platform adapter for the Cadoo gateway.

Allows users to interact with Cadoo by sending emails.
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `cadoo config show` already read. Without the
    ...
    async def connect(self) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `cadoo gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Cadoo Agent")
        ...
        msg_id = f"<cadoo-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# cadoo_cli/gateway.py, and the _send_email dispatch in
