...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_cadoo_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Cadoo does not currently need to route them into the agent loop.
            ...
            # /model work directly without /cadoo prefix). A single regex
            ...
            # manifest (see `cadoo slack manifest`). In Socket Mode, Slack
            ...
            from cadoo_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/cadoo$")
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Cadoo handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Cadoo session — matching the per-thread behavior channels already
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/cadoo <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/cadoo
        ...
        if slash_name in {"cadoo", ""}:
            # Legacy /cadoo <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from cadoo_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/cadoo   ``).
        ...
        # questions via "/cadoo <question>" must produce public replies so
...
# ``cadoo_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``cadoo_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under CADOO_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from cadoo_cli.slack_cli import _build_full_manifest
            from cadoo_constants import get_cadoo_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Cadoo",
                bot_description="Your Cadoo agent on Slack",
            )
            target = Path(get_cadoo_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `cadoo slack manifest --write` anytime to refresh after "
                "Cadoo adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `cadoo update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://doostudio.com.br/cadoo/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``cadoo_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'cadoo[slack]'",
        # Interactive setup wizard — replaces cadoo_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in cadoo_cli/gateway.py.
