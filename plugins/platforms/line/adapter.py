"""
LINE Messaging API platform adapter for Cadoo Agent.
...
to Cadoo Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CADOO_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            cadoo_home = Path.home().joinpath(".cadoo").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``cadoo status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``cadoo status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``cadoo setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.cadoo/.env`` via ``cadoo_cli.config``.
    """
    print()
    ...
    try:
        from cadoo_cli.config import get_env_var, set_env_var
    except ImportError:
        print("cadoo_cli.config not available; set LINE_* vars manually in ~/.cadoo/.env")
        return
