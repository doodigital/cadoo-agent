...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {"title": "Cadoo", "text": normalized},
        }
...
# dict in cadoo_cli/gateway.py, and the _send_dingtalk dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces cadoo_cli/setup.py-era _setup_dingtalk + the static
    _PLATFORMS["dingtalk"] dict in cadoo_cli/gateway.py. CLI helpers are
    ...
    if method == 0:
        try:
            from cadoo_cli.dingtalk_auth import dingtalk_qr_auth
