"""ntfy platform adapter (Cadoo plugin).
...
``plugins/platforms/ntfy/``. The Cadoo plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "cadoo-in"              # subscribe topic (incoming)
          publish_topic: "cadoo-out"     # optional — defaults to topic
...
_ECHO_TAG = "cadoo"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``cadoo cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Cadoo dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `cadoo gateway status` without
