...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Cadoo-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on Cadoo' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Cadoo must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `cadoo update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Cadoo or OpenClaw instance is running "
            "with this token, then restart the gateway with 'cadoo gateway restart'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Cadoo process (shared mount, same machine, etc.).
            ...
            try:
                ...
                from cadoo_cli.commands import telegram_menu_commands, telegram_menu_max_commands
                # Telegram allows up to 100 commands but has an undocumented
                # payload size limit (~4KB total).  Cadoo defaults to 60 to
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``cadoo model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from cadoo_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from cadoo_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.cadoo/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".cadoo" / "scripts" / "gmail-triage" / script_name
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Cadoo bot profiles. A message like
...
# cadoo_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in cadoo_cli/gateway.py.
    """
    from cadoo_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'cadoo[telegram]'",
