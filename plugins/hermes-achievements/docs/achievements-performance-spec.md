# Cadoo Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: cadoo-achievements plugin
...
### `GET /api/plugins/cadoo-achievements/achievements`
...
### `POST /api/plugins/cadoo-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/cadoo-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/cadoo-achievements/overview`
...
- Persist plugin data under:
  - `~/.cadoo/plugins/cadoo-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.cadoo/plugins/cadoo-achievements/scan_checkpoint.json`
...
- `~/.cadoo/plugins/cadoo-achievements/`
