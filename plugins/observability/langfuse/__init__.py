"""langfuse — Cadoo plugin for Langfuse observability.

Traces Cadoo conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Cadoo plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``cadoo plugins enable
observability/langfuse`` or ``cadoo tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``cadoo tools`` or ~/.cadoo/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Cadoo plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Cadoo paths only expose ``task_id``/``session_id``. Newer paths
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Cadoo branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Cadoo also has a turn-scoped
    ...
    # messages is a list only for legacy Cadoo branches that fired
    # pre_llm_call with API messages directly. Current Cadoo fires
...
def register(ctx) -> None:
    ...
    # Cadoo versions.  pre_api_request / post_api_request fire per API
