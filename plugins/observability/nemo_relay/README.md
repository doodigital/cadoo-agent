...
Optional Cadoo observability plugin that maps Cadoo observer hooks to
...
not replace Cadoo Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Cadoo emit NeMo Relay lifecycle events for
the work Cadoo already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Cadoo Agent can:

- Preserve Cadoo execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
cadoo plugins enable observability/nemo_relay
```
...
For isolated test homes, enable the plugin in the same `CADOO_HOME` that the
agent run will use:

```bash
env CADOO_HOME=/tmp/cadoo-nemo-relay-test \
  cadoo plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`CADOO_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`CADOO_HOME` is the Cadoo profile/config home used by both
`cadoo plugins enable ...` and the later `cadoo chat ...` run. If unset,
Cadoo uses the user's default home, usually `~/.cadoo`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export CADOO_HOME=/tmp/cadoo-nemo-relay-test
cadoo plugins enable observability/nemo_relay
cadoo chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `cadoo` command you run is built from the
...
uv run cadoo plugins enable observability/nemo_relay
uv run cadoo chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/cadoo-*.whl
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Cadoo does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Cadoo keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Cadoo routes LLM and tool
...
For the full generic Cadoo middleware contract, see
...
export CADOO_HOME=/tmp/cadoo-nemo-relay-docs/cadoo-home
mkdir -p "$CADOO_HOME"

cat > "$CADOO_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Cadoo session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/cadoo-nemo-relay-docs/subagent/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/cadoo-nemo-relay-docs/subagent/atif
export HERMES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Cadoo Agent E2E'
...
{"kind":"mark","name":"cadoo.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Cadoo Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Cadoo dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/cadoo-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/cadoo-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/cadoo-nemo-relay-docs/workdir/beta.txt
cd /tmp/cadoo-nemo-relay-docs/workdir

export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/cadoo-nemo-relay-docs/parallel/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/cadoo-nemo-relay-docs/parallel/atif
...
- Cadoo sessions map to `agent` scopes.
- Cadoo API request hooks map to `llm` scope start/end events.
- Cadoo tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Cadoo execution middleware to hand
...
```bash
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/cadoo-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Cadoo routes execution
...
Cadoo provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Cadoo provider adapter next_call(...)

Cadoo tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Cadoo tool dispatcher next_call(...)
```
...
middleware for a local Cadoo run. This path requires a NeMo Relay runtime that
...
```bash
export CADOO_HOME=/tmp/cadoo-middleware-test/cadoo-home
mkdir -p "$CADOO_HOME" /tmp/cadoo-middleware-test/nemo-relay
...
cat > /tmp/cadoo-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/cadoo-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/cadoo-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Cadoo Middleware E2E"
agent_version = "local"
...
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/cadoo-middleware-test/nemo-relay/plugins.toml
