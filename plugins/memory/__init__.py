...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with cadoo)
2. User-installed providers: ``$CADOO_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$CADOO_HOME/plugins/`` or None if unavailable."""
    try:
        from cadoo_constants import get_cadoo_home
        d = get_cadoo_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$CADOO_HOME/plugins/<name>/``) directories.  Bundled takes
