...
Config file: `$CADOO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `cadoo` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `cadoo-{identity}` → `cadoo-coder`). |
...
All Supermemory API calls send `x-sm-source: cadoo`, and document writes stamp
`metadata.sm_source: cadoo`. This is a **functional routing key, not telemetry**:
it groups Cadoo-written memories into a dedicated "Cadoo" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Cadoo profile:

```json
{
  "container_tag": "cadoo-{identity}"
}
```

For a profile named `coder`, this resolves to `cadoo-coder`. The default profile resolves to `cadoo-default`. Without `{identity}`, all profiles share the same container.
