...
def _load_supermemory_config(cadoo_home: str) -> dict:
    config = _default_config()
    config_path = Path(cadoo_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, cadoo_home: str) -> None:
    config_path = Path(cadoo_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "cadoo"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "cadoo", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `cadoo memory setup`.
        # All other options are documented for $CADOO_HOME/supermemory.json
    ...
    def save_config(self, values, cadoo_home):
        ...
        _save_supermemory_config(sanitized, cadoo_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from cadoo_constants import get_cadoo_home
        self._cadoo_home = kwargs.get("cadoo_home") or str(get_cadoo_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._cadoo_home)
