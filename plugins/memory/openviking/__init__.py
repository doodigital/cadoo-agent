...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Cadoo peer ID in OpenViking (default: cadoo)
...
_DEFAULT_AGENT = "cadoo"
_AGENT_PROMPT_LABEL = "Cadoo peer ID in OpenViking"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Cadoo slash-skill scaffolding before sending content to OpenViking.
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("CADOO_HOME", "")).expanduser() if os.environ.get("CADOO_HOME") else Path.home() / ".cadoo"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Cadoo session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Cadoo only", "write values only to Cadoo .env"),
    ...
    _print_openviking_ready("Connection saved to Cadoo .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See cadoo#28296 review.
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Cadoo peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, cadoo_home: str, config: dict) -> None:
        ...
        from cadoo_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        cadoo_home_path = Path(cadoo_home)
        env_path = cadoo_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Cadoo run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Cadoo' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Cadoo canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See cadoo#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/cadoo/.overview.md).
