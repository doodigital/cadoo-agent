...
```bash
cadoo memory setup    # select "mem0"
```
...
Behavioral settings live in `$CADOO_HOME/mem0.json` (set them via `cadoo memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.cadoo/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `cadoo-user` | User identifier on Mem0 |
...
```bash
cadoo memory setup mem0 --mode oss \
...
```bash
cadoo memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$CADOO_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.cadoo/mem0_qdrant"}}
  }
}
```
...
```bash
cadoo memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$CADOO_HOME/mem0.json`).
