...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/cadoo>
...
```bash
cadoo memory setup honcho   # configure Honcho directly (works on a fresh install)
cadoo memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
cadoo config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.cadoo/.env
```

> `cadoo honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `cadoo memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CADOO_HOME/honcho.json` | Profile-local (isolated Cadoo instances) |
| 2 | `~/.cadoo/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Cadoo profile: `cadoo` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `cadoo honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Cadoo session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`cadoo`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`cadoo`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-cadoo`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening cadoo in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Cadoo session ID (timestamp + hex). Every `cadoo` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Cadoo profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`cadoo`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Cadoo profile: `cadoo` (default) or `hermes_<profile>` (e.g. `cadoo -p coder` -> host key `hermes_coder`). Older `cadoo.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `cadoo`) |
...
|---------|-------------|
| `cadoo memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `cadoo honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `cadoo memory setup`) |
| `cadoo honcho status` | Show resolved config for active profile |
| `cadoo honcho enable` / `disable` | Toggle Honcho for active profile |
| `cadoo honcho mode <mode>` | Change recall or observation mode |
| `cadoo honcho peer --user <name>` | Update user peer name |
| `cadoo honcho peer --ai <name>` | Update AI peer name |
| `cadoo honcho tokens --context <N>` | Set context token budget |
| `cadoo honcho tokens --dialectic <N>` | Set dialectic max chars |
| `cadoo honcho map <name>` | Map current directory to a session name |
| `cadoo honcho sync` | Create host blocks for all Cadoo profiles |
