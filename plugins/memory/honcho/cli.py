...
Handles: cadoo honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "cadoo" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Cadoo profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'cadoo honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'cadoo honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `cadoo update` -- no output, no exceptions.
    """
    try:
        from cadoo_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Cadoo profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CADOO_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_cadoo_home() / "honcho.json"
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom cadoo_cli already uses for gateway refs).
    """
    try:
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Cadoo persistent cross-session memory.")
    ...
    if not is_local:
        ...
        if use_oauth:
            ...
            try:
                ...
                print("  Re-run 'cadoo honcho setup' to retry, or choose an API key instead.\n")
                return
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'cadoo honcho setup' again once you have a key.\n")
                return
    ...
    current_ai = hermes_host.get("aiPeer") or cfg.get("aiPeer", "cadoo")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hermes_host["aiPeer"] = new_ai

    current_workspace = hermes_host.get("workspace") or cfg.get("workspace", "cadoo")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        hermes_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Cadoo GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Cadoo gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    try:
        ...
        print("  Run: cadoo config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    cadoo honcho status     -- show full config")
    print("    cadoo honcho mode       -- change recall/observation mode")
    print("    cadoo honcho tokens     -- tune context and dialectic budgets")
    print("    cadoo honcho peer       -- update peer names")
    print("    cadoo honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Cadoo profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: cadoo honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'cadoo honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'cadoo honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: cadoo honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        cadoo = hosts.get(_host_key(), {})
        user = cadoo.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = cadoo.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = cadoo.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = cadoo.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Cadoo' identity in Honcho. Seed with 'cadoo honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: cadoo honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: cadoo honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = cadoo.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = cadoo.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = cadoo.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Cadoo asks Honcho's AI peer a question")
        ...
        print("\n  Set with: cadoo honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'cadoo honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'cadoo honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    cadoo honcho identity --show        — show both peer representations")
        print("    cadoo honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Cadoo + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Cadoo\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Cadoo persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  cadoo honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'cadoo honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'cadoo honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  cadoo honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'cadoo'")
        print("  with Honcho configured and no prior session history.")
        print("  (Cadoo calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    cadoo honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'cadoo honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Cadoo, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'cadoo honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    cadoo honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  cadoo honcho identity <path/to/SOUL.md>")
    ...
    print("    Cadoo:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Cadoo:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Cadoo:   Honcho observes every message and updates representations")
    ...
    print("    Cadoo:   per-session by default — each run gets its own session")
    print("              Map a custom name:  cadoo honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. cadoo honcho setup              — configure API key (required)")
        print("  2. cadoo honcho migrate            — re-run this walkthrough")
    else:
        print("  1. cadoo honcho status             — verify Honcho connection")
        print("  2. cadoo                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. cadoo honcho identity --show    — verify AI peer representation")
        print("  4. cadoo honcho tokens             — tune context and dialectic budgets")
        print("  5. cadoo honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'cadoo memory setup'...\n")
        from cadoo_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``cadoo honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``cadoo honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to cadoo memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Cadoo Honcho",
    )
