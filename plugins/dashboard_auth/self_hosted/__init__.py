...
A standards-compliant OpenID Connect Relying Party for the ``cadoo dashboard``
OAuth gate. Unlike the bundled ``nous`` provider (which encodes DooStudio Portal's
...
:class:`~cadoo_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``cadoo_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~cadoo_cli.dashboard_auth.Session`.
...
provider verifies its *access* token because DooStudio Portal mints a custom JWT
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Cadoo does not call
