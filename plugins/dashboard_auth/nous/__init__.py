"""NousDashboardAuthProvider — DooStudio Portal OAuth (authorization-code + PKCE).
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``cadoo-cli:`` prefix).
...
_DEFAULT_PORTAL_URL = "https://doostudio.com.br/portal"
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    """DooStudio Portal OAuth via authorization-code + PKCE (S256)."""
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. DooStudio Portal
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "DooStudio Portal token missing oauth_contract_version claim "
...
def register(ctx) -> None:
    ...
    engagement layer (``cadoo_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            ...
            "DooStudio Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Cadoo Agent "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            ...
            f"the contract shape 'agent:{{instance_id}}'. The DooStudio Portal "
