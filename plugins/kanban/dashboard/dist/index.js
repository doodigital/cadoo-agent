/**
 * Cadoo Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.cadoo/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "cadoo-kanban-dot-triage",
    todo: "cadoo-kanban-dot-todo",
    ready: "cadoo-kanban-dot-ready",
    running: "cadoo-kanban-dot-running",
    blocked: "cadoo-kanban-dot-blocked",
    done: "cadoo-kanban-dot-done",
    archived: "cadoo-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-cadoo-task";
  ...
  const DOCS_URL = "https://doostudio.io/cadoo/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://doostudio.io/cadoo/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "cadoo.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="cadoo-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])cadoo-kanban-md-code\1/i.test(attrs)) {
      return ' class="cadoo-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "cadoo-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "cadoo-kanban-md",
  ...
  // standard `drop` event and our `cadoo-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("cadoo-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("cadoo-kanban-column--drop");
          if (target) target.classList.add("cadoo-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("cadoo-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("cadoo-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("cadoo-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "cadoo-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "cadoo-kanban-attention",
        "cadoo-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "cadoo-kanban-attention-bar" },
        h("span", { className: "cadoo-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "cadoo-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "cadoo-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "cadoo-kanban-attention-row",
                  "cadoo-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "cadoo-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "cadoo-kanban-attention-row-id" }, task.id),
                h("span", { className: "cadoo-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "cadoo-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "cadoo-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "cadoo-kanban-diag-action-btn",
      action.suggested ? "cadoo-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " cadoo-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".cadoo-kanban-drawer-comment-row input, .cadoo-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "cadoo-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("cadoo-kanban-diag", sevClass) },
      h("div", { className: "cadoo-kanban-diag-header" },
        h("span", { className: "cadoo-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "cadoo-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "cadoo-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "cadoo-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "cadoo-kanban-diag-data-row" },
                  h("span", { className: "cadoo-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "cadoo-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "cadoo-kanban-diag-data-row" },
                h("span", { className: "cadoo-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "cadoo-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "cadoo-kanban-diag-reassign-row" },
            h("span", { className: "cadoo-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "cadoo-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "cadoo-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "cadoo-kanban-diag-msg",
              msg.ok ? "cadoo-kanban-diag-msg--ok" : "cadoo-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "cadoo-kanban-section" },
      h("div", { className: "cadoo-kanban-section-head-row" },
        h("span", { className: "cadoo-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "cadoo-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "cadoo-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "cadoo-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "cadoo-kanban-docs-link",
      title: "Open Cadoo Kanban docs in a new tab",
      "aria-label": "Cadoo Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `cadoo kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "cadoo-kanban-msg-ok" : "cadoo-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "cadoo-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "cadoo-kanban-boardswitcher" },
      h("div", { className: "cadoo-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "cadoo-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "cadoo-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "cadoo-kanban-dialog-title" },
        ...
        h("div", { className: "cadoo-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Cadoo profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "cadoo-kanban-bulk" },
      h("span", { className: "cadoo-kanban-bulk-count" },
      ...
      h("div", { className: "cadoo-kanban-bulk-priority",
      ...
      h("div", { className: "cadoo-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Cadoo profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "cadoo-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("cadoo-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("cadoo-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "cadoo-kanban-trash",
        dragOver ? "cadoo-kanban-trash--drop" : "",
        props.draggingTaskId ? "cadoo-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "cadoo-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "cadoo-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".cadoo-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "cadoo-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("cadoo-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("cadoo-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "cadoo-kanban-column",
        dragOver ? "cadoo-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "cadoo-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "cadoo-kanban-col-check",
        ...
        h("span", { className: cn("cadoo-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "cadoo-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "cadoo-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "cadoo-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "cadoo-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "cadoo-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "cadoo-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "cadoo-kanban-lane" },
                  h("div", { className: "cadoo-kanban-lane-head" },
                    h("span", { className: "cadoo-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "cadoo-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "cadoo-kanban-card--stale-red";
    if (age >= tier.amber) return "cadoo-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".cadoo-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "cadoo-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "cadoo-kanban-card",
        props.selected ? "cadoo-kanban-card--selected" : "",
        props.failed ? "cadoo-kanban-card--failed" : "",
        props.draggingSource ? "cadoo-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "cadoo-kanban-card-content" },
          h("div", { className: "cadoo-kanban-card-row" },
            h("label", {
              className: "cadoo-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "cadoo-kanban-card-check",
            ...
            h("span", { className: "cadoo-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or cadoo kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "cadoo-kanban-warning-badge",
                    "cadoo-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "cadoo-kanban-needs-assignee",
          ...
          h("div", { className: "cadoo-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "cadoo-kanban-card-row cadoo-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "cadoo-kanban-assignee",
                            title: `Assigned to Cadoo profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "cadoo-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "cadoo-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "cadoo-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "cadoo-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Cadoo profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Cadoo profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "cadoo-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "cadoo-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "cadoo-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "cadoo-kanban-drawer-close",
        ...
        data ? h("div", { className: "cadoo-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "cadoo-kanban-section" },
      h("div", { className: "cadoo-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "cadoo-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "cadoo-kanban-drawer-body" },
      h("div", { className: "cadoo-kanban-drawer-title" },
        h("span", { className: cn("cadoo-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "cadoo-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "cadoo-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "cadoo-kanban-section" },
        h("div", { className: "cadoo-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "cadoo-kanban-section" },
        h("div", { className: "cadoo-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "cadoo-kanban-comment" },
                h("div", { className: "cadoo-kanban-comment-head" },
                  h("span", { className: "cadoo-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "cadoo-kanban-comment-ago" },
      ...
      h("div", { className: "cadoo-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "cadoo-kanban-event",
              isDiag ? "cadoo-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "cadoo-kanban-event-header" },
                  h("span", { className: "cadoo-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "cadoo-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "cadoo-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "cadoo-kanban-event-header-plain" },
                  h("span", { className: "cadoo-kanban-event-kind" }, e.kind),
                  h("span", { className: "cadoo-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "cadoo-kanban-event-phantom-row" },
                  h("span", { className: "cadoo-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "cadoo-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "cadoo-kanban-event-payload" },
    ...
    return h("div", { className: "cadoo-kanban-section" },
      h("div", { className: "cadoo-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "cadoo-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `cadoo-kanban-run--${r.outcome || r.status || "ended"}`
          : "cadoo-kanban-run--active";
        return h("div", { key: r.id, className: cn("cadoo-kanban-run", outcomeClass) },
          h("div", { className: "cadoo-kanban-run-head" },
            h("span", { className: "cadoo-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "cadoo-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "cadoo-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "cadoo-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "cadoo-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "cadoo-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "cadoo-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "cadoo-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "cadoo-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "cadoo-kanban-pre cadoo-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "cadoo-kanban-meta-row" },
      h("span", { className: "cadoo-kanban-meta-label" }, props.label),
      h("span", { className: "cadoo-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "cadoo-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "cadoo-kanban-meta-row" },
        h("span", { className: "cadoo-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "cadoo-kanban-meta-value cadoo-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "cadoo-kanban-meta-row" },
        h("span", { className: "cadoo-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "cadoo-kanban-meta-value cadoo-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "cadoo-kanban-section" },
      h("div", { className: "cadoo-kanban-section-head-row" },
        h("span", { className: "cadoo-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "cadoo-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "cadoo-kanban-section" },
      h("div", { className: "cadoo-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "cadoo-kanban-deps-row" },
        h("span", { className: "cadoo-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "cadoo-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "cadoo-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "cadoo-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "cadoo-kanban-dep-chip-x",
      ...
      h("div", { className: "cadoo-kanban-deps-row" },
        h("span", { className: "cadoo-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "cadoo-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "cadoo-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "cadoo-kanban-section" },
      ...
      h("div", { className: "cadoo-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "cadoo-kanban-home-sub cadoo-kanban-home-sub--on"
              : "cadoo-kanban-home-sub",
          }, label);
        })
      )
    );
