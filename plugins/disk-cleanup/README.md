...
Auto-tracks and cleans up ephemeral files created during Cadoo Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CADOO_HOME` and `/tmp/cadoo-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CADOO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CADOO_HOME` or `/tmp/cadoo-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CADOO_HOME/disk-cleanup/` is itself excluded
- `$CADOO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
