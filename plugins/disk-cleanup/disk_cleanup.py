"""disk_cleanup — ephemeral file cleanup for Cadoo Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under CADOO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CADOO_HOME and /tmp/cadoo-*
Never touches: ~/.cadoo/logs/ or any system directory.
"""
...
try:
    ...
    def get_cadoo_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CADOO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".cadoo").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CADOO_HOME/logs/``."""
    return get_cadoo_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CADOO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CADOO_HOME or ``/tmp/cadoo-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    cadoo_home = get_cadoo_home()
    try:
        path.resolve().relative_to(cadoo_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/cadoo-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("cadoo-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "cadoo", "backups", "profiles", ".worktrees",
})
...
# Paths under $CADOO_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so CADOO_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        cadoo_home = get_cadoo_home()
        for parent in ("cron", "cronjobs"):
            base = cadoo_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CADOO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CADOO_HOME, but never recurse into known
    # durable state trees.  Some installs place the Cadoo checkout, venv,
    # and desktop build under CADOO_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    cadoo_home = get_cadoo_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in cadoo_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    cadoo_home = get_cadoo_home()
    try:
        rel = path.resolve().relative_to(cadoo_home)
        ...
        # Path isn't under CADOO_HOME (e.g. /tmp/cadoo-*) — fall through.
        pass
