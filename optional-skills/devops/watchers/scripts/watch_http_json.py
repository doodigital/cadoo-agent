...
Usage (via cron with --no-agent):
    ...
    cadoo cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CADOO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Cadoo-Watcher/1.0"})
