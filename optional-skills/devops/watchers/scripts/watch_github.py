...
Usage (via cron with --no-agent):
    ...
    cadoo cron create cadoo-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CADOO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name cadoo-issues --repo DooStudio/cadoo --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Cadoo .env file
(``${CADOO_HOME:-~/.cadoo}/.env``) to avoid the 60 req/hr
