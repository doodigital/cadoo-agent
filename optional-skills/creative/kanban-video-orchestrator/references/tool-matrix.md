...
Maps each role archetype to the Cadoo skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public cadoo
...
## Cadoo skills relevant to video production

### Visual / rendering skills (`cadoo/skills/creative/`)
...
### Design / pre-production skills (`cadoo/skills/creative/`)
...
### Audio / media skills (`cadoo/skills/creative/` + `skills/media/`)
...
These are **not** Cadoo skills but external CLIs / APIs that profiles invoke.
...
## Built-in Cadoo tools for media review

These are native Cadoo tools — not invoked via terminal but through their own
...
key is present in `${CADOO_HOME:-~/.cadoo}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Cadoo profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${CADOO_HOME:-~/.cadoo}/.env` → environment variable.
...
When a new Cadoo-public video skill ships:
