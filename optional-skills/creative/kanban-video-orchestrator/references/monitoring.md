...
cadoo kanban watch --tenant <project-slug>

# Snapshot of the board
cadoo kanban list --tenant <project-slug>
cadoo kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
cadoo kanban stats --tenant <project-slug>

# Visual dashboard (browser)
cadoo dashboard

# Inspect a specific task (includes comments + events)
cadoo kanban show <task-id>

# Follow a single task's event stream
cadoo kanban tail <task-id>
```

Verify available subcommands with `cadoo kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `cadoo kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `cadoo kanban block <id>` then `cadoo kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `cadoo kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `cadoo kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `cadoo profile list` (profile exists?), `cadoo status` (gateway/dispatcher up?), and verify tenant. |
...
cadoo kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
cadoo kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(cadoo kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
cadoo kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
cadoo kanban block <task-id>
cadoo kanban archive <task-id>

# Diagnose what happened
cadoo kanban show <task-id>      # task body, comments, recent events
cadoo kanban tail <task-id>      # follow the live event stream
cadoo kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `cadoo kanban create` for the director
...
while true; do
    clear
    cadoo kanban list --tenant <slug>
    echo "---"
    cadoo kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `cadoo kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `cadoo profile list` and start
