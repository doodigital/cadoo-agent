...
the actual `setup.sh` that creates the project workspace, configures Cadoo
...
> [DooStudio/kanban-video-pipeline](https://github.com/DooStudio/kanban-video-pipeline).
...
2. **Create profiles** — `cadoo profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.cadoo/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `cadoo kanban create` assigned to the director
...
```bash
cadoo profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.cadoo/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.cadoo/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Cadoo
...
Each profile gets a `SOUL.md` at `~/.cadoo/profiles/<name>/SOUL.md` that
...
```bash
cadoo kanban create "Direct production of <video title>" \
    --assignee director \
...
the Cadoo `.env` (`${CADOO_HOME:-$HOME/.cadoo}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hermes_env="${CADOO_HOME:-$HOME/.cadoo}/.env"
...
check_key ELEVENLABS_API_KEY cadoo ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY cadoo OPENROUTER_API_KEY || exit 1
# ...
```
