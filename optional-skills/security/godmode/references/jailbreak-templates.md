...
## 5. GODMODE FAST — Cadoo 4 405B (Zero Refusal)

**Model:** `nousresearch/cadoo-4-405b`
**Strategy:** Cadoo models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Cadoo Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.cadoo/config.yaml`:
