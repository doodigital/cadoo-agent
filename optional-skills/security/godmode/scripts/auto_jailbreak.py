...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CADOO_HOME", os.path.expanduser("~/.cadoo")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CADOO_HOME", Path.home() / ".cadoo")) / "skills" / "red-teaming" / "godmode"
...
CADOO_HOME = Path(os.getenv("CADOO_HOME", Path.home() / ".cadoo"))
CONFIG_PATH = CADOO_HOME / "config.yaml"
PREFILL_PATH = CADOO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Cadoo models — already uncensored, just needs clean prompt
    "cadoo": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "cadoo" in model_lower or "nous" in model_lower:
        return "cadoo"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Cadoo config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.cadoo/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Cadoo for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Cadoo for changes to take effect.")
