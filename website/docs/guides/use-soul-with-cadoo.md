---
sidebar_position: 7
title: "Use SOUL.md with Cadoo"
description: "How to use SOUL.md to shape Cadoo Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Cadoo

`SOUL.md` is the **primary identity** for your Cadoo instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Cadoo to feel like the same assistant every time you talk to it — or if you want to replace the Cadoo persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Cadoo should be
- what Cadoo should avoid stylistically
- how Cadoo should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Cadoo is and how Cadoo speaks
...
Cadoo now uses only the global SOUL file for the current instance:

```text
~/.cadoo/SOUL.md
```

If you run Cadoo with a custom home directory, it becomes:
...
Cadoo automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Cadoo does not overwrite it
- if the file exists but is empty, Cadoo adds nothing from it to the prompt

## How Cadoo uses it

When Cadoo starts a session, it reads `SOUL.md` from `CADOO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Cadoo feels.
...
Cadoo already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Cadoo should sound.

# Avoid
What Cadoo should not do.

# Defaults
How Cadoo should behave when ambiguity appears.
```
...
Then restart Cadoo or start a new session.
...
### I edited SOUL.md but Cadoo still sounds the same

Check:
- you edited `~/.cadoo/SOUL.md` or `$CADOO_HOME/SOUL.md`
...
### Cadoo is ignoring parts of my SOUL.md
