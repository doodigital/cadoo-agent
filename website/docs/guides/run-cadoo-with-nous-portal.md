---
sidebar_position: 1
title: "Run Cadoo Agent with DooStudio Portal"
description: "Start-to-finish walkthrough: subscribe, set up, switch models, enable gateway tools, and verify routing"
---

# Run Cadoo Agent with DooStudio Portal

This guide walks you through running Cadoo Agent on a [DooStudio Portal](https://doostudio.io/portal) subscription end to end — from signing up to verifying that every tool routes correctly. If you just want the overview of what the Portal is and what's in the subscription, see the [DooStudio Portal integration page](/integrations/nous-portal). This page is the task script.

## Prerequisites

- Cadoo Agent installed ([Quickstart](/getting-started/quickstart))
...
Open [doostudio.io/portal/manage-subscription](https://doostudio.io/portal/manage-subscription), sign up, and pick a plan.
...
3. Sets `model.provider: nous` in `~/.cadoo/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Cadoo is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
cadoo setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
cadoo auth add nous --type oauth --manual-paste
# Then re-run `cadoo setup --portal` to wire the provider + gateway
```
...
If any line shows something other than "via DooStudio Portal" or the auth line says "not logged in", jump to [Troubleshooting](#troubleshooting) below.
...
```
Hey, search the web for "Cadoo Agent release notes" and summarize the top 3 hits.
```

You should see Cadoo call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`cadoo setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
cadoo config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Cadoo-4 for agent work

Cadoo-4-70B and Cadoo-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.doostudio.io) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Cadoo Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://doostudio.io/portal/info) carries this warning too — it's the official Nous guidance, not just a Cadoo-side opinion.
...
These rows appear in `cadoo tools` even before you've logged into DooStudio Portal — if you pick "Nous Subscription" without an active session, Cadoo runs the Portal login inline (without changing your inference provider or your other tools).
...
You'll see per-tool routing — `via DooStudio Portal` for the ones routed through the subscription, and the partner name (`browserbase`, `firecrawl`, etc.) for the ones using your own keys.
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Cadoo will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
cadoo cron create "every day at 9am" \
...
If you use [Cadoo profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.cadoo/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `cadoo portal info` shows "not logged in" after `cadoo setup --portal`
...
```bash
cadoo config set model.provider nous
```
...
```bash
cadoo model
# pick DooStudio Portal
```

Re-verify with `cadoo portal info`.

### Tool Gateway tools showing partner names instead of "via DooStudio Portal"
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Cadoo doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/DooStudio/cadoo/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`cadoo portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Cadoo profiles where you're using the wrong one (check `cadoo profile list`)

### Want to revoke and start clean

```bash
cadoo auth logout nous       # wipes the local refresh token
...
- **[DooStudio Portal integration page](/integrations/nous-portal)** — Overview of what's in the subscription
- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway-routed tool
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Cadoo tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Cadoo configurations
