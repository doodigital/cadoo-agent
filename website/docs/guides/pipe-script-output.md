...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `cadoo send`."
---

# Pipe Script Output to Messaging Platforms

`cadoo send` is a small, scriptable CLI that pushes a message to any
messaging platform Cadoo is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | cadoo send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `cadoo
...
cadoo send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
cadoo send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
cadoo send --list

# Filter by platform
cadoo send --list telegram
```
...
Any platform Cadoo ships adapters for works as a target:
...
`cadoo send` resolves the message body in this order:

1. **Positional argument** — `cadoo send --to telegram "hi"`
2. **`--file PATH`** — `cadoo send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | cadoo send --to telegram`

When stdin is a TTY (no pipe), Cadoo does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  cadoo send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `cadoo send` reuses your Cadoo config, the same script works on
any host where Cadoo is installed — no need to export bot tokens into
...
`cadoo send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  cadoo send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | cadoo send \
...
```bash
./train.py --epochs 200 && \
  cadoo send --to telegram "training done" || \
  cadoo send --to telegram "training failed (exit $?)"
```
...
cadoo send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(cadoo send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `cadoo send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `cadoo send` calls
the platform's REST endpoint directly using credentials from
`~/.cadoo/.env` and `~/.cadoo/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `cadoo gateway start` and retry.
...
# Machine-readable
cadoo send --list --json
```

The listing is built from `~/.cadoo/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`cadoo gateway
...
| Approach | Multi-platform | Reuses Cadoo creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `cadoo send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`cadoo send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `cadoo send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `cadoo send` shares with cron delivery.
