...
This tutorial walks you through setting up a Telegram bot powered by Cadoo Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Cadoo Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.cadoo/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Cadoo itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Cadoo Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Cadoo Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Cadoo Gateway...
...
# View live logs
journalctl --user -u cadoo-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `cadoo gateway install` to update the plist.
:::
...
```bash
cadoo gateway stop && cadoo gateway start
```
...
cadoo pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.cadoo/.env`:
...
Control how much detail the bot shows when using tools. In `~/.cadoo/config.yaml`:
...
Customize how the bot communicates by editing `~/.cadoo/SOUL.md`:

For a full guide, see [Use SOUL.md with Cadoo](/guides/use-soul-with-cadoo).
...
```bash
# From the CLI
cadoo cron list          # View all scheduled jobs
cadoo cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u cadoo-gateway` (Linux) or `~/.cadoo/logs/gateway.log` (macOS) |
| Cron job output | `~/.cadoo/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.cadoo/cron/jobs.json` |
| Pairing data | `~/.cadoo/pairing/` |
| Session history | `~/.cadoo/sessions/` |
