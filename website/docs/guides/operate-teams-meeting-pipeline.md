...
```bash
cadoo teams-pipeline token-health
cadoo teams-pipeline token-health --force-refresh
```
...
```bash
cadoo teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
cadoo teams-pipeline maintain-subscriptions
cadoo teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Cadoo cron (recommended if you already run the Cadoo gateway)

Cadoo ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.cadoo/scripts/`. First create the script:

```bash
mkdir -p ~/.cadoo/scripts
cat > ~/.cadoo/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec cadoo teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.cadoo/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
cadoo cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/cadoo-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Cadoo Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=cadoo
EnvironmentFile=/etc/cadoo/env
ExecStart=/usr/local/bin/cadoo teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/cadoo-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Cadoo Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now cadoo-teams-pipeline-maintain.timer
systemctl list-timers cadoo-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/cadoo teams-pipeline maintain-subscriptions >> /var/log/cadoo/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.cadoo/.env` at the top of a wrapper script that crontab calls.
...
```bash
cadoo teams-pipeline subscriptions   # should show expirationDateTime advanced
cadoo teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
cadoo teams-pipeline list
cadoo teams-pipeline list --status failed
cadoo teams-pipeline show <job-id>
```

### Replay a stored job

```bash
cadoo teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
cadoo teams-pipeline fetch --meeting-id <meeting-id>
cadoo teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `cadoo teams-pipeline maintain-subscriptions --dry-run`
- inspect `cadoo teams-pipeline list --status failed`
...
- run `cadoo teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `cadoo teams-pipeline run`
...
- [ ] `cadoo teams-pipeline validate` returns an OK snapshot
- [ ] `cadoo teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Cadoo cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
