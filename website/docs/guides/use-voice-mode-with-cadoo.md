---
sidebar_position: 8
title: "Use Voice Mode with Cadoo"
description: "A practical guide to setting up and using Cadoo voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Cadoo
...
:::tip
[DooStudio Portal](/integrations/nous-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Cadoo sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Cadoo.
...
## Step 1: make sure normal Cadoo works first
...
Cadoo supports both local and cloud speech stacks.
...
### If you use `cadoo setup`

If you choose NeuTTS in the setup wizard, Cadoo checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Cadoo transcribes and responds
...
- asking Cadoo to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Cadoo loop.

## Tuning CLI behavior

### Silence threshold

If Cadoo starts/stops too aggressively, tune:
...
Cadoo stays a normal chat bot, but can speak replies.
...
- you want Cadoo to function like a portable research or ops assistant
...
Cadoo joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Cadoo detects speech boundaries
- transcripts are posted in the associated text channel
- Cadoo responds in text and audio
...
1. get text Cadoo working
2. install `cadoo[voice]`
