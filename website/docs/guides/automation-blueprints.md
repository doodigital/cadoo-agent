...
Copy-paste blueprints for common automation patterns. Each blueprint uses Cadoo's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`cadoo webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `cadoo webhook subscribe`) |
...
```bash
cadoo cron create "0 2 * * *" \
  "You are a project manager triaging the DooStudio/cadoo GitHub repo.

1. Run: gh issue list --repo DooStudio/cadoo --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
cadoo webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
cadoo cron create "0 9 * * 1" \
  "Scan the DooStudio/cadoo repo for documentation drift.

1. Run: gh pr list --repo DooStudio/cadoo --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (cadoo_cli/commands.py, cadoo_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (cadoo_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
cadoo cron create "0 6 * * *" \
  "Run a dependency security audit on the cadoo project.

1. cd ~/.cadoo/cadoo && source .venv/bin/activate
...
```bash
cadoo webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
cadoo webhook subscribe alert-triage \
...
```python title="~/.cadoo/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Cadoo-Monitor/1.0"})
...
cadoo cron create "every 30m" \
  ...
  --script ~/.cadoo/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
cadoo cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Cadoo Agent development." \
...
```bash
cadoo webhook subscribe github-issues \
  --events "issues" \
...
```bash
cadoo webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
cadoo webhook subscribe stripe-payments \
...
```bash
cadoo cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the cadoo codebase.
