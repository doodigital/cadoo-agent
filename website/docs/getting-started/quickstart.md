...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Cadoo Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Cadoo setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Cadoo Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Cadoo Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Cadoo for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Cadoo working on my machine | `cadoo setup` | Run a real chat and verify it responds |
| I already know my provider | `cadoo model` | Save the config, then start chatting |
| I want a bot or always-on setup | `cadoo gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `cadoo model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `cadoo model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Cadoo cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Cadoo Agent
### With the Cadoo Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Cadoo Desktop installer](https://doostudio.io/cadoo/) from our website and run it.

### Without Cadoo Desktop:
For a command-line only install without Cadoo Desktop, run:
...
The single most important setup step. Use `cadoo model` to walk through the choice interactively:

```bash
cadoo model
```

:::tip Easiest path: DooStudio Portal
...
:::info Setup modes
On a fresh install, `cadoo setup` offers three modes:

- **Quick Setup (DooStudio Portal)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `cadoo update`. Re-enable anything later with `cadoo tools`, seed skills with `cadoo skills opt-in --sync`, or tune settings with `cadoo setup agent`.
:::
...
|----------|-----------|---------------|
| **DooStudio Portal** | Subscription-based, zero-config | OAuth login via `cadoo model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `cadoo model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `cadoo model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `cadoo_cli/models.py` may change between releases) | `cadoo model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `cadoo model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `cadoo model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `cadoo model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `cadoo model` (requires `copilot` CLI + `copilot login`) |
...
Cadoo Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `cadoo model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Cadoo separates secrets from normal config:

- **Secrets and tokens** → `~/.cadoo/.env`
- **Non-secret settings** → `~/.cadoo/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
cadoo config set model anthropic/claude-opus-4.6
cadoo config set terminal.backend docker
cadoo config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
cadoo            # classic CLI
cadoo --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Cadoo ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `cadoo` vs `cadoo --tui`.
:::
...
```bash
cadoo --continue    # Resume the most recent session
cadoo -c            # Short form
```
...
```bash
cadoo gateway setup    # Interactive platform configuration
```
...
- `cadoo tools` — tune tool access per platform
- `cadoo skills` — browse and install reusable workflows
...
```bash
cadoo config set terminal.backend docker    # Docker isolation
cadoo config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Cadoo install directory (the curl installer placed it at
# ~/.cadoo/cadoo on Linux/macOS or %LOCALAPPDATA%\cadoo\cadoo on Windows):
cd ~/.cadoo/cadoo
...
Skills are on-demand instruction documents that teach Cadoo how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Cadoo ships with a catalog of bundled skills already installed in `~/.cadoo/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
cadoo skills browse                      # list everything available
cadoo skills search kubernetes           # find skills by keyword
cadoo skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `cadoo skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Cadoo ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.cadoo/cadoo && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Cadoo opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `cadoo model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `cadoo gateway setup` and check `cadoo gateway status` |
| `cadoo --continue` can't find old session | Switched profiles or session never saved | Check `cadoo sessions list` and confirm you're in the right profile |
...
| `cadoo doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `cadoo model`
3. `cadoo setup`
4. `cadoo sessions list`
5. `cadoo --continue`
6. `cadoo gateway status`
...
| `cadoo tools` | Configure which tools are enabled per platform |
| `cadoo setup` | Full setup wizard (configures everything at once) |
| `cadoo doctor` | Diagnose issues |
| `cadoo update` | Update to latest version |
| `cadoo gateway` | Start the messaging gateway |
| `cadoo --continue` | Resume last session |
