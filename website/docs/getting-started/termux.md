...
description: "Run Cadoo Agent directly on an Android phone with Termux"
---

# Cadoo on Android with Termux

This is the tested path for running Cadoo Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Cadoo from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Cadoo now ships a Termux-aware installer path:
...
- links `cadoo` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `cadoo` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/cadoo" "$PREFIX/bin/cadoo"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `cadoo` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.cadoo/.env`.
...
### `cadoo doctor` says ripgrep or Node is missing
