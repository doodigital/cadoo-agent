...
description: "How Cadoo stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `cadoo_cli/cron.py` | CLI `cadoo cron` subcommands |
...
Jobs are stored in `~/.cadoo/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `cadoo cron` commands are run or during active CLI sessions.
...
```python
# ~/.cadoo/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.cadoo/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `cadoo cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `cadoo cron` CLI provides direct job management:

```bash
cadoo cron list                    # Show all jobs
cadoo cron create                  # Interactive job creation (alias: add)
cadoo cron edit <job_id>           # Edit job configuration
cadoo cron pause <job_id>          # Pause a running job
cadoo cron resume <job_id>         # Resume a paused job
cadoo cron run <job_id>            # Trigger immediate execution
cadoo cron remove <job_id>         # Delete a job
```
