...
sidebar_position: 1
title: "Architecture"
description: "Cadoo Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Cadoo Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ cadoo_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # CadooCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── cadoo_state.py           # SQLite session/state database with FTS5
├── cadoo_constants.py       # CADOO_HOME, profile-aware paths
...
│
├── cadoo_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `cadoo` subcommands (large file)
...
│   ├── skills_config.py      # cadoo skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # cadoo tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # cadoo gateway start/stop
│
...
```text
User input → CadooCLI.process_input()
...
Three discovery sources: `~/.cadoo/plugins/` (user), `.cadoo/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `cadoo plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-cadoo-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Cadoo as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`cadoo -p <name>`) gets its own CADOO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
