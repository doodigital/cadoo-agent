...
The ACP adapter wraps Cadoo' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
cadoo acp / cadoo-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'cadoo[acp]==<version>' cadoo-acp`, pointed at the `cadoo` PyPI release.
...
- `allow_once` -> Cadoo `once`
- `allow_always` -> Cadoo `always`
- reject options -> Cadoo `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Cadoo tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["cadoo-acp"])
...
Instead it reuses Cadoo' runtime resolver:

- `acp_adapter/auth.py`
- `cadoo_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Cadoo provider/credentials. It also always advertises a terminal setup auth method (`cadoo-setup`, args `--setup`) so first-run registry clients can open Cadoo' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.cadoo/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `cadoo-acp` toolset definition
- `cadoo_cli/main.py` — `cadoo acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `cadoo-acp` script
