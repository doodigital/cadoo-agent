...
This page covers all commands related to [Cadoo profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `cadoo profile` without a subcommand shows help.
...
```bash
cadoo profile use <name>
```

Sets `<name>` as the active profile. All subsequent `cadoo` commands (without `-p`) will use this profile.
...
cadoo profile use default
```

## `cadoo profile create`

```bash
cadoo profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `cadoo profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `cadoo update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.cadoo`), use `cadoo skills opt-out` / `cadoo skills opt-in`. |
...
cadoo profile create work2 --clone-from work

# Clone everything from a specific profile
cadoo profile create work2-backup --clone-from work --clone-all
```

## `cadoo profile describe`

```bash
cadoo profile describe [<name>] [options]
```
...
cadoo profile describe researcher

# Set it explicitly
cadoo profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
cadoo profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
cadoo profile describe --all --auto
```

## `cadoo profile delete`

```bash
cadoo profile delete <name> [options]
```
...
```bash
cadoo profile delete mybot
cadoo profile delete mybot --yes
```
...
```bash
cadoo profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Cadoo home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ cadoo profile show work
Profile: work
Path:    ~/.cadoo/profiles/work
...
```bash
cadoo profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Cadoo installation.
...
```bash
cadoo profile alias work
# Creates/updates ~/.local/bin/work

cadoo profile alias work --name mywork
# Creates ~/.local/bin/mywork

cadoo profile alias work --remove
# Removes the wrapper script
```

## `cadoo profile rename`

```bash
cadoo profile rename <old-name> <new-name>
```
...
```bash
cadoo profile rename mybot assistant
# ~/.cadoo/profiles/mybot → ~/.cadoo/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `cadoo profile export`

```bash
cadoo profile export <name> [options]
```
...
```bash
cadoo profile export work
# Creates work.tar.gz in the current directory

cadoo profile export work -o ./work-2026-03-29.tar.gz
```

## `cadoo profile import`

```bash
cadoo profile import <archive> [options]
```
...
```bash
cadoo profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

cadoo profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`cadoo profile export` / `import` are still the right commands for
...
### `cadoo profile install`

```bash
cadoo profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `cadoo -p telemetry`). |
...
cadoo profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
cadoo profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
cadoo profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
cadoo profile install ./telemetry/
```

### `cadoo profile update`

```bash
cadoo profile update <name> [--force-config] [--yes]
```
...
```bash
cadoo profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Cadoo version, author, env var requirements, the source URL/path, and
...
`cadoo profile list` also shows the distribution name and version in a
`Distribution` column, and `cadoo profile show <name>` / `delete <name>`
...
cadoo profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
cadoo profile install https://github.com/your-org/internal-assistant.git
```
...
Cadoo version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Cadoo can clone from.
3. Tell recipients to run `cadoo profile install <your-repo-url>`.
...
## `cadoo -p` / `cadoo --profile`

```bash
cadoo -p <name> <command> [options]
cadoo --profile <name> <command> [options]
```

Global flag to run any Cadoo command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
cadoo -p work chat -q "Check the server status"
cadoo --profile dev gateway start
cadoo -p personal skills list
cadoo -p work config edit
```

## `cadoo completion`

```bash
cadoo completion <shell>
```
...
- `cadoo profile <TAB>` — subcommands (list, use, create, etc.)
- `cadoo profile use <TAB>` — profile names
- `cadoo -p <TAB>` — profile names
