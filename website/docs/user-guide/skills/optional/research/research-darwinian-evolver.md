...
| | |
|---|---|
| Source | Optional — install with `cadoo skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Cadoo Agent |
| License | MIT |
...
import upstream classes into Cadoo itself.
...
```bash
mkdir -p ~/.cadoo/cache/darwinian-evolver && cd ~/.cadoo/cache/darwinian-evolver
...
```bash
cd ~/.cadoo/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.cadoo/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.cadoo/skills/research/darwinian-evolver
DE_DIR=~/.cadoo/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Cadoo core.
   Custom driver scripts under `~/.cadoo/skills/...` are user-side and fine.
