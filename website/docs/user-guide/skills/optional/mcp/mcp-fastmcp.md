...
| | |
|---|---|
| Source | Optional — install with `cadoo skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Cadoo or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Cadoo. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.cadoo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.cadoo/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Cadoo integration, either:

- configure the server in `~/.cadoo/config.yaml` using the `native-mcp` skill, or
...
### Cadoo cannot see the deployed server

The server-building part may be correct while the Cadoo config is not. Load the `native-mcp` skill and configure the server in `~/.cadoo/config.yaml`, then restart Cadoo.
