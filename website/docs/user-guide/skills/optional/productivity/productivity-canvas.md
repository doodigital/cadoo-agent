...
| | |
|---|---|
| Source | Optional — install with `cadoo skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Cadoo Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${CADOO_HOME:-~/.cadoo}/.env`:
...
```bash
CANVAS="python $CADOO_HOME/skills/productivity/canvas/scripts/canvas_api.py"
