...
| | |
|---|---|
| Source | Optional — install with `cadoo skills install official/productivity/siyuan` |
...
3. Store it in `${CADOO_HOME:-~/.cadoo}/.env`:
   ```
...
```yaml
# In ~/.cadoo/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
