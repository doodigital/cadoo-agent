...
| | |
|---|---|
| Source | Optional — install with `cadoo skills install official/productivity/shopify` |
...
4. Save to `${CADOO_HOME:-~/.cadoo}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Cadoo","productType":"Apparel","tags":["test"]}}'
```
