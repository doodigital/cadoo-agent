...
| | |
|---|---|
| Source | Optional — install with `cadoo skills install official/productivity/here-now` |
...
bash "$PUBLISH" {file-or-dir} --client cadoo
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client cadoo
```
...
| `--client {name}`      | Agent name for attribution (e.g. `cadoo`)    |
