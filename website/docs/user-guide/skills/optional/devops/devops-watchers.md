...
| | |
|---|---|
| Source | Optional — install with `cadoo skills install official/devops/watchers` |
...
All three live in `$CADOO_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$CADOO_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $CADOO_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `${CADOO_HOME:-~/.cadoo}/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $CADOO_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name cadoo-issues --repo DooStudio/cadoo --scope issues
```

Poll an arbitrary JSON API:

```bash
python $CADOO_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$CADOO_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $CADOO_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $CADOO_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$CADOO_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
