...
description: "Configure and use Honcho memory with Cadoo -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Cadoo -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `cadoo skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Cadoo

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Cadoo profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/cadoo#running-honcho-locally-with-cadoo

### Verify

```bash
cadoo honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Cadoo profile's AI peer
...
Honcho models conversations as interactions between **peers**. Cadoo creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Cadoo instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Cadoo run |
| `global` | Single session across all directories |

Manual override: `cadoo honcho map my-project-name`
...
Each Cadoo profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block cadoo.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `cadoo.coder` host block in `honcho.json`
...
```bash
cadoo honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Cadoo when Honcho memory is active.
...
Config file: `$CADOO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `cadoo honcho setup`. Ensure `memory.provider: honcho` is in `~/.cadoo/config.yaml`.

### Memory not persisting across sessions
Check `cadoo honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `cadoo profile create <name> --clone`. For existing profiles: `cadoo honcho sync`.
...
| Command | Description |
|---------|-------------|
| `cadoo honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `cadoo honcho status` | Show resolved config, connection test, peer info for active profile |
| `cadoo honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `cadoo honcho disable` | Disable Honcho for the active profile |
| `cadoo honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `cadoo honcho peers` | Show peer identities across all profiles |
| `cadoo honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `cadoo honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `cadoo honcho sessions` | List known directory-to-session-name mappings |
| `cadoo honcho map <name>` | Map current working directory to a Honcho session name |
| `cadoo honcho identity` | Seed AI peer identity or show both peer representations |
| `cadoo honcho sync` | Create host blocks for all Cadoo profiles that don't have one yet |
| `cadoo honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Cadoo + Honcho |
| `cadoo memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `cadoo memory status` | Show active memory provider and config |
| `cadoo memory off` | Disable external memory provider |
