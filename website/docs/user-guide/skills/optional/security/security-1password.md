...
| | |
|---|---|
| Source | Optional — install with `cadoo skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Cadoo Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Cadoo terminal calls (desktop app flow only)
...
### Service Account (recommended for Cadoo)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${CADOO_HOME:-~/.cadoo}/.env` (the skill will prompt for this on first load).
...
## Cadoo Execution Pattern (desktop app flow)

Cadoo terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/cadoo-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/cadoo-op.sock"
