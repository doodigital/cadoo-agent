...
title: "Git Worktrees"
description: "Run multiple Cadoo agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Cadoo Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Cadoo so each session has a clean, isolated working directory.

## Why Use Worktrees with Cadoo?

Cadoo treats the **current working directory** as the project root:

- CLI: the directory where you run `cadoo` or `cadoo chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.cadoo/config.yaml`
...
git worktree add ../repo-feature feature/cadoo-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/cadoo-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Cadoo there:

```bash
cd ../repo-feature

# Start Cadoo in the worktree
cadoo
```
...
git worktree add ../repo-experiment-a feature/cadoo-a
git worktree add ../repo-experiment-b feature/cadoo-b
```
...
- Works on its own branch (`feature/cadoo-a` vs `feature/cadoo-b`).
...
- Cadoo checkpoint data under `~/.cadoo/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Cadoo experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/cadoo-checkpoints-docs`, `feature/cadoo-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Cadoo from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `cadoo -w` (Automatic Worktree Mode)

Cadoo has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `cadoo/cadoo-<hash>`).
...
```bash
cadoo -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `cadoo -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Cadoo session its own clean checkout.
