...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Cadoo's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Cadoo instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
cadoo memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `cadoo` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Cadoo syncs them back at session init.
...
The `cadoo honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `cadoo memory setup honcho` (or run `cadoo memory setup` and pick it from the list); the `cadoo honcho` subcommand then appears on the next invocation.

```bash
cadoo memory setup honcho    # Configure Honcho directly (works before activation)
cadoo honcho status          # Connection status, config, and key settings
cadoo honcho setup           # Redirects to `cadoo memory setup` (post-activation alias)
cadoo honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
cadoo honcho peer            # Show or update peer names + dialectic reasoning level
cadoo honcho mode            # Show or set recall mode (hybrid/context/tools)
cadoo honcho tokens          # Show or set token budget for context and dialectic
cadoo honcho identity        # Seed or show the AI peer's Honcho identity
cadoo honcho sync            # Sync Honcho config to all existing profiles
cadoo honcho peers           # Show peer identities across all profiles
cadoo honcho sessions        # List known Honcho session mappings
cadoo honcho map             # Map current directory to a Honcho session name
cadoo honcho enable          # Enable Honcho for the active profile
cadoo honcho disable         # Disable Honcho for the active profile
cadoo honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `cadoo honcho`

If you previously used the standalone `cadoo honcho setup`:
...
No re-login or re-setup needed. Run `cadoo memory setup` and select "honcho" — the wizard detects your existing config.
