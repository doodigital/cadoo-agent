...
Cadoo can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<CADOO_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Cadoo renders the sprite at full fidelity when your
...
cadoo pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `cadoo pets list [query] [--limit N]` |
| List installed pets | `cadoo pets list --installed` |
| Install a pet | `cadoo pets install <slug> [--select] [--force]` |
...
| Preview/animate | `cadoo pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `cadoo pets off` |
| Remove an installed pet | `cadoo pets remove <slug>` |
| Diagnose setup | `cadoo pets doctor` |

`cadoo pets show` flags:
...
always-on-top desktop window. Out there it stays visible while Cadoo is
...
Run `cadoo pets doctor` — it reports:
