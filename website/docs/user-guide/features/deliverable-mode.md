...
When Cadoo Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.cadoo/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.cadoo/config.yaml` (switchable per session
via `/personality`).
...
If you use Cadoo' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.cadoo/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Cadoo Agent's deliverable
