...
description: "Use your DooStudio Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Cadoo-managed provider subscription as their
...
| Use case | "Use Cadoo as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the DooStudio Portal OAuth flow. Cadoo stores
the refresh token in `~/.cadoo/auth.json` — the same place all Cadoo
...
Model:      Cadoo-4-70B    # or Cadoo-4.3-36B, Cadoo-4-405B
```
...
Currently shipped: `nous` (DooStudio Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `cadoo_cli/proxy/adapters/`.
...
If you see `not logged in`, run `cadoo portal`. If you see
...
supports; check `doostudio.io/portal/models`.
...
[doostudio.io/portal](https://doostudio.io/portal).
...
`cadoo_cli/proxy/adapters/<provider>.py` and registering it in
