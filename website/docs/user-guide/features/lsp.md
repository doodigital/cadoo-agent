...
Cadoo runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Cadoo
...
1. Cadoo captures a baseline of current diagnostics for the file.
...
…). Cadoo auto-detects the binary on PATH or in
`<CADOO_HOME>/lsp/bin/`.
...
`node_modules` tree — Cadoo installs both packages together when you
run `cadoo lsp install typescript` or auto-install fires on first
use.

## CLI

```
cadoo lsp status          # service state + per-server install status
cadoo lsp list            # registry, optionally --installed-only
cadoo lsp install <id>    # eagerly install one server
cadoo lsp install-all     # try every server with a known recipe
cadoo lsp restart         # tear down running clients
cadoo lsp which <id>      # print resolved binary path
```

`cadoo lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CADOO_HOME>/lsp/bin
...
When `install_strategy: auto`, Cadoo installs binaries into
`<CADOO_HOME>/lsp/bin/`. NPM packages land in
`<CADOO_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Cadoo-owned and is
...
Servers are kept alive for the life of the Cadoo process. There's
...
**`cadoo lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CADOO_HOME>/lsp/bin/`. Run
`cadoo lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `cadoo lsp status`**
...
When `cadoo lsp status` shows a `Backend warnings` section, install
...
`~/.cadoo/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.cadoo/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `cadoo lsp restart` to clear the set;
