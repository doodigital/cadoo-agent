...
**Tool Search** is Cadoo' opt-in progressive-disclosure layer for that
...
:::info Built-in Cadoo tools never defer
The tools that make up Cadoo' core capability set (`terminal`,
...
When the model invokes `tool_call`, Cadoo **unwraps the bridge** and
...
- **No JS sandbox.** Cadoo uses the simpler "structured tools" mode
