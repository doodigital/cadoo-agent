...
description: "Customize Cadoo Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Cadoo Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CADOO_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Cadoo is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Cadoo now seeds a default `SOUL.md` automatically in:

```text
~/.cadoo/SOUL.md
```

More precisely, it uses the current instance's `CADOO_HOME`, so if you run Cadoo with a custom home directory, it will use:
...
- Cadoo creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Cadoo loads `SOUL.md` only from `CADOO_HOME`
- Cadoo does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Cadoo falls back to a built-in default identity
...
If Cadoo loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CADOO_HOME`, the personality belongs to the Cadoo instance itself.

That also makes it easier to teach users:
- "Edit `~/.cadoo/SOUL.md` to change Cadoo' default personality."
...
- how Cadoo should handle uncertainty, disagreement, or ambiguity
...
## What Cadoo injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Cadoo falls back to a built-in default identity ("You are Cadoo Agent, an intelligent AI assistant created by DooStudio..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Cadoo ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Cadoo, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Cadoo its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.cadoo/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.cadoo/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-cadoo)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Cadoo speaks
- `display.skin` and `/skin` affect how Cadoo looks in the terminal
