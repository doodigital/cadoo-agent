...
Cadoo Agent can drive your desktop — clicking, typing, scrolling,
...
```
cadoo computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `cadoo computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `cadoo tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Cadoo app). `cadoo computer-use doctor` will tell you which permission is missing. |
...
```
cadoo -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.cadoo/config.yaml`.

## `cadoo computer-use doctor` — your first triage stop

`cadoo computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ cadoo computer-use doctor
...
agent is acting here." Each Cadoo run declares its own cua-driver
**session id** (something like `cadoo-3a7b9c14d2e8`); the cursor's
...
Cadoo intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Cadoo-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Cadoo skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Cadoo
...
Cadoo applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.cadoo/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Cadoo applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Cadoo agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Cadoo agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Cadoo warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Cadoo from inside the RDP / console
...
upstream. **Cadoo disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Cadoo sets
...
When it's on, `cadoo computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Cadoo at a binary you built from source instead
of the published release. Cadoo resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Cadoo testing (see notes).
...
### Option B — point Cadoo straight at the built binary (fastest loop)
...
### Confirm Cadoo is using your build

- `cadoo computer-use status` prints the resolved binary path and
  version.
- `cadoo computer-use doctor` confirms the binary is reachable and
...
- **Cadoo spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Cadoo
  change needed either way.
- **Local builds skip the version check.** Cadoo warns when the
...
**First action when anything's off: run `cadoo computer-use doctor`.**
...
Run `cadoo computer-use install` to fetch the cua-driver binary, or
run `cadoo tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `cadoo computer-use doctor` will
...
- **Cadoo-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Cadoo `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Cadoo (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Cadoo among them)
