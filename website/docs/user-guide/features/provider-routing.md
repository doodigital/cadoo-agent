...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Cadoo Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::tip
Traffic routed through [DooStudio Portal](/integrations/nous-portal) still respects per-model routing and priority configs — and Portal subscribers get 10% off token-billed providers.
:::

## Configuration

Add a `provider_routing` section to your `~/.cadoo/config.yaml`:
...
- **CLI mode** — configured in `~/.cadoo/config.yaml`, loaded at startup
