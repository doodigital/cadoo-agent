...
Connect Cadoo to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Cadoo supports two WeCom integration modes:
...
> Run `cadoo gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Cadoo decrypts the message, queues it for the agent
...
(Use `cadoo gateway start` only after `cadoo gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Cadoo and the token the
...
in `~/.cadoo/.env` values around `=` will also break signature checks. After
fixing, restart `cadoo gateway run`.
...
Check `cadoo gateway run` logs for the bound host/port. If the adapter bound to
