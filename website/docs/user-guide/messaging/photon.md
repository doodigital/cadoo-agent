...
Connect Cadoo to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Cadoo — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Cadoo runs it in a
...
cadoo photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Cadoo Agent` project on your account.
...
Runtime credentials are written to `~/.cadoo/.env`
...
(device token, dashboard project id) lives in `~/.cadoo/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Cadoo
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Cadoo replies with a pairing code. Approve it with:

```bash
cadoo pairing approve photon <CODE>
```

Use `cadoo pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.cadoo/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.cadoo/.env`):
...
By default Cadoo responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Cadoo` and
`@Cadoo agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Cadoo will reply.
...
assigned iMessage line Cadoo uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run cadoo photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `cadoo photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `cadoo photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `cadoo photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Cadoo sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Cadoo wake words  | JSON list / comma / newline regex patterns for group mentions |
