...
description: "Chat with Cadoo from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Cadoo from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Cadoo](/guides/use-voice-mode-with-cadoo).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [DooStudio Portal](/integrations/nous-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Cadoo Gateway"]
...
For group chats, hooks, and automation flows, Cadoo supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Cadoo keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Cadoo does not hide failures just because the text resembles a silence token.
...
```bash
cadoo gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
cadoo gateway              # Run in foreground
cadoo gateway setup        # Configure messaging platforms interactively
cadoo gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo cadoo gateway install --system   # Linux only: install a boot-time system service
cadoo gateway start        # Start the default service
cadoo gateway stop         # Stop the default service
cadoo gateway status       # Check default service status
cadoo gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Cadoo Agent to the latest version |
...
Configure per-platform overrides in `~/.cadoo/gateway.json`:
...
cadoo pairing approve telegram XKGH5N7P

# Other pairing commands:
cadoo pairing list          # View pending + approved users
cadoo pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Cadoo appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.cadoo/config.yaml`:
...
Off by default. When enabled, Cadoo prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.cadoo/config.yaml`:
...
```bash
cadoo gateway install               # Install as user service
cadoo gateway start                 # Start the service
cadoo gateway stop                  # Stop the service
cadoo gateway status                # Check status
journalctl --user -u cadoo-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo cadoo gateway install --system
sudo cadoo gateway start --system
sudo cadoo gateway status --system
journalctl -u cadoo-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `cadoo update`. When `cadoo update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart cadoo-gateway` command (it never blocks on an interactive password prompt).
...
```bash
cadoo gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `cadoo update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo cadoo update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/cadoo-gateway`:

```
cadoo ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed cadoo-gateway*, /usr/bin/systemctl --no-ask-password start cadoo-gateway*, /usr/bin/systemctl --no-ask-password restart cadoo-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Cadoo will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Cadoo installations on the same machine (with different `CADOO_HOME` directories), each gets its own systemd service name. The default `~/.cadoo` uses `cadoo-gateway`; other installations use `cadoo-gateway-<hash>`. The `cadoo gateway` commands automatically target the correct service for your current `CADOO_HOME`.
:::

### macOS (launchd)

```bash
cadoo gateway install               # Install as launchd agent
cadoo gateway start                 # Start the service
cadoo gateway stop                  # Stop the service
cadoo gateway status                # Check status
tail -f ~/.cadoo/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.cadoo.gateway.plist`. It includes three environment variables:
...
- **CADOO_HOME** — scopes the gateway to your Cadoo installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `cadoo gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `CADOO_HOME` directory gets its own launchd label. The default `~/.cadoo` uses `ai.cadoo.gateway`; other installations use `ai.cadoo.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `cadoo-cli` | Full access |
| Telegram | `cadoo-telegram` | Full tools including terminal |
| Discord | `cadoo-discord` | Full tools including terminal |
| WhatsApp | `cadoo-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `cadoo-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `cadoo-slack` | Full tools including terminal |
| Google Chat | `cadoo-google_chat` | Full tools including terminal |
| Signal | `cadoo-signal` | Full tools including terminal |
| SMS | `cadoo-sms` | Full tools including terminal |
| Email | `cadoo-email` | Full tools including terminal |
| Home Assistant | `cadoo-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `cadoo-mattermost` | Full tools including terminal |
| Matrix | `cadoo-matrix` | Full tools including terminal |
| DingTalk | `cadoo-dingtalk` | Full tools including terminal |
| Feishu/Lark | `cadoo-feishu` | Full tools including terminal |
| WeCom | `cadoo-wecom` | Full tools including terminal |
| WeCom Callback | `cadoo-wecom-callback` | Full tools including terminal |
| Weixin | `cadoo-weixin` | Full tools including terminal |
| BlueBubbles | `cadoo-bluebubbles` | Full tools including terminal |
| QQBot | `cadoo-qqbot` | Full tools including terminal |
| Yuanbao | `cadoo-yuanbao` | Full tools including terminal |
| Microsoft Teams | `cadoo-teams` | Full tools including terminal |
| API Server | `cadoo-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `cadoo-webhook` | Full tools including terminal |
| Raft | `cadoo-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.cadoo/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
