...
`~/.cadoo/config.yaml`, `~/.cadoo/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/cadoo`:

```text
/etc/cadoo/
├── config.yaml     # managed config layer (wins over ~/.cadoo/config.yaml)
└── .env            # managed env layer (wins over ~/.cadoo/.env + shell)
```
...
knob — like `CADOO_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Cadoo.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export HERMES_MANAGED_DIR=/opt/org/cadoo-policy
```
...
left user-settable. `cadoo doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/cadoo/config.yaml` (managed) | `/etc/cadoo/.env` (managed) |
| 2 | `~/.cadoo/config.yaml` (user) | `~/.cadoo/.env` (user) |
...
```bash
cadoo config        # shows a header naming the managed source + the pinned keys
cadoo doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Cadoo refuses and names the source:

```bash
$ cadoo config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/cadoo/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `cadoo config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/cadoo

# Pin some config values for every user on this machine
sudo tee /etc/cadoo/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/cadoo/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/cadoo
sudo chmod 0644 /etc/cadoo/config.yaml /etc/cadoo/.env
```

Changes take effect on the next Cadoo start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`cadoo doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Cadoo as `root`), managed scope is advisory.
