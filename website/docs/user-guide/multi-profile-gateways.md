...
If you only run one Cadoo agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Cadoo agents that should all
...
(`ai.cadoo.gateway-<name>.plist`) or systemd user service
(`cadoo-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
cadoo profile create coder
cadoo profile create personal-bot
cadoo profile create research
...
```bash
cadoo config set gateway.multiplex_profiles true
cadoo gateway restart
```

Equivalently, in the default profile's `~/.cadoo/config.yaml`:
...
You do **not** run `cadoo gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `cadoo gateway start` / `run` is a
...
home). `cadoo status` reports the multiplexer and the profiles it serves;
`cadoo status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/cadoo-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: cadoo-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    cadoo gateway "$action"
  else
    cadoo -p "$profile" gateway "$action"
  fi
}
...
```bash
cadoo-gateways start      # start every configured profile
cadoo-gateways stop       # stop every configured profile
cadoo-gateways restart    # restart all
cadoo-gateways status     # status across all
cadoo-gateways list       # delegates to `cadoo gateway list`
```

:::tip
The `default` profile is targeted with `cadoo gateway <action>` (no `-p`),
not `cadoo -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `cadoo -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.cadoo.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/cadoo-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.cadoo.gateway.plist` /
`cadoo-gateway.service`.
...
```bash
# Default profile
tail -f ~/.cadoo/logs/gateway.log
tail -f ~/.cadoo/logs/gateway.error.log

# Named profile
tail -f ~/.cadoo/profiles/<name>/logs/gateway.log
tail -f ~/.cadoo/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.cadoo/logs/gateway.log ~/.cadoo/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
cadoo logs -f                  # follow default profile
cadoo -p coder logs -f         # follow one profile
cadoo logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
cadoo profile list             # profiles + model + gateway state
cadoo-gateways status          # full status across every profile
launchctl list | grep cadoo    # macOS — PIDs and labels
systemctl --user list-units 'cadoo-gateway-*'   # Linux — units
```
...
The default profile uses `~/.cadoo/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
cadoo config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.cadoo/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=cadoo --why="gateways running" \
  sleep infinity &
...
`cadoo-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.cadoo/.env ~/.cadoo/profiles/*/.env
```

## Updating the code

`cadoo update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `cadoo gateway start` after a previous `cadoo gateway stop`. The
...
```bash
ps -ef | grep "cadoo_cli.*-p <profile>"
cat ~/.cadoo/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.cadoo.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.cadoo.gateway-<profile>.plist

# Linux
systemctl --user restart cadoo-gateway-<profile>.service
```

### Health check

```bash
cadoo doctor                  # default profile
cadoo -p <profile> doctor     # one profile
```
