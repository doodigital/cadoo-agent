...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/DooStudio/cadoo/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hermes_meta = (fm.get("metadata") or {}).get("cadoo") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `cadoo skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Cadoo Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Cadoo ships with a large built-in skill library copied into `~/.cadoo/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Cadoo also syncs bundled skills on `cadoo update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.cadoo/skills/` tree, it is still shipped with Cadoo; restore it with `cadoo skills reset <name> --restore`.",
        "",
