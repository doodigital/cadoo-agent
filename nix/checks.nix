...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      cadoo = self'.packages.default;
      hermesVenv = cadoo.cadooVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "cadoo-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "cadoo-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "cadoo-build-package" { } ''
          echo "PASS: package built at ${cadoo}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "cadoo-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "cadoo-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${cadoo}/bin/cadoo || (echo "FAIL: cadoo binary missing"; exit 1)
          test -x ${cadoo}/bin/cadoo || (echo "FAIL: cadoo binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${cadoo}/bin/cadoo version 2>&1 | grep -qi "cadoo" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "cadoo-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in cadoo cadoo cadoo-acp; do
            test -x ${cadoo}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "cadoo-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking cadoo --help ==="
          ${cadoo}/bin/cadoo --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${cadoo}/bin/cadoo --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "cadoo-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${cadoo}/share/cadoo/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${cadoo}/share/cadoo/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERMES_BUNDLED_SKILLS" ${cadoo}/bin/cadoo || \
        ...
        bundled-plugins = pkgs.runCommand "cadoo-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${cadoo}/share/cadoo/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${cadoo}/share/cadoo/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERMES_BUNDLED_PLUGINS" ${cadoo}/bin/cadoo || \
        ...
        bundled-locales = pkgs.runCommand "cadoo-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${cadoo}/share/cadoo/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${cadoo}/share/cadoo/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${cadoo}/share/cadoo/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "HERMES_BUNDLED_LOCALES" ${cadoo}/bin/cadoo || \
          ...
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HERMES_BUNDLED_LOCALES=${cadoo}/share/cadoo/locales \
        ...
        bundled-tui = pkgs.runCommand "cadoo-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${cadoo}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${cadoo}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HERMES_TUI_DIR" ${cadoo}/bin/cadoo || \
        ...
        cadoo-node = pkgs.runCommand "cadoo-node-version" { } ''
          set -e
          echo "=== Checking HERMES_NODE in wrapper ==="
          grep -q "HERMES_NODE" ${cadoo}/bin/cadoo || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${cadoo}/bin/cadoo)
        ...
        managed-guard = pkgs.runCommand "cadoo-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${cadoo}/bin/cadoo config set model foo
          check_blocked "config edit" ${cadoo}/bin/cadoo config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = cadoo.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "cadoo-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/cadoo || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/cadoo || \
          ...
          if grep -q "PYTHONPATH" ${cadoo}/bin/cadoo; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = cadoo.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "cadoo-extra-dependency-groups" { } ''
          set -e
          ...
          echo "venv: ${hermesWithGroups.cadooVenv}"
        ...
        messaging-variant = pkgs.runCommand "cadoo-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.cadooVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "cadoo-config-roundtrip" {
          ...
          merge_and_load() {
            local cadoo_home="$1"
            export CADOO_HOME="$cadoo_home"
            ${configMergeScript} ${nixSettings} "$cadoo_home/config.yaml"
            ${hermesVenv}/bin/python3 -c '
import json, sys
