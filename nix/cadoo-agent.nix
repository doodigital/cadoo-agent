# nix/cadoo.nix — Overridable Cadoo Agent package
#
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in cadoo sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/cadoo $out/bin
    cp -r ${bundledSkills} $out/share/cadoo/skills
    cp -r ${bundledPlugins} $out/share/cadoo/plugins
    cp -r ${bundledLocales} $out/share/cadoo/locales
    cp -r ${hermesWeb} $out/share/cadoo/web_dist

    mkdir -p $out/ui-tui
    cp -r ${hermesTui}/lib/cadoo-tui/* $out/ui-tui/

    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          --suffix PATH : "${runtimePath}" \
          --set HERMES_BUNDLED_SKILLS $out/share/cadoo/skills \
          --set HERMES_BUNDLED_PLUGINS $out/share/cadoo/plugins \
          --set HERMES_BUNDLED_LOCALES $out/share/cadoo/locales \
          --set HERMES_WEB_DIST $out/share/cadoo/web_dist \
  ...
  passthru = {
    ...
    # ("existing cadoo on PATH") then picks up the fully wrapped
    # `cadoo` binary — venv with all deps, bundled skills/plugins,
    ...
    devShellHook = ''
      STAMP=".nix-stamps/cadoo"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "cadoo: installing Python dependencies..."
