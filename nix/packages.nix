# nix/packages.nix — Cadoo Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    { pkgs, lib, inputs', ... }:
    let
      hermesAgent = pkgs.callPackage ./cadoo.nix {
    ...
    in
    {
      packages = {
        ...
        tui = hermesAgent.cadooTui;
        web = hermesAgent.cadooWeb;
        desktop = hermesAgent.cadooDesktop;
      };
    };
}
