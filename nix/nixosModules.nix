...
# container recreation. Environment variables are written to $CADOO_HOME/.env
# and read by cadoo at startup — no container recreation needed for env changes.
#
# Tool resolution: the cadoo wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    cadoo = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-cadoo)
    deepConfigType = lib.types.mkOptionType {
      name = "cadoo-config-attrs";
      description = "Cadoo YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "cadoo-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # CADOO_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "cadoo-documents" { } (
      ''
        mkdir -p $out
    ...
    # cadoo user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "cadoo-container-entrypoint" ''
      set -eu
      ...
      # Ensure CADOO_HOME is owned by the target user.
      ...
      if [ -n "''${CADOO_HOME:-}" ] && [ -d "$CADOO_HOME" ]; then
        find "$CADOO_HOME" \! -user "$HERMES_UID" -exec chown "$HERMES_UID:$HERMES_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/cadoo-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/cadoo-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/cadoo ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/cadoo
        chmod 0440 /etc/sudoers.d/cadoo
      fi
    ...
    # so they can update without recreation. Env vars go through $CADOO_HOME/.env.
    ...
    # Default: /var/lib/cadoo/workspace → /data/workspace.
  ...
  in {
    options.services.cadoo = with lib; {
      enable = mkEnableOption "Cadoo Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = cadoo;
        description = "The cadoo package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/cadoo";
        description = "State directory. Contains .cadoo/ subdir (CADOO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Cadoo config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $CADOO_HOME/.env at activation time.
          Cadoo reads this file on every startup via load_hermes_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $CADOO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $CADOO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `cadoo gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the cadoo user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the cadoo plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Cadoo discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          cadoo.plugins entry-point group. Each package must be built
          with the same Python interpreter as cadoo (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in cadoo's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the cadoo CLI to environment.systemPackages and export
          CADOO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.cadoo symlink to the service
            stateDir. These users are automatically added to the cadoo group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.cadoo.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the cadoo CLI to system PATH and export CADOO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.cadoo/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.CADOO_HOME = "${cfg.stateDir}/.cadoo";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.cadoo.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the cadoo user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.cadoo: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed cadoo
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure cadoo is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.cadoo        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cadoo/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cadoo/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cadoo/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cadoo/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.cadoo/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."cadoo-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.cadoo
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.cadoo ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.cadoo ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.cadoo -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.cadoo/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.cadoo/$_subdir"
            chmod 2770 "${cfg.stateDir}/.cadoo/$_subdir"
            find "${cfg.stateDir}/.cadoo/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # cadoo-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.cadoo/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.cadoo/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cadoo/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.cadoo/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.cadoo/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cadoo/.managed
          chmod 0644 ${cfg.stateDir}/.cadoo/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.cadoo/.container-mode <<'HERMES_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hermes_bin=${containerDataDir}/current-package/bin/cadoo
    HERMES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cadoo/.container-mode
            chmod 0644 ${cfg.stateDir}/.cadoo/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.cadoo/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.cadoo";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.cadoo" ]; then
                  rm -f "${symlinkPath}"
                  echo "cadoo: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.cadoo -> stateDir/.cadoo for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.cadoo";
                target = "${cfg.stateDir}/.cadoo";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "cadoo: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.cadoo/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.cadoo/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.cadoo/auth.json
              fi
            ''}
          ''}
          ...
          # Cadoo reads $CADOO_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.cadoo/.env"
    ...
    HERMES_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.cadoo/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.cadoo/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.cadoo/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.cadoo = {
          description = "Cadoo Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $CADOO_HOME/.env by the activation script. load_hermes_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/cadoo"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the cadoo group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.cadoo = {
          description = "Cadoo Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env CADOO_HOME=${containerDataDir}/.cadoo \
                --env HERMES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/cadoo gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
