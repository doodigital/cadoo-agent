# nix/desktop.nix — Cadoo Desktop (Electron) app build + wrapper
#
# `hermesAgent` is the fully-built `.#default` package — it ships the
# `cadoo` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `HERMES_DESKTOP_HERMES` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Cadoo CLI").
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/cadoo-desktop $out/bin
    cp -r ${renderer}/* $out/share/cadoo-desktop/
    ...
    substituteInPlace $out/share/cadoo-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/cadoo-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # HERMES_DESKTOP_HERMES to the absolute path of the nix-built `cadoo`
    # binary so the desktop's resolver step 4 ("existing Cadoo CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/cadoo-desktop \
      --add-flags "$out/share/cadoo-desktop" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Cadoo Agent";
    homepage = "https://github.com/DooStudio/cadoo";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "cadoo-desktop";
  };
}
