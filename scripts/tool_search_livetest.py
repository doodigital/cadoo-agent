#!/usr/bin/env python3
"""Live test harness for Cadoo Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any cadoo imports.
ORIGINAL_HOME = os.environ.get("CADOO_HOME")
ORIGINAL_AUTH = Path.home() / ".cadoo" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.cadoo/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.cadoo/.env`` so
    ...
    (cadoo_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return cadoo_home
...
def reset_module_state():
    """Drop cached modules so the new CADOO_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "cadoo_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["CADOO_HOME"] = str(home)
...
def main():
    ...
    # Restore original CADOO_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CADOO_HOME"] = ORIGINAL_HOME
    else:
