#!/usr/bin/env bash
# Canonical test runner for cadoo. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.cadoo/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.cadoo/cadoo/venv"; do
...
if [ -f "$HOME/.cadoo/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.cadoo"
