 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.cadoo/whatsapp/session
 */
...
// `cadoo update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Cadoo Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Cadoo Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Cadoo' own reply messages in self-chat mode to avoid loops.
