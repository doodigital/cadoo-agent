...
#   2. ~/.cadoo/node/ from a prior Cadoo-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.cadoo/node/ (always works, zero shell rc edits)
#
# Usage:
...
# Redirect a Cadoo-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $CADOO_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$CADOO_HOME/node/etc/npmrc"
}
...
_nb_install_bundled_node() {
    ...
    _nb_log "Extracting to $CADOO_HOME/node/..."
    ...
    mv "$extracted" "$CADOO_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    mkdir -p "$_link_dir"
    ln -sf "$CADOO_HOME/node/bin/node" "$_link_dir/node"
    ln -sf "$CADOO_HOME/node/bin/npm"  "$_link_dir/npm"
    ln -sf "$CADOO_HOME/node/bin/npx"  "$_link_dir/npx"
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $CADOO_HOME/node/"
    return 0
}
...
ensure_node() {
    ...
    if [ -x "$CADOO_HOME/node/bin/node" ]; then
        export PATH="$CADOO_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Cadoo-managed)"
            HERMES_NODE_AVAILABLE=true
            return 0
        fi
    fi
