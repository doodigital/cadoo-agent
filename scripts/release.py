...
VERSION_FILE = REPO_ROOT / "cadoo_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "jake@doostudio.com.br": "simpolism",
    ...
    "teknium@doostudio.com.br": "teknium1",
    ...
    "274096618+cadoo-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@doostudio.com.br": "shannonsands",
    ...
    "cadoo@doostudio.com.br": "DooStudio",
    ...
    "ben@doostudio.com.br": "benbarclay",
    ...
    "emozilla@doostudio.com.br": "emozilla",
    ...
    "jonny@doostudio.com.br": "yoniebans",
    "jake@doostudio.com.br": "simpolism",
    ...
    "cadoo@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (CADOO_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: cadoo-eval community link)
    ...
    "clement@doostudio.com.br": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@cadoo": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Cadoo runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level CADOO_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Cadoo
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"cadoo[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "cadoo@doostudio.com.br"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/DooStudio/cadoo",
    ...
    lines.append(f"# Cadoo Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Cadoo Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Cadoo Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Cadoo Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Cadoo Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Cadoo Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Cadoo Agent v{new_version} ({calver_date})' "
