# ============================================================================
# Cadoo Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://cadoo.nousresearch.com/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:CADOO_HOME) { $env:CADOO_HOME } else { "$env:LOCALAPPDATA\cadoo" }),
    [string]$InstallDir = $(if ($env:CADOO_HOME) { "$env:CADOO_HOME\cadoo" } else { "$env:LOCALAPPDATA\cadoo\cadoo" }),
    ...
    # builds apps/desktop into a launchable Cadoo.exe.
    #
    # Why opt-in:
    #   * Cadoo-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Cadoo.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Cadoo.exe
    ...
    #     `cadoo desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:DooStudio/cadoo.git"
$RepoUrlHttps = "https://github.com/DooStudio/cadoo.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Cadoo Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by DooStudio.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Install-Uv {
    # Cadoo owns its own uv at $HermesHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (cadoo_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `cadoo update` stay in sync.
...
function Resolve-UvCmd {
    ...
    # sibling process and CADOO_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Cadoo-Setup.exe
...
function Install-Git {
    ...
    which Cadoo uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\cadoo\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Cadoo needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Cadoo copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\cadoo\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HERMES_GIT_BASH_PATH`` (User scope) so Cadoo can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Cadoo features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Cadoo needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so Cadoo can find it even before
    ...
    Write-Warn "Could not locate bash.exe -- Cadoo may not find Git Bash."
...
function Test-Node {
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        $env:Path = "$HermesHome\node;$env:Path"
        Write-Success "Node.js $version found (Cadoo-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Cadoo-managed Node.js $NodeVersion LTS..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\cadoo-node-extract"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\cadoo-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `cadoo
                    # update` path (#4735).
                    ...
                    $stashName = "cadoo-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Cadoo behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running cadoo processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/DooStudio/cadoo/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/DooStudio/cadoo/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/DooStudio/cadoo/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\cadoo-$zipLabel.zip"
                $extractPath = "$env:TEMP\cadoo-extract"
    ...
    # next `cadoo update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Cadoo-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # DLLs by any running cadoo process. Windows denies deletion of loaded
        # DLLs, so kill any cadoo.exe tree before removing the venv.
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running cadoo processes before recreating venv..."
            & taskkill /F /T /IM cadoo.exe /FI "PID ne $myPid" 2>$null | Out-Null
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `cadoo.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install cadoo package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and cadoo.exe broken with
    ...
    # here, not 30 seconds later when the user runs `cadoo`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `cadoo dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `cadoo dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `cadoo dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up cadoo command..."
    ...
    # Add the venv Scripts dir to user PATH so cadoo is globally available
    # On Windows, the cadoo.exe in venv\Scripts\ has the venv Python baked in
    ...
    # Set CADOO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\cadoo instead
    # of the Unix default ~/.cadoo
    $currentHermesHome = [Environment]::GetEnvironmentVariable("CADOO_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("CADOO_HOME", $HermesHome, "User")
        Write-Success "Set CADOO_HOME=$HermesHome"
    }
    $env:CADOO_HOME = $HermesHome
    
    # Update current session
    $env:Path = "$hermesBin;$env:Path"
    
    Write-Success "cadoo command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.cadoo-bootstrap-complete which tells the Cadoo
    ...
    $markerPath = Join-Path $InstallDir ".cadoo-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the CADOO_HOME directory structure ($HermesHome, default %LOCALAPPDATA%\cadoo)
    ...
    # (the default PS5 behaviour), and Cadoo's prompt-injection scanner
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Cadoo-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'cadoo setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\cadoo-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\cadoo-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\cadoo-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Cadoo` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Cadoo.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/Cadoo.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Cadoo-managed
    ...
    # The --dir mode produces an unpacked Cadoo.exe in
    ...
    # are NOT trying to work around). The Cadoo icon + product name are
    # stamped onto Cadoo.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\cadoo-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Cadoo.exe",
        "$desktopDir\release\win-arm64-unpacked\Cadoo.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Cadoo.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Cadoo icon + identity are stamped onto Cadoo.exe by the
    ...
    #    Cadoo.exe. We deliberately do NOT point them at `cadoo desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Cadoo.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Cadoo.lnk')
        )
        ...
        # with the Cadoo icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.cadoo/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'cadoo setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m cadoo_cli.main setup
    } else {
        python -m cadoo_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\cadoo.exe"
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'cadoo whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: cadoo gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: cadoo gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: cadoo gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   cadoo              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   cadoo setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   cadoo config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   cadoo config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   cadoo gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   cadoo update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Cadoo repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Cadoo-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Cadoo to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://cadoo.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
