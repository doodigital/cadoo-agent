...
# Python-driven tool session (e.g. Cadoo terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:DooStudio/cadoo.git"
REPO_URL_HTTPS="https://github.com/DooStudio/cadoo.git"
CADOO_HOME="${CADOO_HOME:-$HOME/.cadoo}"
...
#   code at /usr/local/lib/cadoo, command at /usr/local/bin/cadoo,
#   data still at /root/.cadoo (CADOO_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$CADOO_HOME/.no-bundled-skills so future"
            echo "                   'cadoo update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Cadoo.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.cadoo/cadoo"
            echo "                   default (root, Linux): /usr/local/lib/cadoo"
            echo "  --cadoo-home PATH  Data directory (default: ~/.cadoo, or \$CADOO_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Cadoo installs the code under"
            echo "  /usr/local/lib/cadoo and links the command into"
            echo "  /usr/local/bin/cadoo (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CADOO_HOME"
            echo "  (default /root/.cadoo).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CADOO_HOME/cadoo are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs cadoo setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             🟣 Cadoo Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by DooStudio.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `cadoo update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Cadoo-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Cadoo Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install cadoo command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `cadoo` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CADOO_HOME/cadoo
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CADOO_HOME/cadoo
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/cadoo
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CADOO_HOME/cadoo — then preserve it)
#
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CADOO_HOME.
    if is_termux; then
        INSTALL_DIR="$CADOO_HOME/cadoo"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CADOO_HOME/cadoo/.git" ]; then
            INSTALL_DIR="$CADOO_HOME/cadoo"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/cadoo)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/cadoo"
        ...
        # /usr/local/bin/cadoo wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/cadoo"
        log_info "  Data:    $CADOO_HOME (unchanged)"
...
# Point a Cadoo-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $CADOO_HOME/node/bin — which is NOT on
...
# Cadoo's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Cadoo-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$CADOO_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$CADOO_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$CADOO_HOME/node/etc/npmrc"
}

get_hermes_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/cadoo" ]; then
        echo "$link_dir/cadoo"
    else
        echo "cadoo"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://doostudio.io/cadoo/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Cadoo owns its own uv at $CADOO_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (cadoo_cli/managed_uv.py) looks in the same
    # place, so install.sh and `cadoo update` stay in sync.
    local _managed_uv="$CADOO_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $CADOO_HOME/bin ..."
    mkdir -p "$CADOO_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/cadoo-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/cadoo-uv-installer.$$.sh")"
    ...
    # directly into $CADOO_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$CADOO_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Cadoo-
...
check_node() {
    ...
    # Repair pre-existing Cadoo-managed installs where `npm install -g` lands
    ...
    # Prefer a Cadoo-managed Node from a previous run over a too-old system one.
    if [ -x "$CADOO_HOME/node/bin/node" ] && node_satisfies_build "$("$CADOO_HOME/node/bin/node" --version)"; then
        export PATH="$CADOO_HOME/node/bin:$PATH"
        log_success "Node.js $("$CADOO_HOME/node/bin/node" --version) found (Cadoo-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Cadoo-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.cadoo/node/..."
    ...
    # Place into ~/.cadoo/node/ and symlink binaries into the same bin dir
    # the cadoo command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$CADOO_HOME/node"
    mkdir -p "$CADOO_HOME"
    mv "$extracted_dir" "$CADOO_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$CADOO_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$CADOO_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$CADOO_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$CADOO_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.cadoo/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Cadoo install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Cadoo Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `cadoo update` path
                # (#4735).
                ...
                local stash_name
                stash_name="cadoo-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Cadoo behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up cadoo command..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/cadoo"
    else
        HERMES_BIN="$(which cadoo 2>/dev/null || echo "")"
        if [ -z "$HERMES_BIN" ]; then
            log_warn "cadoo not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERMES_BIN" ]; then
        log_warn "cadoo entry point not found at $HERMES_BIN"
    ...
    # Create a user-facing shim for the cadoo command.
    ...
    rm -f "$command_link_dir/cadoo"
    cat > "$command_link_dir/cadoo" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HERMES_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/cadoo"
    log_success "Installed cadoo launcher → $command_link_display_dir/cadoo"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "cadoo command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v cadoo' \
                >/dev/null 2>&1; then
        ...
        log_info "cadoo not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Cadoo Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Cadoo Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Cadoo Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so cadoo works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.cadoo directory structure (config at top level, code in subdir)
    mkdir -p "$CADOO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.cadoo/.env (top level, easy to find)
    if [ ! -f "$CADOO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CADOO_HOME/.env"
            log_success "Created ~/.cadoo/.env from template"
        else
            touch "$CADOO_HOME/.env"
            log_success "Created ~/.cadoo/.env"
        fi
    else
        log_info "~/.cadoo/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$CADOO_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.cadoo/config.yaml (top level, easy to find)
    if [ ! -f "$CADOO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CADOO_HOME/config.yaml"
            log_success "Created ~/.cadoo/config.yaml from template"
        fi
    else
        log_info "~/.cadoo/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$CADOO_HOME/SOUL.md" ]; then
        cat > "$CADOO_HOME/SOUL.md" << 'SOUL_EOF'
# Cadoo Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Cadoo communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.cadoo/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.cadoo/"

    # Seed bundled skills into ~/.cadoo/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `cadoo update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'cadoo update'." \
            > "$CADOO_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $CADOO_HOME/.no-bundled-skills"
        log_info "  Future 'cadoo update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.cadoo/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.cadoo/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CADOO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$CADOO_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.cadoo/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Cadoo Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Cadoo will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Cadoo Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (cadoo --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `cadoo update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'cadoo setup' after install."
        return 0
    fi
    ...
    # Run cadoo setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m cadoo_cli.main setup < /dev/tty
    else
        python -m cadoo_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CADOO_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Cadoo to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CADOO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'cadoo whatsapp' to pair via QR code..."
            echo ""
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'cadoo whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'cadoo gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: cadoo gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: cadoo gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HERMES_CMD gateway > "$CADOO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.cadoo/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: cadoo gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: cadoo gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $CADOO_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $CADOO_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $CADOO_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}cadoo${NC}              Start chatting"
    echo -e "   ${GREEN}cadoo setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}cadoo config${NC}       View/edit configuration"
    echo -e "   ${GREEN}cadoo config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}cadoo gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}cadoo update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'cadoo' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'cadoo' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'cadoo' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CADOO_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CADOO_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CADOO_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$CADOO_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$CADOO_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Cadoo for pip install"
    ...
    HERMES_CMD="$(command -v cadoo 2>/dev/null || echo "")"
    if [ -n "$HERMES_CMD" ]; then
        log_info "Running cadoo setup..."
        "$HERMES_CMD" setup
    else
        log_warn "cadoo command not found on PATH"
        log_info "Try: python -m cadoo_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Cadoo` rename
...
# _purge_electron_build_cache() used by `cadoo desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Cadoo-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `cadoo desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Cadoo" ]; then
            app="$desktop_dir/release/linux-unpacked/Cadoo"
        elif [ -x "$desktop_dir/release/linux-unpacked/cadoo" ]; then
            app="$desktop_dir/release/linux-unpacked/cadoo"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Cadoo.app" \
            "$desktop_dir/release/mac/Cadoo.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Cadoo is damaged and can't be opened").
    ...
    # checkout stays clean for the next `cadoo update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Cadoo-managed Node
            # provisioned during prerequisites/node-deps (at $CADOO_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $CADOO_HOME.
    # $CADOO_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'cadoo update' on this host install.
