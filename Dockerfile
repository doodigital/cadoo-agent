...
# Do not write .pyc files at runtime: /opt/cadoo is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/cadoo/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when cadoo
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# cadoo process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main cadoo process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Cadoo WebUI' catalog). The image has moved to s6-overlay /init\
...
RUN useradd -u 10000 -m -d /opt/data cadoo
...
#
# ui-tui/packages/cadoo-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/cadoo-ink/ ui-tui/packages/cadoo-ink/
...
# lock on the @cadoo/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/cadoo/.venv at first
...
# ---------- Permissions ----------
# Link cadoo itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Keep /opt/cadoo immutable for the runtime cadoo user. Hosted/container
...
# supervised Cadoo processes drop to the non-root cadoo user.
USER root
RUN mkdir -p /opt/cadoo/bin && \
    cp /opt/cadoo/docker/cadoo-exec-shim.sh /opt/cadoo/bin/cadoo && \
    chmod 0755 /opt/cadoo/bin/cadoo && \
    printf 'docker\n' > /opt/cadoo/.install_method && \
    chown -R root:root /opt/cadoo && \
    chmod -R a+rX /opt/cadoo && \
    chmod -R a-w /opt/cadoo
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $CADOO_HOME. $CADOO_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``cadoo update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the cadoo user via
# `s6-setuidgid cadoo` in its run script. If HERMES_UID is unset, services
# run as the default cadoo user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `cadoo dump` reports
...
# /opt/cadoo/.cadoo_build_sha at build time, and have
# cadoo_cli/build_info.py read it at runtime.  Both `cadoo dump` and
...
RUN if [ -n "${HERMES_GIT_SHA}" ]; then \
        chmod u+w /opt/cadoo && \
        printf '%s\n' "${HERMES_GIT_SHA}" > /opt/cadoo/.cadoo_build_sha && \
        chmod a-w /opt/cadoo /opt/cadoo/.cadoo_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-cadoo + dashboard.
...
# `exec cadoo`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $CADOO_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/cadoo/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-cadoo-setup && \
    chmod +x /etc/cont-init.d/01-cadoo-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV HERMES_WEB_DIST=/opt/cadoo/cadoo_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/cadoo/ui-tui/dist/entry.js`) and skip the
...
ENV HERMES_TUI_DIR=/opt/cadoo/ui-tui
ENV CADOO_HOME=/opt/data
ENV HERMES_WRITE_SAFE_ROOT=/opt/data
ENV HERMES_DISABLE_LAZY_INSTALLS=1

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> cadoo ...` they default to root, and any file the
# command writes under $CADOO_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/cadoo/bin/cadoo, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid cadoo`
# when invoked as root. Non-root callers (supervised processes,
# `--user cadoo`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/cadoo/.venv/bin/cadoo). See the shim source for
...
# bin globally so `docker exec <container> cadoo ...` and any
# subprocess that doesn't activate the venv first still find cadoo.
#
# /opt/cadoo/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/cadoo/bin:/opt/cadoo/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. cadoo
# subcommand vs. no-args), drops to the cadoo user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/cadoo/docker/main-wrapper.sh" ]
CMD [ ]
