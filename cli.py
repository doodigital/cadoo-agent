#!/usr/bin/env python3
"""
Cadoo Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Cadoo Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills cadoo-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: cadoo_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See cadoo_bootstrap.py for full rationale.
try:
    import cadoo_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when cadoo_bootstrap isn't registered in the venv
    # yet — happens during partial ``cadoo update`` where git-reset landed
...
from cadoo_cli.fallback_config import get_fallback_chain
from cadoo_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from cadoo_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from cadoo_cli.pt_input_extras import (
...
from cadoo_cli.banner import _format_context_length, format_banner_version_label
...
from cadoo_cli.browser_connect import (
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.cadoo/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from cadoo_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.cadoo/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``cadoo chat --ignore-user-config``),
    the user config at ``~/.cadoo/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({CADOO_HOME}/config.yaml)
    user_config_path = _cadoo_home / 'config.yaml'
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Cadoo, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Cadoo - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see cadoo_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from cadoo_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and CadooCLI.__init__ checks "default" first.
    ...
    from cadoo_cli.config import _expand_env_vars
    ...
    # cadoo_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `cadoo config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (cadoo_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && cadoo` to control it.
...
# Initialize centralized logging early — agent.log + errors.log in ~/.cadoo/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from cadoo_logging import setup_logging
...
try:
    from cadoo_cli.config import print_config_warnings
...
try:
    from cadoo_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``cadoo --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from cadoo_cli.mcp_startup import wait_for_mcp_discovery
...
from cadoo_cli.banner import build_welcome_banner
from cadoo_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from cadoo_cli.callbacks import prompt_for_secret
...
def _notify_session_finalize(
    *,
    ...
    try:
        from cadoo_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.cadoo/cadoo`` clone is updated only by ``cadoo update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run cadoo -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"cadoo-{short_id}"
    branch_name = f"cadoo/{wt_name}"
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"cadoo pid={os.getpid()}", str(wt_path)],
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`cadoo_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from cadoo_cli.config import load_config as _load_full_config
        from cadoo_constants import get_cadoo_home as _get_cadoo_home
        _cadoo_home_maint = _get_cadoo_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_cadoo_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`cadoo_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``cadoo/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("cadoo-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``cadoo/cadoo-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``cadoo -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("cadoo/cadoo-") or b.startswith("pr-"))
    ]
...
#   6. Default: assume dark (matches the legacy Cadoo assumption)
#
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from cadoo_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from cadoo_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Cadoo output it's common to display cron schedules verbatim.
    #
...
from cadoo_constants import is_termux as _is_termux_environment
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``CadooCLI._busy_command()``.
        """
        yield self

# ASCII Art - HERMES-AGENT logo (full width, single line - requires ~95 char terminal)
CADOO_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Cadoo Agent") if _skin else "Cadoo Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        from cadoo_cli import __release_date__ as _release_date
        from cadoo_cli import __version__ as _version

        version_line = f"Cadoo Agent v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- DooStudio[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from cadoo_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.cadoo/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.cadoo/config.yaml on first use)
...
class CadooCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Cadoo Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from cadoo_cli.runtime_provider import _auto_detect_local_model
        ...
        # by `cadoo chat --ignore-rules` in cadoo_cli/main.py. When true we
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `cadoo update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Cadoo processes for this CADOO_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.cadoo/checkpoints/.  Opt-in via
        ...
        self._history_file = _cadoo_home / ".cadoo_history"
    ...
    def _claim_active_session(self, surface: str = "cli", *, stderr: bool = False) -> bool:
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from cadoo_cli.active_sessions import try_acquire_active_session
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``cadoo pets`` without a restart, mirroring
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from cadoo_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Cadoo'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from cadoo_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from cadoo_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from cadoo_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from cadoo_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from cadoo_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "🟣 Cadoo")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - CadooCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``cadoo doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``cadoo doctor`` so the banner stays
        small.
        """
        try:
            from cadoo_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Cadoo needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Cadoo LLM (not agentic)
        from cadoo_cli.model_switch import is_nous_hermes_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  DooStudio Cadoo 3 & 4 models are NOT agentic and are not "
                "designed for use with Cadoo Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.cadoo/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from cadoo_cli.clipboard import save_clipboard_image

        img_dir = get_cadoo_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'cadoo setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from cadoo_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from cadoo_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from cadoo_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Cadoo!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from cadoo_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from cadoo_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            print(f"\n  [Cadoo #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``cadoo sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `cadoo sessions list` (gemini-cli#27770 port).
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.cadoo/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``cadoo --resume <id>``
        ...
        saved_dir = get_cadoo_home() / "sessions" / "saved"
        try:
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: cadoo --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from cadoo_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Cadoo session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                from cadoo_cli.context_switch_guard import merge_preflight_compression_warning
        ...
        try:
            from cadoo_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `cadoo model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from cadoo_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from cadoo_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        from cadoo_cli.providers import get_label
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from cadoo_cli.commands import resolve_command
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_cadoo_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_cadoo_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in cadoo_cli/commands.py instead of touching every dispatch site.
        from cadoo_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from cadoo_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from cadoo_cli.skin_engine import get_active_skin
            ...
            if len(parts) > 1:
                ...
                if raw_title:
                    if self._session_db:
                        ...
                        from cadoo_state import format_session_db_unavailable
            ...
            from cadoo_cli.main import _print_version_info
            ...
            from cadoo_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `cadoo plugins
                ...
                # catalog lives behind `cadoo plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: cadoo plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_cadoo_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: cadoo plugins list)")
                else:
                    # Loaded-plugin details (tools/hooks/commands counts, errors)
                    # keyed by name, when available.
                    loaded: dict = {}
                    try:
                        from cadoo_cli.plugins import get_plugin_manager
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: cadoo plugins list)")
                    print("  Enable/disable: cadoo plugins enable/disable <name>")
            except Exception as e:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from cadoo_cli.commands import COMMANDS
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from cadoo_cli.goals import GoalManager
            from cadoo_cli.config import load_config
        except Exception as exc:
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        try:
            from cadoo_cli.goals import gather_background_processes as _gather_bg
    ...
    def _toggle_verbose(self):
        ...
        from cadoo_cli.colors import Colors as _Colors
        labels = {
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from cadoo_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by cadoo_logging not
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            print()
            _cprint(f"  💳 {_d('Not logged into DooStudio Portal.')}")
            print("  Run `cadoo portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into DooStudio Portal.')}")
                print("  Run `cadoo portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow DooStudio to charge your card."
        )
        ...
        from cadoo_cli.nous_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from cadoo_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `cadoo portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from cadoo_cli.auth import step_up_nous_billing_scope
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize DooStudio to charge {card.masked} "
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from cadoo_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.cadoo/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_cadoo_home / "config.yaml", TOOL_PROGRESS_FLAG)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _cadoo_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard CADOO_HOME, so no -p needed.
            try:
            ...
            print(f"  cadoo --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  cadoo -c \"{session_title}\"{profile_flag}")
            print()
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from cadoo_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from cadoo_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Cadoo Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Cadoo Agent! Type your message or /help for commands."
        ...
        try:
            from cadoo_cli.model_switch import prewarm_picker_cache_async
        ...
        # after an OpenClaw→Cadoo migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from cadoo_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in cadoo_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from cadoo_cli.model_switch import resolve_persist_behavior
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_cadoo_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Cadoo binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from cadoo_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Cadoo Agent")
        ...
        # Dynamic prompt: shows Cadoo symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Cadoo needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Cadoo needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Cadoo needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = CadooCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the cadoo process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: cadoo setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `cadoo sessions list`
...
def _run_kanban_goal_loop_q(cli: "CadooCLI", first_response: str) -> None:
    ...
    from cadoo_cli import kanban_db as _kb
    from cadoo_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Cadoo Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Cadoo Gateway (messaging platforms)...")
    ...
    # Default to cadoo-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Cadoo): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside CadooCLI.run(), but `-q` runs
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``cadoo chat -q "work kanban task <id>"``;
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Cadoo" box, no tool-gen
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            ...
                            if os.environ.get("HERMES_KANBAN_TASK") and result.get(
                                ...
                                try:
                                    from cadoo_cli.kanban_db import (
                ...
                # Single-query mode (`cadoo chat -q "…"`): skip the welcome
