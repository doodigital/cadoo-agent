"""Persistent slash-command worker — one CadooCLI per TUI session.
...
def _run(cli: CadooCLI, command: str) -> str:
...
def main():
    ...
    # Start before the (hundreds-of-ms) CadooCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = CadooCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
