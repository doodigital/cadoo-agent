# Contributing to Cadoo Agent

Thank you for contributing to Cadoo Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Cadoo to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo DooStudio/cadoo "<your terms>"
  gh search prs --repo DooStudio/cadoo --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/DooStudio/cadoo/issues?q=) · [PRs (all states)](https://github.com/DooStudio/cadoo/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Cadoo itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Cadoo install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.cadoo/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(cadoo_home, config)` for setup-wizard integration
...
- Integrate with `cadoo memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the Cadoo venv, wires the `cadoo` command, stamps the
install method for `cadoo update`, and clones the full git project into
`$CADOO_HOME/cadoo` (usually `~/.cadoo/cadoo`). That keeps your
...
Use this only if you intentionally do not want Cadoo' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `cadoo` entrypoint from this venv; running the
system `python3 -m cadoo_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.cadoo/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.cadoo/config.yaml
touch ~/.cadoo/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.cadoo/.env
```

### Run

```bash
# The standard installer already put `cadoo` on PATH.
cadoo doctor
cadoo chat -q "Hello"
```

If you used the manual clone fallback, run `./cadoo` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/cadoo" ~/.local/bin/cadoo
```
...
├── cli.py                    # CadooCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (cadoo-cli, cadoo-telegram, etc.)
├── cadoo_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── cadoo_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, DooStudio Portal
...
│
├── skills/                   # Bundled skills (copied to ~/.cadoo/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (doostudio.io/cadoo)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.cadoo/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.cadoo/`)

| Path | Purpose |
|------|---------|
| `~/.cadoo/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.cadoo/.env` | API keys and secrets |
| `~/.cadoo/auth.json` | OAuth credentials (DooStudio Portal) |
| `~/.cadoo/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.cadoo/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.cadoo/state.db` | SQLite session database |
| `~/.cadoo/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.cadoo/cron/` | Scheduled job data |
| `~/.cadoo/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`cadoo_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.cadoo/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (DooStudio Portal OAuth, OpenRouter API key, or custom endpoint).
...
Four fields are supported under `metadata.cadoo`:
...
The user may skip setup and keep loading the skill. Cadoo only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `cadoo setup` or update `~/.cadoo/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Cadoo tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Cadoo Agent" is the secondary collaborator. If the contributor's commit shows "Cadoo Agent" as author because they used Cadoo to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Cadoo tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `cadoo --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Cadoo uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.cadoo/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `cadoo_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `cadoo_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Cadoo runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the cadoo wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `cadoo_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `cadoo_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `cadoo_cli/gateway_windows.py::
...
Cadoo has terminal access. Security matters.
...
2. **Test manually**: Run `cadoo` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/DooStudio/cadoo/issues)
- Include: OS, Python version, Cadoo version (`cadoo version`), full error traceback
