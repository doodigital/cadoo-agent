...
prefix (e.g. ``mission-control.tilos.com/cadoo/*`` -> local Caddy ->
:9119), injecting ``X-Forwarded-Prefix: /cadoo`` on every request.
...
honour it too:
  ...
  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/cadoo/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/cadoo/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/cadoo`` when behind a prefix so they
...
from cadoo_cli.dashboard_auth import clear_providers, register_provider
from tests.cadoo_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        r = gated_app_proxied.get(
            "/sessions",
            headers={"x-forwarded-prefix": "/cadoo"},
        ...
        assert r.headers["location"].startswith("/cadoo/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/cadoo/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/cadoo/auth/callback`` — the
        ...
        assert parsed.path == "/cadoo/auth/callback", (
...
class TestPublicUrlOverride:
    ...
    def patch_config(self, monkeypatch):
        """Replace ``cadoo_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "cadoo_cli.config.load_config", lambda: cfg
            )
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        (``https://example.com/cadoo``), the OAuth callback URL is
        the path appended verbatim. The operator is declaring the
        whole authority; we trust them."""
        patch_config(None)
        monkeypatch.setenv(
            "HERMES_DASHBOARD_PUBLIC_URL", "https://example.com/cadoo",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://example.com/cadoo/auth/callback"
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        from cadoo_cli.dashboard_auth import prefix as prefix_mod

        # Reset the per-value dedup cache so the warning fires in-test
        # regardless of test ordering.
        prefix_mod._warned_malformed_public_urls.clear()
        patch_config(None)
        monkeypatch.setenv("HERMES_DASHBOARD_PUBLIC_URL", "cadoo.domain.com")
    ...
    def test_scheme_less_public_url_warning_is_deduplicated(
        ...
        scheme_warnings = [
            ...
            for r in caplog.records
            if r.levelno == logging.WARNING
            and "cadoo.domain.com" in r.getMessage()
        ]
    ...
    def test_valid_public_url_emits_no_warning(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "HERMES_DASHBOARD_PUBLIC_URL", "https://cadoo.domain.com"
        )

        with caplog.at_level(logging.WARNING, logger=prefix_mod.__name__):
            result = prefix_mod.resolve_public_url()

        assert result == "https://cadoo.domain.com"
        assert not [
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        # the cookie's Path attribute, so we need /cadoo here. Bare
        # /-rooted cookies would still be sent but would also be sent
        # to /billing/... etc.
        assert "Path=/cadoo" in pkce, (
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from cadoo_cli.dashboard_auth.cookies import set_pkce_cookie
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/cadoo, so the next request under
        ...
        (``/cadoo/auth/login``, ``/cadoo/auth/callback``). A cookie
        set with ``Path=/cadoo`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/cadoo/auth/callback``. To
        ...
        # automatically send a Path=/cadoo cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/cadoo",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        assert "Path=/cadoo" in at_cookies[0]
