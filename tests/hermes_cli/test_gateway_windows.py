"""Tests for cadoo_cli.gateway_windows."""

from pathlib import Path

import pytest

import cadoo_cli.gateway as gateway
import cadoo_cli.gateway_windows as gateway_windows
import cadoo_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda cadoo_home: "")
    monkeypatch.setattr("cadoo_cli.config.get_cadoo_home", lambda: str(cadoo_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "cadoo_cli.main"]
    assert cwd == str(cadoo_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_cadoo_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".cadoo"
        home.mkdir()
        monkeypatch.setattr("cadoo_cli.config.get_cadoo_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".cadoo"
        project = tmp_path / "checkout"
        monkeypatch.setattr("cadoo_cli.config.get_cadoo_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_cadoo_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    cadoo_home = tmp_path / "cadoo-home"
    cadoo_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(cadoo_home.resolve()))}" in content
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe.replace("python.exe", "pythonw.exe"), r"C:\\Cadoo\\cadoo\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Cadoo\\cadoo\\venv\\Scripts\\python.exe",
        r"C:\\Cadoo\\cadoo",
...
def test_elevated_gateway_command_uses_pythonw_hidden_console(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Cadoo\venv\Scripts\python.exe")
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "pythonw.exe" in content
    assert "cadoo_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("CADOO_HOME", "PYTHONIOENCODING", "HERMES_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Cadoo gateway install prompt" in out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "cadoo --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Cadoo gateway uninstall prompt" in out
...
# resume_pending=True) never fires when `cadoo gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("cadoo_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("cadoo_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `cadoo gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
    ...
    monkeypatch.setattr("cadoo_cli.gateway._get_restart_drain_timeout", lambda: 1.0)
