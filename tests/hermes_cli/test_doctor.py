...
import cadoo_cli.doctor as doctor
import cadoo_cli.gateway as gateway_cli
from cadoo_cli import doctor as doctor_mod
from cadoo_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `cadoo doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "CADOO_HOME", cadoo_home)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_cadoo_home(self, tmp_path, provider=""):
        """Create a minimal CADOO_HOME with config.yaml."""
        home = tmp_path / ".cadoo"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_cadoo_home(tmp_path, provider)
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from cadoo_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("cadoo_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "cadoo_cli.doctor", raising=False)
        from cadoo_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "DooStudio Portal auth" in out
        assert "logged in" in out
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        (cadoo_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "CADOO_HOME", cadoo_home)
        monkeypatch.setattr(doctor_mod, "get_cadoo_home", lambda: cadoo_home)
        ...
        return buf.getvalue(), cadoo_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, cadoo_home = self._run_config_section(
        ...
        assert "HERMES_MAX_ITERATIONS=90" in (cadoo_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (cadoo_home / ".env").read_text(encoding="utf-8")
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`cadoo doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Cadoo install.
