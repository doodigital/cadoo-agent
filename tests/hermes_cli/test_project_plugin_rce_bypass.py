...
1. ``cadoo_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.cadoo/plugins/`` source via
...
2. ``cadoo_cli.web_server._mount_plugin_api_routes`` then imported
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.cadoo/plugins``
