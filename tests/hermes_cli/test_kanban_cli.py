"""Tests for the kanban CLI surface (cadoo_cli.kanban)."""
...
from cadoo_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`cadoo kanban list --session <id>` filters by the originating
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="cadoo", add_help=False)
...
def test_kanban_in_autocomplete_table():
    from cadoo_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_not_gateway_only():
    ...
    from cadoo_cli.commands import COMMAND_REGISTRY
