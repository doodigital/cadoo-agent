...
``cadoo_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``cadoo --help``, ``cadoo version``,
``cadoo logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``cadoo --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from cadoo_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["cadoo", "-w", "--tui"], None),
        (["cadoo", "version"], "version"),
        (["cadoo", "--tui", "chat"], "chat"),
        (["cadoo", "-w", "logs"], "logs"),
        (["cadoo", "chat", "hello world"], "chat"),
        (["cadoo", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["cadoo", "-m", "gpt5", "chat"], "chat"),
        (["cadoo", "--model", "gpt5", "chat", "hi"], "chat"),
        (["cadoo", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["cadoo", "-z", "hello world"], None),
        (["cadoo", "-z", "hello", "chat"], "chat"),
        (["cadoo", "--model=gpt5", "chat"], "chat"),     # inline form
        (["cadoo", "--", "chat"], "chat"),               # -- terminator
        (["cadoo", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["cadoo", "some-plugin-cmd"], "some-plugin-cmd"),
        (["cadoo", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["cadoo"],                          # bare → chat
        ["cadoo", "--help"],                # top-level help
        ["cadoo", "-h"],
        ["cadoo", "version"],               # known built-in
        ["cadoo", "logs"],
        ["cadoo", "gateway", "run"],
        ["cadoo", "--tui"],
        ["cadoo", "-w", "--tui"],
        ["cadoo", "chat", "hi"],
        ["cadoo", "help"],                  # accepted built-in-ish
        ["cadoo", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["cadoo", "meet", "join"],          # potential google_meet plugin
        ["cadoo", "honcho", "status"],      # potential memory plugin
        ["cadoo", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``cadoo help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"cadoo_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
