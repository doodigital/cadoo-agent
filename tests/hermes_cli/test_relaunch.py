"""Tests for cadoo_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from cadoo_cli import relaunch as relaunch_mod


class TestResolveHermesBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/cadoo"
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'cadoo' on PATH
    ...
    def test_falls_back_to_path_which(self, monkeypatch):
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/cadoo" if name == "cadoo" else None
        )
        assert relaunch_mod.resolve_hermes_bin() == "/usr/bin/cadoo"
...
class TestBuildRelaunchArgv:
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/cadoo", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/cadoo", ["/usr/bin/cadoo", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        cadoo).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_hermes_bin", lambda: r"C:\Users\test\cadoo.exe")
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\cadoo.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_hermes_bin", lambda: r"C:\cadoo.exe")
...
class TestResolveHermesBinWindowsPyGuard:
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a cadoo.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\cadoo.exe" if name == "cadoo" else None,
        )

        bin_path = relaunch_mod.resolve_hermes_bin()
        # Must NOT be the .py — must be the cadoo.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\cadoo.exe"
    ...
    def test_windows_py_argv0_with_no_hermes_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND cadoo.exe
        isn't on PATH, return None so the caller falls back to
        python -m cadoo_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
