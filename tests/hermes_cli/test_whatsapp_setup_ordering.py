...
Before the fix, ``cadoo whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``cadoo gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    cadoo = home / ".cadoo"
    cadoo.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("CADOO_HOME", str(cadoo))
...
def _env_value(cadoo_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from cadoo_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("cadoo_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``cadoo whatsapp`` with an existing paired session and
