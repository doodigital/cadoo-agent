"""Tests for the multi-board kanban layer (``cadoo kanban boards …``).
...
* CLI surface: ``cadoo kanban boards list/create/switch/rm``.
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated CADOO_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test CADOO_HOME plus a path-init cache reset
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "cadoo", "proj_1", "a",
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``cadoo kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
    env["PYTHONPATH"] = str(_WORKTREE)
    if env_extra:
        env.update(env_extra)
    return subprocess.run(
        [sys.executable, "-m", "cadoo_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
