"""Tests for cadoo_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_cadoo_home):
    """Drop the example-dashboard fixture into the per-test CADOO_HOME
    ...
    real bundled plugins — kanban, cadoo-achievements, model providers
    ...
    from cadoo_cli import web_server
    ...
    #      and start serving requests against a torn-down CADOO_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Cadoo vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Cadoo env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import cadoo_cli.web_server as ws
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_cadoo_home):
        """Create a TestClient and isolate the state DB under the test CADOO_HOME."""
        try:
        ...
        import cadoo_state
        from cadoo_constants import get_cadoo_home
        from cadoo_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(cadoo_state, "DEFAULT_DB_PATH", get_cadoo_home() / "state.db")
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        import cadoo_cli.web_server as web_server
    ...
    def test_dashboard_update_capability_detects_generic_container(self, monkeypatch):
        ...
        monkeypatch.setattr(cadoo_constants, "is_container", lambda: True)
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in cadoo-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "cadoo"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from cadoo_constants import get_cadoo_home
        from cadoo_cli.config import load_config, load_env
        ...
        config_path = get_cadoo_home() / "hindsight" / "config.json"
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_cadoo_home() / "images" / "nope.png"
        resp = self.client.get("/api/media", params={"path": str(missing)})
        assert resp.status_code == 404

    def test_get_media_requires_auth(self):
        from cadoo_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("cadoo_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from cadoo_cli.web_server import app
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn cadoo update")

        # Bypass the managed-externally gate so we reach the docker install check.
        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_hermes_action", fail_spawn)
        web_server._ACTION_PROCS.pop("cadoo-update", None)
        web_server._ACTION_RESULTS.pop("cadoo-update", None)

        resp = self.client.post("/api/cadoo/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "cadoo-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/cadoo:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/cadoo-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/cadoo:latest" in line for line in status_data["lines"])
    ...
    def test_update_hermes_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn cadoo update")
    ...
    def test_update_hermes_spawns_on_non_docker_install(self, monkeypatch):
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "cadoo-update"}
        assert calls == [(["update"], "cadoo-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("cadoo-update", None)
        web_server._ACTION_RESULTS.pop("cadoo-update", None)
        web_server._ACTION_PROCS["cadoo-update"] = proc

        resp = self.client.get("/api/actions/cadoo-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "cadoo-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["cadoo-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from cadoo_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `cadoo model` but
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from cadoo_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "cadoo_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Cadoo provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `cadoo import` doesn't auto-abort at the overwrite prompt."""
        import cadoo_cli.web_server as ws
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from cadoo_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from cadoo_cli.config import save_env_value
        from cadoo_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://doostudio.io/cadoo/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_update_messaging_platform_saves_slack_allowed_users(self):
        from cadoo_cli.config import load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Cadoo Agent"}
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``cadoo gateway restart`` child (e.g. when a stale
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/cadoo_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/cadoo"})
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import cadoo_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "cadoo-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from cadoo_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from cadoo_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``cadoo model`` custom flow. Regression for the desktop loop where a
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "cadoo-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `cadoo model`), not the first curated paid entry."""
        import cadoo_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from cadoo_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from cadoo_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_cadoo_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("cadoo://")
    ...
    def test_profiles_list_includes_default(self):
        from cadoo_constants import get_cadoo_home
        get_cadoo_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from cadoo_constants import get_cadoo_home
        import cadoo_cli.profiles as profiles_mod

        cadoo_home = get_cadoo_home()
        cadoo_home.mkdir(parents=True, exist_ok=True)
        (cadoo_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = cadoo_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_cadoo_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_hermes_for_default_profile(self):
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "cadoo setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/cadoo/bin/cadoo")
        ...
        if is_windows:
            assert lines == ["@echo off", "cadoo -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/cadoo/bin/cadoo -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_cadoo_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_cadoo_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_cadoo_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_cadoo_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_cadoo_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_cadoo_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_cadoo_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_cadoo_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_cadoo_home() / "profiles" / "builder"
        token = set_cadoo_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_cadoo_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import cadoo_cli.skills_config as skills_config
        import cadoo_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import cadoo_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from cadoo_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from cadoo_cli.web_server import _denormalize_config_from_web
        from cadoo_cli.config import save_config
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from cadoo_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.cadoo/dashboard-themes/."""

    def test_returns_empty_when_dir_missing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CADOO_HOME", str(tmp_path))
        from cadoo_cli import web_server
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_cadoo_home):
        ...
        monkeypatch.setattr(
            cadoo_state, "DEFAULT_DB_PATH", get_cadoo_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``cadoo_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in cadoo_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_cadoo_home):
        ...
        # Pin the SessionDB to the isolated CADOO_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_cadoo_home, _install_example_plugin):
        ...
        installs it into the per-test ``CADOO_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into CADOO_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, cadoo-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (cadoo-achievements is loaded by default).
        resp = self.client.get("/api/plugins/cadoo-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``cadoo --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import cadoo_cli.main as main_mod
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_cadoo_home
    ):
        import cadoo_cli.main as main_mod

        config_path = Path(os.environ["CADOO_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/cadoo-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/cadoo-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"cadoo-ws-ok" in buf:
                    break
            assert b"cadoo-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from cadoo_cli.pty_bridge import PtyUnavailableError
        ...
        import cadoo_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from cadoo_cli import web_server as ws_mod
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_cadoo_home, _install_example_plugin):
        ...
        the per-test ``CADOO_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_cadoo_home):
        import threading
    ...
    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_cadoo_home):
        import threading
        import cron.scheduler as sched
