...
def cadoo_home(tmp_path, monkeypatch):
    ...
    # /etc/cadoo on the dev/CI box can't influence the test.
...
def test_user_config_overrides_default(cadoo_home, monkeypatch):
    from cadoo_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(cadoo_home, monkeypatch):
    from cadoo_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(cadoo_home):
