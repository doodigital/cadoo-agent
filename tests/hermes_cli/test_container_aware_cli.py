...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("cadoo_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "cadoo"
    assert info["exec_user"] == "cadoo"
    assert info["hermes_bin"] == "/data/current-package/bin/cadoo"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("cadoo_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        cadoo_home = Path(tmpdir) / ".cadoo"
        cadoo_home.mkdir()
        (cadoo_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("cadoo_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_cadoo_home": lambda: cadoo_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=cadoo-custom\n"
        "exec_user=myuser\n"
        "hermes_bin=/opt/cadoo/bin/cadoo\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "cadoo-custom"
    assert info["exec_user"] == "myuser"
    assert info["hermes_bin"] == "/opt/cadoo/bin/cadoo"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from cadoo_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "cadoo"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "cadoo" in cmd
    assert "/data/current-package/bin/cadoo" in cmd
    assert "chat" in cmd
