"""Tests for cadoo_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate CADOO_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".cadoo"
    home.mkdir()
    ...
    import importlib
    from cadoo_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "nous": {
                "metadata": {"display_name": "DooStudio Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from cadoo_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://doostudio.io/cadoo/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/DooStudio/cadoo"
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in cadoo_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from cadoo_cli import model_catalog
        from cadoo_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from cadoo_cli import model_catalog
        from cadoo_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``cadoo model`` CLI — not the live
        ...
        # ``CADOO_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` CADOO_HOME directly instead.
        import importlib
        from cadoo_cli import model_catalog
        from cadoo_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from cadoo_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["CADOO_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("cadoo_cli.models.check_nous_free_tier", return_value=False), patch(
                "cadoo_cli.models.union_with_portal_free_recommendations",
