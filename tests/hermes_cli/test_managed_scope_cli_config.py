...
cadoo_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `cadoo config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_cadoo_home = get_cadoo_home()`` at import time (module
    singleton), so monkeypatching CADOO_HOME after import doesn't move it.
    ...
    this only matters for tests that swap CADOO_HOME.)
    """
    import cli

    cli._cadoo_home = home
