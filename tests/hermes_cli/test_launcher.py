"""Tests for the top-level `./cadoo` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./cadoo` should use `cadoo_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "cadoo"
    called = []

    fake_main_module = types.ModuleType("cadoo_cli.main")

    def fake_main():
        called.append("cadoo_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "cadoo_cli.main", fake_main_module)
