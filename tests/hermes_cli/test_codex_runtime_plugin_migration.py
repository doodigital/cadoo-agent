...
from cadoo_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from cadoo_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from cadoo_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_expose_hermes_tools_writes_callback_mcp_entry(self, tmp_path):
        ...
        [mcp_servers.cadoo-tools] entry is written so codex calls back
        into Cadoo for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by cadoo' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_hermes_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.cadoo-tools]" in text
        ...
        assert "cadoo-tools" in report.migrated
    ...
    def test_expose_hermes_tools_disabled_skips_entry(self, tmp_path):
        ...
        migrate({}, codex_home=tmp_path,
                ...
                expose_hermes_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.cadoo-tools]" not in text
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Cadoo appends
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Cadoo' managed block, re-running
        ...
        migrate({"mcp_servers": {"cadoo-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.cadoo-mcp]" in final
...
class TestStripUnmanagedPluginTables:
    ...
    ``cadoo codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "cadoo_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: CADOO_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``CADOO_HOME``
    ...
    once codex spawned the cadoo-tools MCP subprocess.
    """
    ...
    def test_tempdir_detector_recognizes_pytest_paths(self):
        ...
        assert _looks_like_test_tempdir(
            "/tmp/pytest-of-user/pytest-12/test_X/cadoo"
        )
        assert _looks_like_test_tempdir(
            "/private/var/folders/zz/T/pytest-of-bob/pytest-1"
        )

    def test_tempdir_detector_accepts_real_cadoo_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.cadoo")
        assert not _looks_like_test_tempdir("/home/bob/.cadoo")
        assert not _looks_like_test_tempdir("/opt/cadoo")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when CADOO_HOME points at a pytest
        ...
        env = entry.get("env", {})
        assert "CADOO_HOME" not in env, (
            f"pytest-tempdir CADOO_HOME leaked into codex MCP entry: "
            f"{env.get('CADOO_HOME')!r}"
        )

    def test_real_cadoo_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate CADOO_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.cadoo"
        monkeypatch.setenv("CADOO_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("CADOO_HOME") == real_path

    def test_unset_cadoo_home_omits_env_key(self, monkeypatch):
        """When CADOO_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever CADOO_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "CADOO_HOME" not in env, (
            f"CADOO_HOME should not be set when env var is unset, got: "
            f"{env.get('CADOO_HOME')!r}"
        )
