"""Tests for normalized DooStudio Portal account entitlement helpers."""
...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    ...
    monkeypatch.setattr("cadoo_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    ...
    assert info.product_id == "nous-cadoo"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("cadoo_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("cadoo_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("cadoo_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("cadoo_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "cannot verify your DooStudio Portal paid access" in message
    assert "Log in with `cadoo model`" in message
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    assert "Run `cadoo model`" in message
...
def test_entitlement_message_for_account_missing():
    ...
    assert message is not None
    assert "could not find a DooStudio Portal account or organisation" in message
...
def test_topup_url_defaults_to_production_portal_for_none():
    ...
    assert url == "https://doostudio.io/portal/billing?topup=open"
