"""Tests for IRC gateway configuration via `cadoo setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Cadoo install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Cadoo install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#cadoo")
            monkeypatch.setenv("IRC_NICKNAME", "cadoo-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `cadoo setup gateway` experience with IRC present."""
