"""Tests for the `cadoo memory reset` CLI command.
...
- Profile-scoped reset (uses CADOO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake CADOO_HOME with memory files."""
    cadoo_home = tmp_path / ".cadoo"
    memories = cadoo_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("CADOO_HOME", str(cadoo_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.cadoo/cadoo\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `cadoo memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        cadoo_home = tmp_path / ".cadoo"
        (cadoo_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("CADOO_HOME", str(cadoo_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's CADOO_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_cadoo_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
