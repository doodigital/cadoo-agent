"""
Tests for mcp_serve — Cadoo MCP server.
...
def _isolate_cadoo_home(tmp_path, monkeypatch):
    """Redirect CADOO_HOME to a temp directory."""
    monkeypatch.setenv("CADOO_HOME", str(tmp_path))
    try:
        import cadoo_constants
        monkeypatch.setattr(cadoo_constants, "get_cadoo_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking cadoo_state schema."""
