...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.cadoo/cadoo/utils.py)
...
2. ``cadoo update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``cadoo_cli/main.py`` (the ``cadoo update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``cadoo_constants`` after the
