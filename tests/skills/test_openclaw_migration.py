...
"""
    ...
    assert "Tyler Williams > Active Projects: Cadoo Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `cadoo cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.cadoo/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Cadoo``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Cadoo prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Cadoo to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Cadoo config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Cadoo uses tools well"
    # All-lowercase matches → lowercase ``cadoo``; this preserves the
    # real filesystem path ``~/.cadoo`` (Cadoo home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "cadoo should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Cadoo remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "cadoo prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Cadoo was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "cadoo uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Cadoo said to ask Cadoo about Cadoo settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.cadoo`` (the real Cadoo home), not the broken
    ``.Cadoo``.
    ...
    rewritten to ``~/.Cadoo/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.cadoo/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .cadoo directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.cadoo'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "cadoo config path: ~/.cadoo/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Cadoo" in result
