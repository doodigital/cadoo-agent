...
def server():
    with patch.dict(
        "sys.modules",
        {
            "cadoo_constants": MagicMock(
                get_cadoo_home=MagicMock(return_value="/tmp/hermes_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'cadoo-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'cadoo-release' patched"
    }
