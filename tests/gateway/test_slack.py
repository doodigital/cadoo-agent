...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.cadoo."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /cadoo, /btw, /stop,
        ...
        for expected in ("/cadoo", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@cadoo`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@cadoo"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@cadoo")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /cadoo subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /cadoo btw."""
        command = {
            "command": "/btw",
    ...
    async def test_legacy_hermes_prefix_still_works(self, adapter):
        """Backward compat: /cadoo btw foo must still route to /btw foo.

        Old workspace manifests only declared /cadoo as the single slash.
    ...
    async def test_legacy_hermes_freeform_question(self, adapter):
        """/cadoo <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/cadoo",
...
class TestSlashEphemeralAck:
    ...
    async def test_legacy_hermes_slash_stashes_context(self, adapter):
        """Legacy /cadoo <subcommand> also stashes context."""
        command = {
            "command": "/cadoo",
            "text": "help",
            "user_id": "U_H",
    ...
    async def test_freeform_hermes_question_does_not_stash_context(self, adapter):
        """Free-form /cadoo <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/cadoo",
