...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.cadoo/media/'
...
class TestMediaDeliveryPathValidation:
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.cadoo/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_hermes_credentials(self, tmp_path, monkeypatch):
        """~/.cadoo/.env and ~/.cadoo/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        hermes_dir = fake_home / ".cadoo"
        hermes_dir.mkdir(parents=True)
        env_file = hermes_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._CADOO_HOME",
            hermes_dir,
        )
    ...
    def test_denylist_blocks_shared_hermes_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Cadoo root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".cadoo" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        hermes_root = fake_home / ".cadoo"
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the CADOO_HOME root (google_token.json)
        ...
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._CADOO_HOME", hermes_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.cadoo/pairing/ (platform pairing tokens) are
    ...
    def test_denylist_blocks_non_cache_file_under_cadoo_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.cadoo
    ...
    def test_root_home_hermes_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.cadoo/.env`` stays blocked under the $HOME exception — it is a
