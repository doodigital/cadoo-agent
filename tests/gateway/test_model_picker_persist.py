...
closure the PR changed, against a real temp ``CADOO_HOME``.
"""
...
def _fake_switch_result():
    ...
    from cadoo_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_cadoo_home", cadoo_home)
    ...
    monkeypatch.setattr(
        "cadoo_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_cadoo_home() / config.yaml`` — point it here.
    monkeypatch.setattr("cadoo_constants.get_cadoo_home", lambda: cadoo_home)
    monkeypatch.setattr("cadoo_cli.config.get_cadoo_home", lambda: cadoo_home)
    return cfg_path
