"""Shared fixtures for the cadoo test suite.
...
2. **Isolated CADOO_HOME.** CADOO_HOME points to a per-test tempdir so
   code reading ``~/.cadoo/*`` via ``get_cadoo_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".cadoo"`` instead of the canonical
   ``get_cadoo_home()`` is a bug to fix at the callsite.)
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    "HERMES_EXEC_ASK",
    "CADOO_HOME_MODE",
    "CADOO_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.cadoo/kanban.db instead of the per-test CADOO_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
    "HERMES_KANBAN_HOME",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CADOO_HOME to per-test tempdirs so code that
    reads ``~/.cadoo/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 3. Redirect CADOO_HOME to a per-test tempdir. Code that reads
    #    ``~/.cadoo/*`` via ``get_cadoo_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.cadoo/*`` via
    #    ``Path.home() / ".cadoo"`` instead of ``get_cadoo_home()``
    #    is a bug to fix at the callsite.
    fake_cadoo_home = tmp_path / "hermes_test"
    fake_cadoo_home.mkdir()
    (fake_cadoo_home / "sessions").mkdir()
    (fake_cadoo_home / "cron").mkdir()
    (fake_cadoo_home / "memories").mkdir()
    (fake_cadoo_home / "skills").mkdir()
    monkeypatch.setenv("CADOO_HOME", str(fake_cadoo_home))
    ...
    #    ~/.cadoo/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import cadoo_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_cadoo_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal cadoo config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``cadoo-gateway`` process
...
# correlated with a ``tests/cadoo_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> cadoo-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart cadoo-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting cadoo/python patterns are also blocked.
    """
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch cadoo-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m cadoo_cli.main".
                if (
                    "cadoo" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live cadoo-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting cadoo/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `cadoo update` (or the
        # equivalent `python -m cadoo_cli.main update`).  These commands
        ...
        if "update" in low and (
            # cadoo update / cadoo update --gateway / setsid bash -c ... cadoo update
            ("cadoo" in low and "update" in low.split())
            or
            # python -m cadoo_cli.main update --gateway
            ("cadoo_cli" in low and "update" in low.split())
            or
            # venv/bin/cadoo update  (absolute path variant used in tests)
            (".venv/bin/cadoo" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`cadoo update` against the real checkout, fetching "
