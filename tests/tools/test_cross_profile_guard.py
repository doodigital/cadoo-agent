...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Cadoo layout and point CADOO_HOME at
    the cadoo-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_hermes):
        ...
        assert "cadoo-security" in result["error"]
        # File untouched.
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        # Re-import the module so SKILLS_DIR picks up CADOO_HOME (set in
        ...
        assert "not found in active profile 'cadoo-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.cadoo/profiles/<name>/."""
        # Don't set CADOO_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_cadoo_home_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        """When active profile is 'cadoo-security', the prompt warns
        ...
        assert "Active Cadoo profile" in src
        assert "cross_profile=True" in src
        assert "~/.cadoo/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Cadoo profile: default" in src
        assert "Active Cadoo profile: {active_profile}" in src
