...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        # ``.env`` under the active CADOO_HOME (profile-aware, not just
        # ``~/.cadoo``) must be write-denied. The hermetic test conftest
        # points CADOO_HOME at a tempdir — resolve via get_cadoo_home()
        # to match the denylist.
        from cadoo_constants import get_cadoo_home
        path = str(get_cadoo_home() / ".env")
        assert _is_write_denied(path) is True
    ...
    def test_hermes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.cadoo/.env`` (whose credentials are inherited by every profile)
