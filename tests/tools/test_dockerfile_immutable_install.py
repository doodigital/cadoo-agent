"""Contract tests for the Docker image's immutable /opt/cadoo install tree."""
...
def test_dockerfile_makes_opt_hermes_root_owned_and_non_writable() -> None:
    ...
    assert "COPY --chown=cadoo:cadoo . ." not in text
    assert "COPY . ." in text
    assert "chown -R root:root /opt/cadoo" in text
    assert "chmod -R a+rX /opt/cadoo" in text
    assert "chmod -R a-w /opt/cadoo" in text

    immutable_block = re.search(
        r"RUN mkdir -p /opt/cadoo/bin && \\\n"
        r"(?:.*\\\n)+?"
        r"\s+chmod -R a-w /opt/cadoo",
        text,
    )
    assert immutable_block, "Dockerfile must lock /opt/cadoo after installing code/deps"


def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    text = _dockerfile_text()

    assert "ENV CADOO_HOME=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "HERMES_TUI_DIR=/opt/cadoo/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+cadoo:cadoo\s+/opt/cadoo/\.venv",
        r"chown\s+-R\s+cadoo:cadoo\s+/opt/cadoo/ui-tui",
        r"chown\s+-R\s+cadoo:cadoo\s+/opt/cadoo/gateway",
        r"chown\s+-R\s+cadoo:cadoo\s+/opt/cadoo/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/cadoo must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/cadoo/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $CADOO_HOME data volume (which a host install may also use).
    It must live inside the immutable block so the runtime user can't alter it.
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/cadoo/.install_method" in text
