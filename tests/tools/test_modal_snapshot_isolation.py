...
def _restore_tool_modules():
    original_cadoo_home = os.environ.get("CADOO_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_cadoo_home is None:
            os.environ.pop("CADOO_HOME", None)
        else:
            os.environ["CADOO_HOME"] = original_cadoo_home
        _reset_modules(("tools", "cadoo_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    cadoo_cli = types.ModuleType("cadoo_cli")
    cadoo_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["cadoo_cli"] = cadoo_cli
    cadoo_home = tmp_path / "cadoo-home"
    os.environ["CADOO_HOME"] = str(cadoo_home)
    sys.modules["cadoo_cli.config"] = types.SimpleNamespace(
        get_cadoo_home=lambda: cadoo_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="cadoo", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": cadoo_home / "modal_snapshots.json",
