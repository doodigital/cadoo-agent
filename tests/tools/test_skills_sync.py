...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.cadoo/)``
    ...
    def test_refuses_cadoo_home_itself(self, tmp_path):
        """``~/.cadoo/`` itself is what the #48200 wipe destroyed."""
        from tools.skills_sync import _rmtree_writable

        cadoo = tmp_path / "home"
        cadoo.mkdir()
        (cadoo / "skills").mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", cadoo / "skills"):
            with pytest.raises(ValueError, match="refusing to rmtree"):
                _rmtree_writable(cadoo)
    ...
    def test_refuses_sibling_directory(self, tmp_path):
        ...
        skills = cadoo / "skills"
        skills.mkdir()
        not_skills = cadoo / "kanban.db"  # any non-skills path
        not_skills.mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", skills):
            with pytest.raises(ValueError, match="refusing to rmtree"):
...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "cadoo skills reset new-skill" in captured
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `cadoo profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.cadoo) rely on so bundled
    skills are never seeded at install time NOR re-injected by `cadoo update`.
    """
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        cadoo_home = tmp_path / "home"
        cadoo_home.mkdir()
        (cadoo_home / ".no-bundled-skills").write_text("opted out\n")

        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.CADOO_HOME", cadoo_home):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    """`cadoo skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.CADOO_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.CADOO_HOME", home):
            sync_skills(quiet=True)
            # User edits 'beta'
