...
Verifies that Cadoo-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Cadoo secrets.

See: https://github.com/DooStudio/cadoo/issues/1002
See: https://github.com/DooStudio/cadoo/issues/1264
"""
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.cadoo/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Cadoo inference secret
        ...
        models`` run inside a Cadoo terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        Cadoo-managed; the rest belongs to the user.
        """
        general_chain = {
...
class TestBlocklistCoverage:
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Cadoo-managed
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Cadoo, and blocklisting
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_hermes_bin_injection(self):
        ...
        cadoo-install-dir prepend (a separate concern, covered by
        TestHermesBinDirOnPath) so a real ``cadoo`` on the test runner's PATH
...
class TestHermesBinDirOnPath:
    """The cadoo install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``cadoo`` via the terminal tool must work
    even when the gateway was launched without the cadoo install dir on
    ...
    def test_resolves_via_which(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/cadoo/bin/cadoo" if name == "cadoo" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/cadoo/bin")
        assert local_mod._resolve_hermes_bin_dir() == "/opt/cadoo/bin"
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "cadoo").write_text("#!/bin/sh\n")
    ...
    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._HERMES_BIN_DIR = "/opt/cadoo/bin"
        out = local_mod._prepend_hermes_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/cadoo/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        assert twice == once
        assert once.split(os.pathsep).count("/opt/cadoo/bin") == 1
    ...
    def test_make_run_env_injects_hermes_bin_dir(self, monkeypatch):
        """A gateway env missing the cadoo dir gets it back in the subshell PATH."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._HERMES_BIN_DIR = "/opt/cadoo/bin"
        ...
        assert entries[0] == "/opt/cadoo/bin"
