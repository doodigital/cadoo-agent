...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        cadoo_home.mkdir()
        (cadoo_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"CADOO_HOME": str(cadoo_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(cadoo_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.cadoo/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        (cadoo_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        (cadoo_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        (cadoo_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.cadoo/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        cadoo_home = tmp_path / ".cadoo"
        (cadoo_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"CADOO_HOME": str(cadoo_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.cadoo")

        assert mounts[0]["container_path"] == "/home/user/.cadoo/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.cadoo/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.cadoo/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape CADOO_HOME."""
        monkeypatch.setenv("CADOO_HOME", str(tmp_path / ".cadoo"))
        (tmp_path / ".cadoo").mkdir()

        # Create a sensitive file one level above cadoo_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside cadoo_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside CADOO_HOME must still be registered."""
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        monkeypatch.setenv("CADOO_HOME", str(cadoo_home))
        (cadoo_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_cadoo_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of CADOO_HOME must be allowed."""
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        subdir = cadoo_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside CADOO_HOME pointing outside must be rejected."""
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        monkeypatch.setenv("CADOO_HOME", str(cadoo_home))

        # Create a sensitive file outside cadoo_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside cadoo_home pointing outside
        symlink = cadoo_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes CADOO_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, cadoo_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape CADOO_HOME."""
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        monkeypatch.setenv("CADOO_HOME", str(cadoo_home))
        ...
        sensitive.write_text("{}")
        self._write_config(cadoo_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(cadoo_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside CADOO_HOME via config must still mount."""
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        monkeypatch.setenv("CADOO_HOME", str(cadoo_home))

        (cadoo_home / "oauth.json").write_text("{}")
        self._write_config(cadoo_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        (cadoo_home / "cache" / "documents").mkdir(parents=True)
        (cadoo_home / "cache" / "audio").mkdir(parents=True)
        (cadoo_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("CADOO_HOME", str(cadoo_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.cadoo/cache/documents" in paths
        assert "/root/.cadoo/cache/audio" in paths
        assert "/root/.cadoo/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.cadoo/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        (cadoo_home / "document_cache").mkdir()
        (cadoo_home / "image_cache").mkdir()
        monkeypatch.setenv("CADOO_HOME", str(cadoo_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(cadoo_home / "document_cache") in host_paths
        assert str(cadoo_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.cadoo/cache/documents" in container_paths
        assert "/root/.cadoo/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        cadoo_home = tmp_path / ".cadoo"
        img_dir = cadoo_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.cadoo/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.cadoo")
            == "/home/agent/.cadoo/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        cadoo_home = tmp_path / ".cadoo"
        (cadoo_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("CADOO_HOME", str(cadoo_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(cadoo_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        cadoo_home = tmp_path / ".cadoo"
        doc_dir = cadoo_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        cadoo_home = tmp_path / ".cadoo"
        ss_dir = cadoo_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.cadoo/cache/screenshots/session_abc/screen1.png"
