...
Hosted/container Cadoo keeps user-writable state under CADOO_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/cadoo must remain root-owned/non-writable by the runtime cadoo user so
...
def test_stage2_does_not_chown_install_tree_to_hermes(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to cadoo; "
            "the Dockerfile keeps /opt/cadoo immutable and writable state "
            "belongs under CADOO_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "/opt/cadoo" in stage2_text
