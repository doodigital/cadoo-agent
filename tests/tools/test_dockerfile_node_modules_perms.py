...
Older images made /opt/cadoo/ui-tui and /opt/cadoo/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERMES_TUI_DIR=/opt/cadoo/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R cadoo:cadoo /opt/cadoo/ui-tui" not in text
    assert "chown -R cadoo:cadoo /opt/cadoo/node_modules" not in text
