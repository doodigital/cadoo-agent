"""Regression tests for CADOO_HOME override propagation onto the MCP loop.
...
get_cadoo_home() inside the coroutine, most visibly OAuth token-store
...
def test_override_propagates_to_mcp_loop(tmp_path, monkeypatch, mcp_loop):
    ...
    process_home.mkdir()
    profile_home.mkdir()
    monkeypatch.setenv("CADOO_HOME", str(process_home))
    ...
    token = set_cadoo_home_override(str(profile_home))
    try:
...
def test_concurrent_scopes_do_not_interfere(tmp_path, monkeypatch, mcp_loop):
    ...
    def scoped_call(key, home):
        token = set_cadoo_home_override(str(home))
        try:
