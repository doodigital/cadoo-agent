...
guidance, while still allowing:
  ...
  - root start (id -u == 0)
  - `--user <cadoo-uid>` (the supported non-root start, #34648 / #34837)
...
root, and the baked image dirs are owned by the cadoo build UID, so an
...
PUID/PGID aliases), which remaps the cadoo user and chowns the volume.

The guard fires only when the current UID is neither root NOR the cadoo UID,
so the #34648 `--user 10000:10000` case (pinning to the cadoo UID itself) is
unaffected.
...
def _guard_block(text: str) -> str:
    ...
    assert m, "expected the --user guard block (cur_uid + non-root/non-cadoo check)"
    return m.group(1)
...
def test_guard_present_and_mentions_remediation(path: Path) -> None:
    ...
    # Must check non-root AND non-cadoo-uid (so --user 10000:10000 is allowed).
    assert '"$cur_uid" != 0' in block
    assert '"$cur_uid" != "$(id -u cadoo)"' in block
...
def _run_guard(text: str, *, cur_uid: int, hermes_uid: int = 10000) -> subprocess.CompletedProcess:
    ...
    with tempfile.TemporaryDirectory() as d:
        script = (
            "set -e\n"
            # Stub `id`: `id -u` -> cur_uid; `id -u cadoo` -> hermes_uid.
            f'id() {{ if [ "$2" = cadoo ]; then echo {hermes_uid}; else echo {cur_uid}; fi; }}\n'
            + block
...
def test_arbitrary_user_uid_is_rejected() -> None:
    """An arbitrary host UID (1000), neither root nor cadoo, is rejected."""
...
def test_user_pinned_to_hermes_uid_passes() -> None:
    """`--user 10000:10000` (the cadoo UID itself) is the supported non-root
...
def test_user_pinned_to_remapped_hermes_uid_passes() -> None:
    """After a HERMES_UID remap the cadoo UID is e.g. 4242; a container pinned
