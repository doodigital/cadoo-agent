...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``cadoo tools`` repaint and at
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.cadoo/auth.json
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `cadoo tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``cadoo tools``,
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``cadoo auth`` sign-in) rather than an env-var key.
    Patches at the ``cadoo_cli.runtime_provider.resolve_runtime_provider``
