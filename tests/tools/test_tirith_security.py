...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(cadoo_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(cadoo_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        """After _INSTALL_FAILED, manual install in CADOO_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
...
# ---------------------------------------------------------------------------
# CADOO_HOME isolation
# ---------------------------------------------------------------------------

class TestHermesHomeIsolation:
    def test_hermes_bin_dir_respects_cadoo_home(self):
        """_hermes_bin_dir must use CADOO_HOME, not hardcoded ~/.cadoo."""
        from tools.tirith_security import _hermes_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CADOO_HOME": tmpdir}):
            result = _hermes_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_cadoo_home(self):
        """_failure_marker_path must use CADOO_HOME, not hardcoded ~/.cadoo."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"CADOO_HOME": "/custom/cadoo"}):
            result = _failure_marker_path()
        assert result == "/custom/cadoo/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets CADOO_HOME; verify it's active."""
        cadoo_home = os.getenv("CADOO_HOME")
        assert cadoo_home is not None, "CADOO_HOME should be set by conftest"
        assert "hermes_test" in cadoo_home, "Should point to test temp dir"

    def test_get_cadoo_home_fallback(self):
        """Without CADOO_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_cadoo_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove CADOO_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".cadoo")
            result = _get_cadoo_home()
