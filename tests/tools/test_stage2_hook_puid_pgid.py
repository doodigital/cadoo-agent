...
to docker/stage2-hook.sh, which is installed as /etc/cont-init.d/01-cadoo-setup
...
def test_no_uid_vars_leaves_values_empty(stage2_text: str) -> None:
    # An empty resolution means the stage2 hook keeps the default cadoo user.
...
def test_stage2_hook_runs_config_migration_as_hermes(stage2_text: str) -> None:
    ...
    assert 's6-setuidgid cadoo "$INSTALL_DIR/.venv/bin/python"' in stage2_text
