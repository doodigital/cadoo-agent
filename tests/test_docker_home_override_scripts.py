...
def test_main_wrapper_preserves_docker_workdir() -> None:
    ...
    # first exec / if-block.
    activate_idx = text.index("/opt/cadoo/.venv/bin/activate")
...
def test_dashboard_run_resets_home_before_dropping_privileges() -> None:
    ...
    assert "exec s6-setuidgid cadoo cadoo dashboard" in text
...
def test_stage2_hook_repairs_profiles_and_cron_ownership_on_every_boot() -> None:
    ...
    assert 'if [ -d "$CADOO_HOME/profiles" ]; then' in text
    assert 'chown -R cadoo:cadoo "$CADOO_HOME/profiles" 2>/dev/null || true' in text

    assert 'if [ -d "$CADOO_HOME/cron" ]; then' in text
    assert 'chown -R cadoo:cadoo "$CADOO_HOME/cron" 2>/dev/null || true' in text
