...
The fix is ``ENV HERMES_TUI_DIR=/opt/cadoo/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the cadoo user, return stdout."""
    inner = (
        "source /opt/cadoo/.venv/bin/activate && "
        "cd /opt/cadoo && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the cadoo user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "cadoo", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    ...
    assert r.stdout.strip() == "/opt/cadoo/ui-tui", (
        f"HERMES_TUI_DIR={r.stdout.strip()!r} (expected /opt/cadoo/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from cadoo_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/cadoo/ui-tui')\n"
