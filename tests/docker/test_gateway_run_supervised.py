...
Before the s6 migration, ``docker run nousresearch/cadoo gateway
...
Every ``docker exec`` runs as ``cadoo`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``cadoo gateway run`` invocation
        (not ``sleep infinity``).
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``cadoo gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``cadoo gateway
    ...
    # Count python processes running `cadoo gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*cadoo.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `cadoo gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-cadoo`
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${CADOO_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Cadoo Gateway Starting…"
    ...
    assert "⚕" in combined or "Cadoo Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Cadoo Gateway Starting" in file_contents, (
