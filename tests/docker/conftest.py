...
IMAGE_TAG = os.environ.get("HERMES_TEST_IMAGE", "cadoo-harness:latest")
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged cadoo user
# ---------------------------------------------------------------------------
#
# Background: every Cadoo runtime path inside the container drops to UID
# 10000 (the ``cadoo`` user) via ``s6-setuidgid cadoo``. ``docker exec``
...
# fails (PermissionError swallowed) for cadoo, so a test that ran as root
...
# helpers below run every probe under ``-u cadoo`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: cadoo).
