...
The shim (docker/cadoo-exec-shim.sh, installed at /opt/cadoo/bin/cadoo)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> cadoo login` would write /opt/data/auth.json as
...
failed: Cadoo is not logged into DooStudio Portal" on every message.

These tests verify:

1. ``docker exec <c> cadoo …`` (defaulting to root) gets dropped to the
   cadoo user before the real binary runs.
2. ``docker exec --user cadoo <c> cadoo …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $CADOO_HOME from a ``docker exec`` session land
   as cadoo:cadoo — the actual user-visible invariant.
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    We invoke `cadoo` with a Python-style `-c` shim equivalent — there's no
    pure-cadoo "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `cadoo` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `cadoo`
    do anything that writes to disk, then check the file's owner.

    Use `cadoo config set` which writes config.yaml under CADOO_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "cadoo", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by cadoo, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "cadoo:cadoo", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected cadoo:cadoo. "
        "The shim did not drop privileges before invoking cadoo."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user cadoo already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "cadoo", sleep_container,
         "cadoo", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user cadoo failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up cadoo:cadoo — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "cadoo", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_shim_opt_out_strict_truthiness(
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "cadoo", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "cadoo:cadoo", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected cadoo:cadoo"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/cadoo/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid cadoo cadoo <args>` which resolves `cadoo`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> cadoo login`` (root) wrote
    ...
    "Cadoo is not logged into DooStudio Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `cadoo config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is cadoo-readable.

    Specifically: pretend the operator ran `cadoo login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the cadoo UID. We use `cadoo auth list` since that touches the
    ...
    # `cadoo auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `cadoo config set` first to
    # provoke a write into CADOO_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "cadoo", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # CADOO_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "cadoo", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the cadoo user "
