"""Tests for cadoo_constants module."""
...
class TestGetHermesHome:
    """Tests for get_cadoo_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When CADOO_HOME is unset on Windows, use %LOCALAPPDATA%\\cadoo."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("CADOO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(cadoo_constants.sys, "platform", "win32")
        monkeypatch.setattr(cadoo_constants, "_profile_fallback_warned", False)

        assert get_cadoo_home() == local_appdata / "cadoo"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(cadoo_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(cadoo_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".cadoo"
