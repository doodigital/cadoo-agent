...
- Path resolution (absolute, relative to CADOO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp CADOO_HOME."""
    cadoo_home = tmp_path / ".cadoo"
    cadoo_home.mkdir()
    (cadoo_home / "cron").mkdir()
    (cadoo_home / "cron" / "output").mkdir()
    (cadoo_home / "scripts").mkdir()
    monkeypatch.setenv("CADOO_HOME", str(cadoo_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "HERMES_DIR", cadoo_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", cadoo_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", cadoo_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", cadoo_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Cadoo provider env (SECURITY.md §2.3)."""
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.cadoo/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
