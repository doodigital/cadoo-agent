...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    cadoo_state = ModuleType("cadoo_state")
    cadoo_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "cadoo_state", cadoo_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Cadoo"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Cadoo"
