...
binary download are exercised manually by `cadoo secrets bitwarden
...
def cadoo_home(tmp_path, monkeypatch):
    """Point Cadoo at an isolated home directory."""
    home = tmp_path / ".cadoo"
    home.mkdir()
    monkeypatch.setenv("CADOO_HOME", str(home))
    # Some modules cache get_cadoo_home; clear if needed.
    import cadoo_constants
    if hasattr(cadoo_constants, "_CADOO_HOME_CACHE"):
        cadoo_constants._CADOO_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(cadoo_home, monkeypatch):
...
def test_install_bws_happy_path(cadoo_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(cadoo_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(cadoo_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from cadoo_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under cadoo_home/cache/."""
    home = tmp_path / ".cadoo"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
