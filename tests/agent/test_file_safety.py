...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_hermes_env(self):
        """Cadoo' own .env inside CADOO_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: cadoo internal .env is in ~/.cadoo/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.cadoo/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.cadoo/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Cadoo cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        cadoo_home = tmp_path / ".cadoo"
        cache = cadoo_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._cadoo_home_path", return_value=cadoo_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Cadoo cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        cadoo_home = tmp_path / ".cadoo"
        hub = cadoo_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_cadoo_home(self, tmp_path):
        """The env basename guard does not depend on CADOO_HOME resolution."""
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        cadoo_home = tmp_path / ".cadoo"
        cache = cadoo_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
