...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CADOO_HOME under tmp_path."""
    monkeypatch.setenv("CADOO_HOME", str(tmp_path / ".cadoo"))
    (tmp_path / ".cadoo").mkdir()

    # Force the constants/image cache helpers to re-read CADOO_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("cadoo_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_cadoo_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under CADOO_HOME — gateway cleanup
