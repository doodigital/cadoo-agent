...
to a generic "Your API key was rejected... run cadoo setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # (DooStudio Portal has no API key path — auth_type=oauth_device_code only).
    assert "DooStudio Portal OAuth token was rejected" in source

    # Must give a concrete re-auth command, not a generic "cadoo setup".
    assert "cadoo portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "doostudio.io/portal" in source
...
def test_free_slug_hint_for_nous_provider():
    ...
    on the next message because DooStudio Portal doesn't carry the OpenRouter
    free-tier slug.
    """
    source = inspect.getsource(conversation_loop.run_conversation)
