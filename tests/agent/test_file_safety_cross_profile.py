"""Tests for the cross-Cadoo-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Cadoo profile's
...
Reference: May 2026 incident — a cadoo-security profile session
accidentally edited skills under both ~/.cadoo/profiles/cadoo-security/skills/
AND ~/.cadoo/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Cadoo root with two profiles, monkeypatch the
...
def fake_hermes(tmp_path, monkeypatch):
    ...
    sec_home = root / "profiles" / "cadoo-security"
...
def _set_active_home(monkeypatch, cadoo_home: Path):
    """Point file_safety._cadoo_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_cadoo_home_path", lambda: cadoo_home)
...
class TestResolveActiveProfileName:
    ...
    def test_named_profile(self, fake_hermes, monkeypatch):
        ...
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "cadoo-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        """If CADOO_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_cadoo_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["active_profile"] == "cadoo-security"
    ...
    def test_default_writing_security_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["target_profile"] == "cadoo-security"
