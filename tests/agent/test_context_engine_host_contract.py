...
engine plugins (e.g. cadoo-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``cadoo plugins`` picker."""
    from cadoo_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "cadoo_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "cadoo_cli.plugins.get_plugin_context_engine",
