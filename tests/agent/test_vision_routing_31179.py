...
def isolated_home(monkeypatch):
    """Temp CADOO_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="hermes_test_31179_")
    cadoo_home = os.path.join(test_home, ".cadoo")
    os.makedirs(cadoo_home)
...
def _fresh_modules():
    """Drop cached cadoo modules so each test reloads against current env."""
