...
""",
        ...
        from cadoo_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        cadoo_home = tmp_path / ".cadoo"
        cadoo_home.mkdir()
        (cadoo_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("CADOO_HOME", str(cadoo_home))
        from cadoo_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from cadoo_cli.plugins import PluginContext, PluginManifest, PluginManager
