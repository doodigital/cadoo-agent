...
Covers four shapes from Phase 4 of ``.cadoo/plans/2026-05-21-dashboard-oauth-auth.md``:
...
class TestConstruction:
    ...
    def test_name_and_display(self):
        ...
        assert p.name == "nous"
        assert p.display_name == "DooStudio"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            nous_plugin.NousDashboardAuthProvider(
                client_id="cadoo-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    ...
    def test_registers_with_default_portal_url_when_only_client_id_set(
        ...
        defaults to the production DooStudio Portal. The user shouldn't have
        ...
        assert registered._portal_url == "https://doostudio.io/portal"
    ...
    def test_skips_when_client_id_malformed(self, monkeypatch):
        monkeypatch.setenv("HERMES_DASHBOARD_OAUTH_CLIENT_ID", "cadoo-dashboard")
        ...
        assert "cadoo-dashboard" in nous_plugin.LAST_SKIP_REASON
...
class TestConfigYamlSource:
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``cadoo_cli.config.load_config``
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "cadoo_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        nous_plugin.register(ctx)
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://cadoo.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://cadoo.fly.dev/auth/callback"
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://cadoo.fly.dev/auth/callback")
