...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate CADOO_HOME for each test.

    The global hermetic fixture already redirects CADOO_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    CADOO_HOME here for clarity.
    """
...
class TestIsSafePath:
    def test_accepts_path_under_cadoo_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_cadoo_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_hermes_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/cadoo-abc/x.log")) is True
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "cadoo" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "cadoo":
                raise AssertionError("quick() descended into protected cadoo/")
...
class TestBundledDiscovery:
    def _write_enabled_config(self, cadoo_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from cadoo_cli import plugins as pmod
