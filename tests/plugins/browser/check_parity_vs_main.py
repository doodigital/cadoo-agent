...
Run from the PR worktree:
    ...
    cd ~/.cadoo/cadoo/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.cadoo/cadoo``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.cadoo/cadoo
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a cadoo checkout"
)
...
# Isolated CADOO_HOME for the config write.
