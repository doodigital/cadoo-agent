...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"cadoo[termux]"' in text
    assert '"cadoo[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"cadoo[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
