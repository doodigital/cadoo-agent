"""Tests for cadoo_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Cadoo entry point
(cadoo, cadoo, cadoo-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Cadoo entry point imports cadoo_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported cadoo_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("cadoo_bootstrap", None)
    import cadoo_bootstrap  # noqa: WPS433
    return cadoo_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Cadoo CLI
...
class TestEntryPointsImportBootstrap:
    """Every Cadoo entry point must import cadoo_bootstrap as its
    ...
    # Entry points that invoke Cadoo as a process.  Each one must
    # import cadoo_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "cadoo_cli/main.py",   # cadoo CLI (console_script)
        "run_agent.py",          # cadoo (console_script)
        "acp_adapter/entry.py",  # cadoo-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import cadoo_bootstrap' and that
        ...
        half-finished ``cadoo update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``cadoo_bootstrap`` as a top-level module) leaves cadoo
        ...
        """
        # Resolve relative to the cadoo repo root.  Tests live
        # at tests/test_cadoo_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # cadoo start even when cadoo_bootstrap hasn't been
        ...
        assert first_import_name == "cadoo_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'cadoo_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import cadoo_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Cadoo's own top-level modules — covering both
    ...
    def _run(self, hb, path_seed, env=None):
        ...
        try:
            ...
            hb.harden_import_path(src_root="/opt/cadoo")
            return sys.path[:]
        finally:
            sys.path[:] = original
    ...
    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/cadoo", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Cadoo root.  The guard must relocate Cadoo to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/cadoo"])
        assert result[0] == "/opt/cadoo"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Cadoo root.
        assert result.index("/opt/cadoo") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/cadoo", "/opt/cadoo", ""])
        assert result.count("/opt/cadoo") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["HERMES_PYTHON_SRC_ROOT"] = "/env/cadoo"
            hb.harden_import_path()
            assert sys.path[0] == "/env/cadoo"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HERMES_PYTHON_SRC_ROOT", None)
