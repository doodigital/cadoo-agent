...
file to be discoverable via ``cadoo sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.cadoo/sessions/saved/`` and prints
...
def cadoo_home(tmp_path, monkeypatch):
    ...
    # Clear any cached cadoo_home computation
    import cadoo_constants
    if hasattr(cadoo_constants, "_cadoo_home_cache"):
        cadoo_constants._cadoo_home_cache = None
    return home
...
def test_save_conversation_writes_under_cadoo_home(cadoo_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.cadoo/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the CADOO_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "cadoo_constants"]:
    ...
    cli.CadooCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("hermes_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.cadoo/sessions/saved/
    saved_dir = cadoo_home / "sessions" / "saved"
    ...
    assert "cadoo --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(cadoo_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "cadoo_constants"]:
        sys.modules.pop(mod, None)
    import cli
