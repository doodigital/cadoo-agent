...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("cadoo/cadoo-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned cadoo/* and pr-* branches."""

    def test_prunes_orphaned_hermes_branch(self, git_repo):
        """cadoo/cadoo-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "cadoo/cadoo-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "cadoo/cadoo-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "cadoo/cadoo-deadbeef" in result.stdout
        ...
        assert "cadoo/cadoo-deadbeef" in orphaned
        ...
        assert "cadoo/cadoo-deadbeef" not in result.stdout
