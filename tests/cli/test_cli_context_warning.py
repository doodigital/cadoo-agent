...
def _isolate(tmp_path, monkeypatch):
    """Isolate CADOO_HOME so tests don't touch real config."""
    home = tmp_path / ".cadoo"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal CadooCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = CadooCLI.__new__(CadooCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["cadoo-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Cadoo' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Cadoo' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Cadoo' minimum context length."""
