"""Tests for ACP Registry metadata shipped with Cadoo."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "cadoo"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Cadoo Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/DooStudio/cadoo"
    assert data["website"].startswith("https://doostudio.io/cadoo/")
    assert data["authors"] == ["DooStudio"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"cadoo[acp]=={data['version']}"
    assert uvx["args"] == ["cadoo-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"cadoo[acp]=={_pyproject_version()}"
    )
