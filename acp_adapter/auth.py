"""ACP auth helpers — detect and advertise Cadoo authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "cadoo-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Cadoo runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Cadoo can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Cadoo.
    ...
    not have Cadoo provider credentials configured yet, so Cadoo always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Cadoo using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Cadoo provider",
            description=(
                "Open Cadoo' interactive model/provider setup in a terminal. "
                "Use this when Cadoo has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
