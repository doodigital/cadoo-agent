"""CLI entry point for the cadoo ACP adapter.

Loads environment variables from ``~/.cadoo/.env``, configures logging
...
try:
    ...
    # shadowing Cadoo's own modules — ``cadoo acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    cadoo_bootstrap.harden_import_path()
...
def _load_env() -> None:
    ...
    if loaded:
        ...
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", cadoo_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="cadoo-acp",
        description="Run Cadoo Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Cadoo version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Cadoo provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.cadoo/node/ "
...
def _run_check() -> None:
    ...
    print("Cadoo ACP check OK")


def _run_setup() -> None:
    from cadoo_cli.main import main as hermes_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "cadoo", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``cadoo postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting cadoo ACP adapter")
