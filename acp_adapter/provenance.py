...
This is an additive Cadoo extension surfaced under ACP ``_meta.cadoo`` so
...
compression rotates the internal Cadoo head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.cadoo.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_hermes_session_id: The live internal Cadoo DB session id
    ...
    Returns:
        A dict suitable for ``{"cadoo": {"sessionProvenance": <dict>}}`` under
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"cadoo": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"cadoo": {"sessionProvenance": prov}}
