...
# 5. Symlinks the 'cadoo' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}🟣 Cadoo Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/cadoo-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink cadoo into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up cadoo command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/cadoo"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/cadoo"
echo -e "${GREEN}✓${NC} Symlinked cadoo → $COMMAND_LINK_DISPLAY_DIR/cadoo"
...
# ============================================================================
# Seed bundled skills into ~/.cadoo/skills/
# ============================================================================

HERMES_SKILLS_DIR="${CADOO_HOME:-$HOME/.cadoo}/skills"
mkdir -p "$HERMES_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.cadoo/skills/ ..."
...
echo "Other commands:"
echo "  cadoo status        # Check configuration"
if is_termux; then
    echo "  cadoo gateway       # Run gateway in foreground"
else
    echo "  cadoo gateway install # Install gateway service (messaging + cron)"
fi
echo "  cadoo cron list     # View scheduled jobs"
echo "  cadoo doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m cadoo_cli.main setup
fi
