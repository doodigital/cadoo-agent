...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.cadoo/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Cadoo profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale cadoo-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=cadoo=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Cadoo process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=cadoo-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a cadoo process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Cadoo
    process leaves its container behind permanently because there's no
    subsequent Cadoo process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=cadoo=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=cadoo-profile={_sanitize_label_value(profile_filter)}"])
...
#   SETUID/SETGID - the image's init drops from root to the 'cadoo'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``cadoo:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.cadoo/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. cadoo:latest) already use /init as PID 1
        ...
        container_name = f"cadoo-{uuid.uuid4().hex[:8]}"
        # Labels make cadoo-created containers identifiable to:
        #   * the orphan reaper (`cadoo=1` for the global sweep filter)
        #   * future cross-process reuse (`cadoo-task-id`, `cadoo-profile`)
        #   * operators running `docker ps --filter label=cadoo=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Cadoo profile is captured at
        ...
        label_args = [
            "--label", "cadoo=1",
            "--label", f"cadoo-task-id={task_label}",
            "--label", f"cadoo-profile={profile_name}",
        ]
        ...
        self._labels = {
            "cadoo": "1",
            "cadoo-task-id": task_label,
            "cadoo-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Cadoo process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Cadoo secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("cadoo-task-id", "")
        profile_label = self._labels.get("cadoo-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"cadoo-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``cadoo-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=cadoo=1",
                    "--filter", f"label=cadoo-task-id={task_label}",
                    "--filter", f"label=cadoo-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Cadoo process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Cadoo exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Cadoo process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Cadoo startup. That covers the
        ...
        # outlives Cadoo processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"cadoo-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``cadoo /quit``
