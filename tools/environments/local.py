...
from cadoo_cli._subprocess_compat import windows_hide_flags
...
# Cadoo-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# Cadoo-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Cadoo inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from cadoo_cli.config import OPTIONAL_ENV_VARS
...
def _inject_context_cadoo_home(env: dict) -> None:
    """Bridge the context-local Cadoo home override into subprocess env."""
    try:
        from cadoo_constants import get_cadoo_home_override

        value = get_cadoo_home_override()
        if value:
            env["CADOO_HOME"] = value
    except Exception:
        pass


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Cadoo-managed secrets from a subprocess environment."""
    try:
    ...
    _inject_context_cadoo_home(sanitized)

    from cadoo_constants import apply_subprocess_home_env
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\cadoo\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\cadoo\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _hermes_portable_git = os.path.join(_local_appdata, "cadoo", "git") if _local_appdata else ""
    ...
    raise RuntimeError(
        "Git Bash not found. Cadoo Agent requires Git for Windows on Windows.\n"
...
# Cached directory containing the ``cadoo`` console-script.
...
def _resolve_hermes_bin_dir() -> str | None:
    """Return the directory holding the ``cadoo`` console-script, or None.
    ...
    cadoo install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``cadoo``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``cadoo`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``cadoo`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("cadoo")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``cadoo`` executable (covers nix-store / venv wrappers).
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "cadoo" or base.startswith("cadoo."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "cadoo.exe" if _IS_WINDOWS else "cadoo"
...
def _prepend_hermes_bin_dir(existing_path: str) -> str:
    """Prepend the cadoo install dir to ``existing_path`` if it's missing.
...
def _make_run_env(env: dict) -> dict:
    ...
    if path_key is not None:
        ...
        # Ensure the cadoo install dir is reachable so plugins can shell out
        # to bare ``cadoo`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_hermes_bin_dir(new_path)

    _inject_context_cadoo_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Cadoo trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``CADOO_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under CADOO_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_cadoo_home() / "cache" / "terminal"
            except Exception:
