...
def iter_sync_files(container_base: str = "/root/.cadoo") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.cadoo to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, cadoo_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.cadoo/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="cadoo-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.cadoo/skills/a.md`` →
        ``~/.cadoo/skills/a.md``, a new remote file at
        ``/root/.cadoo/skills/b.md`` maps to ``~/.cadoo/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
