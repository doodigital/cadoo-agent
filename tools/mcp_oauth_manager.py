...
`mcp_tool.py`, and `cadoo_cli/mcp_config.py`. This module is the ONLY place
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `cadoo mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``cadoo mcp remove <name>`` and (indirectly) by
        ``cadoo mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
