...
Lets the LLM write a Python script that calls Cadoo tools via RPC,
...
# available on every platform Cadoo itself runs on.
...
# are non-secret runtime-location flags (the same set cadoo_cli treats as the
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Cadoo-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Cadoo-profile soft guard."""',
...
"""Auto-generated Cadoo tools RPC stubs."""
...
"""Auto-generated Cadoo tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    try:
        ...
        # Ensure the cadoo root is importable in the sandbox so
        ...
        # HERMES_TIMEZONE is an internal Cadoo setting and must not leak
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.cadoo/.env')).
...
def _load_config() -> dict:
    ...
    ``cadoo --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (cadoo's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``cadoo tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and cadoo's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.cadoo/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Cadoo tools programmatically. "
