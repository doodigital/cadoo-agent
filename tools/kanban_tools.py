...
orchestrator work. A normal ``cadoo chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``cadoo kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``cadoo kanban complete …``
   inside the container, where ``cadoo`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.cadoo/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``cadoo kanban …``), the dashboard
(``cadoo dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``cadoo chat`` without the kanban toolset see zero
...
def _connect(board: Optional[str] = None):
    ...
    the env-pinned active board without restarting Cadoo.
    """
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``cadoo-system`` and poison
