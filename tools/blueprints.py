...
`cadoo skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.cadoo.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.cadoo.blueprint`` is a mapping containing
    ...
    cadoo = meta.get("cadoo") if isinstance(meta, dict) else None
    blueprint = cadoo.get("blueprint") if isinstance(cadoo, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.cadoo.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.cadoo.blueprint`` block) they can hand
    to ``cadoo skills publish`` to share. ``body`` is the plain-language
