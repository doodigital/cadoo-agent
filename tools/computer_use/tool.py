...
`health_report` (surfaced by `cadoo computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Cadoo runs in).
_BLOCKED_KEY_COMBOS = {
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `cadoo computer-use install`. "
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of DooStudio/cadoo#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$CADOO_HOME/cache/vision/``
...
def check_computer_use_requirements() -> bool:
    ...
    `cadoo computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
