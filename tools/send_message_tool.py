    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.cadoo/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.cadoo/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: cadoo config set {home_env} <channel_id>"
            })
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    if platform == Platform.FEISHU and media_files:
        ...
        from cadoo_cli.plugins import discover_plugins as _dp_feishu
        _dp_feishu()
...
def _check_send_message():
    ...
    ``CADOO_HOME`` which has no ``gateway.pid``, so the gateway-running check
...
#   - the ``cadoo send`` CLI command (cadoo_cli/send_cmd.py)
