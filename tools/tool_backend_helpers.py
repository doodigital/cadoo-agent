...
def managed_nous_tools_enabled(*, force_fresh: bool = False) -> bool:
    ...
    Entitlement is paid DooStudio Portal service access OR a live free tool pool
...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `cadoo model` to refresh your "
        "DooStudio Portal login and billing status."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.cadoo/.env`` (via
    ``cadoo_cli.config.get_env_value`` when available) so tool-side
