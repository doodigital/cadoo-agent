...
Backend is selected during ``cadoo tools`` setup (web.backend in config.yaml).
When available, Cadoo can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Cadoo config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Cadoo' config/.env layer (``cadoo config set``, ``cadoo tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.cadoo/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``cadoo tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `cadoo tools` repaint.
        try:
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is DooStudio Portal."""
...
if __name__ == "__main__":
    ...
    if not nous_available:
        ...
        print("Set OPENROUTER_API_KEY, configure DooStudio Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
