...
# via shell expansions like $HOME or $CADOO_HOME, or by the resolved absolute
# active profile home path such as /home/cadoo/.cadoo/config.yaml. The
# resolved-absolute form is folded into the ~/.cadoo/ patterns at detection
...
# go stale when CADOO_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HERMES_ENV_PATH = (
    r'(?:~\/\.cadoo/|'
    r'(?:\$home|\$\{home\})/\.cadoo/|'
    r'(?:\$cadoo_home|\$\{cadoo_home\})/)'
    r'\.env\b'
)
# ~/.cadoo/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _HERMES_ENV_PATH; matches the CADOO_HOME override form as
# well as ~/.cadoo/.
_HERMES_CONFIG_PATH = (
    r'(?:~\/\.cadoo/|'
    r'(?:\$home|\$\{home\})/\.cadoo/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart cadoo gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "cadoo update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `cadoo gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart cadoo-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart cadoo-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(cadoo|gateway|cli\.py)\b', "kill cadoo/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill cadoo` but not
    # `kill -9 $(pgrep -f cadoo)` because the substitution is opaque
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Cadoo file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.cadoo/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Cadoo-managed security file (~/.cadoo/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Cadoo config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Cadoo config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Cadoo config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.cadoo/ form so the Cadoo config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/cadoo/.cadoo/config.yaml`) rather than
    # ~, $HOME, or $CADOO_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live CADOO_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_cadoo_home(command)
    return command
...
def _rewrite_resolved_cadoo_home(command: str) -> str:
    """Rewrite the resolved absolute Cadoo home prefix to ``~/.cadoo/``.

    Resolves the active ``CADOO_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.cadoo/`` so the static ``_HERMES_CONFIG_PATH`` / ``_HERMES_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from cadoo_constants import get_cadoo_home
        home = get_cadoo_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate CADOO_HOME (e.g. "/" or "") rewriting
        ...
        # /home/cadoo/.cadoo or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.cadoo/")
    return command
