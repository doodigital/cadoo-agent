...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.cadoo",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``CADOO_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside CADOO_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    cadoo_home = _resolve_cadoo_home()

    # Reject absolute paths — they bypass the CADOO_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to CADOO_HOME)",
            relative_path,
        )
        return False

    host_path = cadoo_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape CADOO_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, cadoo_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.cadoo/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="cadoo-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching cadoo_constants.get_hermes_dir().
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.cadoo``,
    SSH ``<remote_home>/.cadoo``, etc.) and whether translation is wanted.
