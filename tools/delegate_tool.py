...
# Must match cadoo_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (cadoo-* prefixed), and scenario toolsets.
#
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``cadoo-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on DooStudio Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. cadoo-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. DooStudio Portal).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.cadoo/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'cadoo auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (cadoo_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Cadoo subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
