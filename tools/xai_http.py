...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``cadoo tools`` repaint, tool-registration scans,
    ...
    2. ``~/.cadoo/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_cadoo_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.cadoo/.env`` first, then ``os.environ``.

    Wraps :func:`cadoo_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from cadoo_cli.config import get_env_value as _hermes_get_env_value
...
def hermes_xai_user_agent() -> str:
    """Return a stable Cadoo-specific User-Agent for xAI HTTP calls."""
    try:
        from cadoo_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Cadoo-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Cadoo-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``cadoo_cli.config.get_env_value`` so keys
    stored in ``~/.cadoo/.env`` (the standard Cadoo location) are honored —
