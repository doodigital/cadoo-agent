...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Cadoo doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in cadoo_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `cadoo model` uses — instead of
...
export interface ModelOptionProvider {
   ...
   *  the `cadoo model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `cadoo tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Cadoo); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/cadoo/update/check` — the backend's own update state.
