...
export function mediaStreamUrl(path: string): string {
  return `cadoo-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (cadoo_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.cadooDesktop!.api<{ data_url: string }>({
