...
import { getProfiles, transcribeAudio } from '@/cadoo'
...
async function readImageForRemoteAttach(
  ...
  const dataUrl = await window.cadooDesktop?.readFileDataUrl(filePath)
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.cadooDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `cadoo gateway` process performs the actual transfer, so we
