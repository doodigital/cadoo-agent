...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    ...
    description: 'Hosted Cadoo & Nous-trained models',
    docsUrl: 'https://doostudio.io/portal',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // cadoo_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Cadoo needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Cadoo stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Cadoo can read from one file request.',
  approvals: {
    mode: 'How Cadoo handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Cadoo updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
