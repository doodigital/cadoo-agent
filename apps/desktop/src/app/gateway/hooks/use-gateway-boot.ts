...
import { HermesGateway } from '@/cadoo'
...
import type { RpcEvent } from '@/types/cadoo'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.cadooDesktop>['getConnection']>> | null
  ) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.cadooDesktop
    ...
    // composer stayed disabled forever on "Starting Cadoo...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Cadoo…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Cadoo gateway". resolveGatewayWsUrl
