...
import type { HermesGateway } from '@/cadoo'
...
export function useGatewayRequest() {
  ...
  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.cadooDesktop>['getConnection']>> | null>(
    null
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}) => {
      ...
      if (!gateway) {
        throw new Error('Cadoo gateway unavailable')
      }
