...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { hermesDesktop: { readDir: typeof readDir } }).cadooDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { hermesDesktop?: unknown }).cadooDesktop
})
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { hermesDesktop: unknown }).cadooDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { hermesDesktop: unknown }).cadooDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.cadooDesktop is missing', async () => {
    delete (window as unknown as { hermesDesktop?: unknown }).cadooDesktop
