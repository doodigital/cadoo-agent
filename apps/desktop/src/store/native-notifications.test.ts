...
const initialHermesDesktop = desktopWindow.cadooDesktop
...
beforeEach(() => {
  notify.mockClear()
  desktopWindow.cadooDesktop = { notify } as unknown as Window['hermesDesktop']
...
afterEach(() => {
  if (initialHermesDesktop) {
    desktopWindow.cadooDesktop = initialHermesDesktop
  } else {
    delete desktopWindow.cadooDesktop
  }
})
...
describe('sendTestNativeNotification', () => {
  it('fires regardless of focus or active session', () => {
    setWindowState({ focused: true, hidden: false })
    setActiveSessionId('on-screen')
    sendTestNativeNotification('Cadoo', 'works')
    expect(notify).toHaveBeenCalledTimes(1)
  })
})
