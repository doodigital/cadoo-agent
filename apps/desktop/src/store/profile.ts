...
import { getProfiles, setApiRequestProfile } from '@/cadoo'
...
import type { ProfileInfo } from '@/types/cadoo'
...
// /api/profiles/active `current`). "default" is the root ~/.cadoo. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'cadoo.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'cadoo.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.cadooDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new CADOO_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.cadooDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.cadooDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'cadoo.desktop.showAllProfiles'
...
// Switch to the default (root ~/.cadoo) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.cadooDesktop?.touchBackend?.(target).catch(() => undefined)
}
