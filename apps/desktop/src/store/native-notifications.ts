...
const STORAGE_KEY = 'cadoo:native-notifications'
...
// "Backgrounded" = the user isn't on Cadoo. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.cadooDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.cadooDesktop
