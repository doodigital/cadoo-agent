 ...
 * app's bounds and stays visible while Cadoo is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'cadoo.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'cadoo.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.cadooDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.cadooDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.cadooDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.cadooDesktop?.petOverlay?.close()
}
