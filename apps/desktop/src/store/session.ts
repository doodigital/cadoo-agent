...
import type { SessionInfo, UsageStats } from '@/types/cadoo'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'cadoo.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'cadoo.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'cadoo.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'cadoo.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'cadoo.desktop.composer.fast'
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.cadooDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.cadooDesktop?.sanitizeWorkspaceCwd
