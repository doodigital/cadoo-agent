"""Shared constants for Cadoo Agent.
...
_UNSET = object()
_CADOO_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_CADOO_HOME_OVERRIDE", default=_UNSET
)


def set_cadoo_home_override(path: str | Path | None) -> Token:
    """Set a context-local Cadoo home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _CADOO_HOME_OVERRIDE.set(value)


def reset_cadoo_home_override(token: Token) -> None:
    """Restore the previous context-local Cadoo home override."""
    _CADOO_HOME_OVERRIDE.reset(token)


def get_cadoo_home_override() -> str | None:
    """Return the active context-local Cadoo home override, if any."""
    override = _CADOO_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_cadoo_home() -> Path:
    """Return the platform-native default Cadoo home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "cadoo"
    return Path.home() / ".cadoo"


def get_cadoo_home() -> Path:
    """Return the Cadoo home directory (default: platform-native path).

    Reads CADOO_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``CADOO_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``CADOO_HOME`` explicitly (see the systemd
    template in ``cadoo_cli/gateway.py`` and the kanban dispatcher in
    ``cadoo_cli/kanban_db.py``).  See https://github.com/DooStudio/cadoo/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_cadoo_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[CADOO_HOME fallback] CADOO_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass CADOO_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
...
def get_default_hermes_root() -> Path:
    ...
    (``~/.cadoo`` on POSIX, ``%LOCALAPPDATA%\\cadoo`` on native Windows).

    In Docker or custom deployments where ``CADOO_HOME`` points outside
    ``~/.cadoo`` (e.g. ``/opt/data``), returns ``CADOO_HOME`` directly
    — that IS the root.

    In profile mode where ``CADOO_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.cadoo/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_cadoo_home()
    env_home = os.environ.get("CADOO_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # CADOO_HOME is under ~/.cadoo (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — CADOO_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Cadoo is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_cadoo_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_cadoo_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<CADOO_HOME>/skills`` last-resort
    """
    ...
    if default is not None:
        return default
    return get_cadoo_home() / "skills"


def get_hermes_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Cadoo subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to CADOO_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to CADOO_HOME (e.g. ``"image_cache"``).
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Cadoo-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\cadoo\\node``. POSIX installs use
    ``$CADOO_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_cadoo_home()
    dirs = [root / "node"]
...
def find_hermes_node_executable(command: str) -> str | None:
    """Return a Cadoo-managed Node/npm executable path, if installed."""
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Cadoo-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring Cadoo-managed installs.

    This is for Cadoo-owned subprocesses that should not be broken by a bad,
...
def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Cadoo-managed Node directories prepended to PATH."""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``cadoo update`` — leaving a **dangling symlink** that ``which`` still
...
def display_cadoo_home() -> str:
    """Return a user-friendly display string for the current CADOO_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.cadoo``
        profile:  ``~/.cadoo/profiles/coder``
        custom:   ``/opt/cadoo-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.cadoo``.  For code that needs a real ``Path``, use
    :func:`get_cadoo_home` instead.
    """
    home = get_cadoo_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``CADOO_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/DooStudio/cadoo/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{CADOO_HOME}/home`` when the profile-home directory exists."""
    cadoo_home = get_cadoo_home_override() or (env or {}).get("CADOO_HOME") or os.getenv("CADOO_HOME")
    if not cadoo_home:
        return None
    profile_home = os.path.join(cadoo_home, "home")
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Cadoo profile HOME.

    ``CADOO_HOME`` scopes Cadoo state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={CADOO_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{CADOO_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{CADOO_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Cadoo' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under CADOO_HOME.

    Replaces the ``get_cadoo_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, cadoo_logging.py, cadoo_time.py, etc.).
    """
    return get_cadoo_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under CADOO_HOME."""
    return get_cadoo_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under CADOO_HOME."""
