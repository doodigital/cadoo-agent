...
const logDir = join(process.env.CADOO_HOME?.trim() || join(homedir(), '.cadoo'), 'logs')
...
// a real ~/.cadoo (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('cadoo-tui: parent lifecycle log unavailable\n')
    }
  }
}
