import type { RunExternalProcess } from '@cadoo/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`cadoo ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching cadoo: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`cadoo ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
