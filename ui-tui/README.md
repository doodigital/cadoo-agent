...
React + Ink terminal UI for Cadoo. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
- `src/app/setupHandoff.ts` — launches external `cadoo setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.cadoo/.cadoo_history` or under `CADOO_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `cadoo setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/cadoo-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external cadoo setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.cadoo/tui-parent.log
    ...
    types/
      cadoo-ink.d.ts            type declarations for @cadoo/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent CadooCLI subprocess for slash commands
```